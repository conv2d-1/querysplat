"""gs2x + **opacity / coverage fix** (finetune from gs2x checkpoint).

Inherits gs2x (encoder 504, ``gaussian_query_scale=2`` → DGS render ~1008).

Opacity-related changes vs ``..._ffgs_align_gs2x``:
  * ``init_opacity=0.85`` (was 0.5): sharp-zero-init starts nearer solid splats.
...
Init checkpoint (``trainer.load_from``): newest ``..._ffgs_align_gs2x_*`` run, else ffgs_align.
...
model = dict(
    ...
    model=dict(
        ...
        fuse_encoder=dict(
            ...
            pretrain='/mnt/nasTeam/AI/Autoresearch/Algorithm/Pretrain/MVS/da3_all/mvfr_da3g_rgb_all8stdy_251120_mvpdc_bs16_2f16f_100k_20251124-114829/checkpoint/latest/fuse_encoder.pth',
...
def _resolve_init_ckpt() -> str:
    """Finetune from newest gs2x run; fallback to ffgs_align / trainquality v2."""
    gs2x_dirs = sorted(
        glob.glob(
            os.path.join(
                _RESULTS_ROOT,
                "wfm_rgb_query_dual_4dgs_perpixel_trainquality_v2_wild_norm_ffgs_align_gs2x_*",
            )
        ),
        reverse=True,
    )
    for run_dir in gs2x_dirs:
        if "_opaque" in os.path.basename(run_dir):
            continue
        ckpt = os.path.join(run_dir, "checkpoint", "latest", "ckpt.pth")
        if os.path.isfile(ckpt):
            return ckpt

    patterns = [
        "wfm_rgb_query_dual_4dgs_perpixel_trainquality_v2_wild_norm_ffgs_align_",
        "wfm_rgb_query_dual_4dgs_perpixel_trainquality_v2_wild_norm_",
        "wfm_rgb_query_dual_4dgs_perpixel_trainquality_v2_",
        "wfm_rgb_query_dual_4dgs_perpixel_trainquality_",
    ]
    for pattern in patterns:
        run_dirs = sorted(
            glob.glob(os.path.join(_RESULTS_ROOT, pattern + "*")),
            reverse=True,
        )
        for run_dir in run_dirs:
            base = os.path.basename(run_dir)
            if "_gs2x_" in base or "_opaque" in base:
                continue
            ckpt = os.path.join(run_dir, "checkpoint", "latest", "ckpt.pth")
            if os.path.isfile(ckpt):
                return ckpt
    raise FileNotFoundError(
        "No init checkpoint found under "
        f"{_RESULTS_ROOT} (gs2x or patterns: {patterns})"
    )


_LOAD_FROM = _resolve_init_ckpt()
