...
_CHECKPOINT_PATH=os.path.join(_D4GS_ROOT, 'checkpoint', 'latest', 'ckpt.pth')
_RESULTS_ROOT=os.path.join(_D4GS_ROOT, 'results')
...
data=dict(
    train=os.path.join(_D4GS_ROOT, 'configs', 'local_train.yaml'),
    val=os.path.join(_D4GS_ROOT, 'configs', 'local_val.yaml'),
    vis=os.path.join(_D4GS_ROOT, 'configs', 'local_val.yaml'),
    basic=dict(
        sem_name='',
...
model=dict(
    ...
    model=dict(
        ...
        gaussian_query_scale=1.0,
        gaussian_query_patch_size=14,
        freeze_modules=['fuse_encoder'],
        chunk_size=30000,
        decoder_fp32=False,
        fuse_encoder=dict(
