    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            (
                ...
                else (nn.GroupNorm(1, in_channels) if in_norm == "layer_norm" else (nn.InstanceNorm2d(in_channels) if in_norm == "instance_norm" else nn.Identity()))
            ),
            activation_cls(),
            nn.Conv2d(
                in_channels,
            ...
            (
                ...
                else (nn.GroupNorm(1, hidden_channels) if hidden_norm == "layer_norm" else (nn.InstanceNorm2d(hidden_channels) if hidden_norm == "instance_norm" else nn.Identity()))
            ),
            activation_cls(),
            nn.Conv2d(
                hidden_channels,
...
class ConvStack(nn.Module):
    ...
    def forward(self, in_features: List[torch.Tensor], meta_data=None, patch_start_idx=None):
