...
from hAlgorithm.modules.models2.external.romav2.models.romav2.matcher import _compute_match_embeddings
...
class DPTMatcher(nn.Module):
    ...
    class Cfg:
        ...
        # mv config
        num_mv_layers: int = 2
        mv_feat_dim: int = 1024
        mv_project_mode: str = "conv" # conv conv_seperate none
        mv_embed_type: str = "merge" # merge split
    ...
    def __init__(self, cfg: Cfg):
        ...
        if self.cfg.mv_project_mode in ["conv"]:
            self.mv_project = nn.Conv2d(
                self.cfg.mv_feat_dim * self.cfg.num_mv_layers, cfg.dim, kernel_size=1, stride=1
            )
        elif self.cfg.mv_project_mode in ["conv_seperate"]:
            self.mv_project = nn.ModuleList([
                nn.Conv2d(
                    self.cfg.mv_feat_dim, cfg.dim, kernel_size=1, stride=1
                ) for i in range(cfg.num_mv_layers)
            ])
        else:
            assert self.cfg.mv_project_mode in ["None", "none", None]
            assert self.cfg.mv_feat_dim == self.cfg.dim
        logging.info(f"Using mv mode {self.cfg.mv_project_mode}, embed type {self.cfg.mv_embed_type}.")
        self.freeze()
    ...
    def _compute_head_preds(
        ...
        match_emb_AB: torch.Tensor | list[torch.Tensor],
        f_mv_A: torch.Tensor,
        img_A: torch.Tensor,
        img_B: torch.Tensor,
        enable_amp: bool,
        ...
        for h_i, mv_i in self.cfg.prompt_mv_flag:
            if isinstance(match_emb_AB, (tuple, list)):
                head_input[h_i] = head_input[h_i] + f_mv_A[mv_i] + match_emb_AB[mv_i]
            else:
                head_input[h_i] = head_input[h_i] + f_mv_A[mv_i] + match_emb_AB
        
        warp_and_confidence = self.head(head_input, img_A=img_A, img_B=img_B, enable_amp=enable_amp)
    ...
    def forward(
        ...
        f_mv_list_B: list[torch.Tensor],
        img_A: torch.Tensor,
        img_B: torch.Tensor,
        bidirectional: bool,
    ):
        preds = {}
        B, H_A, W_A, D_feat = f_list_A[0].shape
        B, H_B, W_B, D_feat = f_list_B[0].shape
        ...
        if self.cfg.mv_project_mode in ["conv"]:
            f_mv_A = [self.mv_project(torch.cat(f_mv_list_A, dim=-1).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)]
            f_mv_B = [self.mv_project(torch.cat(f_mv_list_B, dim=-1).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)]
        elif self.cfg.mv_project_mode in ["conv_seperate"]:
            f_mv_A = [self.mv_project[i](feat.permute(0, 3, 1, 2)).permute(0, 2, 3, 1) for i, feat in enumerate(f_mv_list_A)]
            f_mv_B = [self.mv_project[i](feat.permute(0, 3, 1, 2)).permute(0, 2, 3, 1) for i, feat in enumerate(f_mv_list_B)]
        elif self.cfg.mv_project_mode in ["None", "none", None]:
            f_mv_A = f_mv_list_A
            f_mv_B = f_mv_list_B
        else:
        ...
        # mv embed
        if self.cfg.mv_embed_type in ["merge"]:
            attn_AB_logits, attn_AB, match_emb_AB = _compute_match_embeddings(
                f_A=torch.cat(f_mv_A, dim=-1),
                f_B=torch.cat(f_mv_B, dim=-1),
        ...
        elif self.cfg.mv_embed_type in ["split"]:
            match_emb_AB = []
            for i in range(len(f_mv_A)):
                attn_AB_logits, attn_AB, _match_emb_AB = _compute_match_embeddings(
                    f_A=f_mv_A[i],
                    f_B=f_mv_B[i],
                    pos_emb_grid=pos_emb_grid,
                    temp=self.temp,
                ...
                match_emb_AB.append(_match_emb_AB)
        else:
            raise NotImplementedError()
 
        warp_AB, confidence_AB = self._compute_head_preds(
            f_list_A=f_list_A,
        ...
        if bidirectional:
            ...
            warp_BA, confidence_BA = self._compute_head_preds(
                f_list_A=f_list_B,
...
class RoMaV2MatchHead(nn.Module):
    @dataclass(frozen=False)
    class Cfg:
        matcher: DPTMatcher.Cfg = DPTMatcher.Cfg()
    ...
    def __init__(self, cfg: Cfg | None = None, **kwargs):
        ...
        import os
        self.matcher = DPTMatcher(cfg.matcher)
        self.cfg = cfg
    ...
    def _course_match(
        ...
        f_mv_A: list[torch.Tensor],
        f_mv_B: list[torch.Tensor],
        bidirectional: bool,
    ):
        ...
        matcher_output = self.matcher(
            feat_A, feat_B, f_mv_A, f_mv_B, img_A=None, img_B=None, bidirectional=bidirectional
        )
    ...
    def _forward_impl(
        ...
        # organize rgb feat to pair
        feat_A, feat_B = [], []
        for pair in pair_idx:
            feat_A.append(
                [feat[:, pair[0], ...] for feat in rgb_features]
            )
            feat_B.append(
                [feat[:, pair[1], ...] for feat in rgb_features]
            )
        num_feat = len(rgb_features)
        feat_A = [
            # B * Pair, P, C
        ...
        num_mv_feat = len(patch_feats)
        mv_feat_A = [
            # B * Pair, P, C
            torch.cat([feat[i] for feat in mv_feat_A], dim=0) for i in range(num_mv_feat)
        ]
        mv_feat_B = [
            # B * Pair, P, C
            torch.cat([feat[i] for feat in mv_feat_B], dim=0) for i in range(num_mv_feat)
        ]
        
        # coarse match
        predictions = self._course_match(feat_A, feat_B, mv_feat_A, mv_feat_B, bidirectional=bidirectional)
    ...
    def forward(
        ...
        **kwargs
        ...
        # split mvfr for rgb
        if self.cfg.split_feat_for_rgb is not None:
            if rgb_features is not None:
                pass
            else:
                rgb_features = [patch_feats[i] for i in self.cfg.split_feat_for_rgb]
        
        # select mvfr and rgb feat
        if self.cfg.use_feat_layers is not None:
            patch_feats = [patch_feats[i] for i in self.cfg.use_feat_layers]

        if self.cfg.use_rgb_layers is not None:
            rgb_features = [rgb_features[i] for i in self.cfg.use_rgb_layers]

        # reshape back to ph pw
        ...
        if not self.cfg.coarse_only:
            ...
            refine_feats = None
        return self._forward_impl(images, rgb_features, patch_feats, refine_feats, bidirectional, pair_idx)
