...
from hAlgorithm.modules.models2.external.vggt_omega.models.heads.utils import create_uv_grid, position_grid_to_embed
...
class DenseHead(nn.Module):
    ...
    def __init__(
        ...
        if self.depth_channel != 1:
            self.proj = _make_prediction_head(
                features,
                self.final_shuffle_factor**2 * self.depth_channel,
            )
        else:
            self.proj = _make_prediction_head(
                features,
        ...
        if pretrain is not None:
            ...
            logging.info(f"DenseHead, load pretrain {pretrain}")
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert aggregated_tokens_list[0].ndim == 4
        num_frames = aggregated_tokens_list[0].shape[1]

        if patch_token_start is None:
            raise ValueError("patch_token_start is required for DenseHead")

        # _, num_frames, _, _, _ = images.shape
        if frames_chunk_size is None or frames_chunk_size == 0 or frames_chunk_size >= num_frames:
            depth, depth_conf = self._forward_impl(aggregated_tokens_list, patch_h=patch_h, patch_w=patch_w, patch_token_start=patch_token_start, meta_data=meta_data)
            ...
            depth, depth_conf =  torch.cat(depth_chunks, dim=1), torch.cat(depth_conf_chunks, dim=1)
        
        if depth.shape[-1] == 1:
            return_dict = dict(depth=depth.squeeze(-1).unsqueeze(-3), confidence=depth_conf.unsqueeze(-3))
        elif depth.shape[-1] == 3:
            return_dict = dict(points=depth.permute(0, 1, 4, 2, 3).contiguous(), confidence=depth_conf.unsqueeze(-3))
        else:
            raise ValueError(f"output_dim {depth.shape[-1]} not supported")
    ...
    def _forward_impl(
        ...
        height, width = meta_data["input_height"][0], meta_data["input_width"][0]
        ...
        if self.depth_act == "exp":
            depth = torch.exp(depth_logits)
        elif self.depth_act == "inv_log":
            depth = torch.sign(depth_logits) * (torch.expm1(torch.abs(depth_logits)))
