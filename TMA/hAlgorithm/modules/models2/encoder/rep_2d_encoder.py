...
class DenseRepresentationEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # Init the base class
        super(DenseRepresentationEncoder, self).__init__()
        ...
        self.nomalize_mode = nomalize_mode
        ...
        if len(self.intermediate_dims) > 0:
            ...
            self.conv_in = nn.Conv2d(self.in_chans * (self.patch_size**2), self.enc_embed_dim, 3, 1, 1)
    ...
    def forward(self, input_data, **kwargs):
        ...
        Args:
            input_data: Input data for the encoder.
                If input type is ViTEncoderInput, input data must contain image normalization type and normalized image tensor.
                If input type is ViTEncoderNonImageInput, input data must be a tensor of size (B, C, H, W).

        Returns:
            features: Output data from the encoder.
        """
        if self.use_index is not None:
            input_data = input_data[..., self.use_index, :, :]
        ...
        if self.use_index is not None and self.nomalize_mode is not None:
            if self.nomalize_mode == 1:
                if len(self.use_index) == 2:
                    for bi in range(batch_size):
                        valid_mask = input_data[bi, 1].bool()
                        if valid_mask.sum() > 0:
                            valid = input_data[bi, 0][input_data[bi, 1].bool()].float()
                            vmax = torch.quantile(valid, q=0.98)
                            input_data[bi, 0, :, :] /= vmax
                            input_data[bi, 0, :, :] = torch.clamp(input_data[bi, 0, :, :], 0.0, 1.0)
                else:
...
if __name__ == "__main__":
    ...
    patch_embedder_output = patch_embedder(dummy_image)
