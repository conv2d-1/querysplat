...
sys.path.append(os.path.join(os.getcwd(), "hAlgorithm/modules/models2/external/romav2/models/dinov3"))
...
from hAlgorithm.modules.models2.external.romav2.models.dinov3.dinov3.layers import LayerScale, Mlp, PatchEmbed, RMSNorm, RopePositionEmbedding, SelfAttentionBlock, SwiGLUFFN
from hAlgorithm.modules.models2.external.romav2.models.dinov3.dinov3.utils import named_apply
...
class DinoVisionTransformer(nn.Module):
    ...
    def get_intermediate_layers(
        ...
        *,
        ...
        if reshape:
            B, _, h, w = x.shape
            outputs = [out.reshape(B, h // self.patch_size, w // self.patch_size, -1).permute(0, 3, 1, 2).contiguous() for out in outputs]
...
class Dinov3Encoder(nn.Module):
    ...
    def build_dino(self):
        self.dinov3 = eval(self.model_name)(pretrained=False)

        if self.pretrain is not None:
            if self.pretrain.endswith(".pth"):
                res = self.dinov3.load_state_dict(
                    torch.load(self.pretrain, map_location="cpu", weights_only=False),
                    strict=self.pertrain_strict,
                )
            else:
                from safetensors.torch import load_file
                state_dict = load_file(self.pretrain)
                res = self.dinov3.load_state_dict(state_dict, strict=self.pertrain_strict)
    ...
    def forward(self, x, meta_data=None, **kwargs):
        ...
        features = self.dinov3.get_intermediate_layers(x, n=self.layer_idxs)
        return features
