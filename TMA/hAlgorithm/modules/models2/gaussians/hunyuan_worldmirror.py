...
from hAlgorithm.modules.models2.external.anysplat.model.encoder.vggt.utils.pose_enc import extri_intri_to_pose_encoding
from hAlgorithm.utils import instantiate_from_config

from hAlgorithm.modules.models2.external.worldmirror.models.models.rasterization import Rasterizer

from hAlgorithm.modules.models2.external.worldmirror.models.utils.frustum import calculate_unprojected_mask
from hAlgorithm.modules.models2.external.worldmirror.models.utils.geometry import depth_to_world_coords_points, closed_form_inverse_se3
from hAlgorithm.modules.models2.external.worldmirror.models.utils.camera_utils import vector_to_camera_matrices
from hAlgorithm.modules.models2.external.worldmirror.models.utils import sh_utils, act_gs
from hAlgorithm.modules.models2.external.worldmirror.models.heads.dense_head import DPTHead

from hAlgorithm.modules.models2.external.worldmirror.utils.save_utils import save_gs_ply
...
class Gaussians:
    ...
    quats: Float[Tensor, "gaussian 4"]
    colors: Float[Tensor, "gaussian 3"]
...
class GaussianSplatRenderer(nn.Module):
    ...
    def render(
        ...
        chunk_size = 4
        for i in range(0, images.shape[1], chunk_size):
            end_idx = min(i + chunk_size, images.shape[1])
...
class FFGS(nn.Module):
    ...
    def __init__(
        ...
        gs_dim=256, 
        pretrain=None,
        gs_head_pretrain=None,
        gs_renderer_pretrain=None,
    ):
        ...
        self.gs_head_pretrain = gs_head_pretrain
        self.gs_renderer_pretrain = gs_renderer_pretrain
        ...
        if self.gs_head_pretrain is not None:
            self.gs_head.load_state_dict(
                torch.load(self.gs_head_pretrain, map_location="cpu", weights_only=False),
                strict=True,
            )
            logging.info(f"FFGS gs_head, load pretrain {self.gs_head_pretrain}")
        
        if self.gs_renderer_pretrain is not None:
            self.gs_renderer.load_state_dict(
                torch.load(self.gs_renderer_pretrain, map_location="cpu", weights_only=False),
                strict=True,
            )
            logging.info(f"FFGS gs_renderer, load pretrain {self.gs_renderer_pretrain}")

    def render(self, gaussians, extrinsics, intrinsics, image_shape, **kwargs):
        height, width = image_shape
        extrinsics = extrinsics.detach()
        intrinsics = intrinsics.detach()
        intrinsics[..., 0, :] *= width  # NOTE: without normalized intrinsics
        intrinsics[..., 1, :] *= height
        
        rendered_colors_list, rendered_depths_list, rendered_alphas_list = [], [], []
        chunk_size = 4
        for i in range(0, extrinsics.shape[1], chunk_size):
            end_idx = min(i + chunk_size, extrinsics.shape[1])

            rendered_colors, rendered_depths, rendered_alphas = self.gs_renderer.rasterizer.rasterize_batches(
                [gaussians.means], [gaussians.quats], [gaussians.scales], [gaussians.opacities], [gaussians.colors.unsqueeze(-2)],
                extrinsics[:, i:end_idx], intrinsics[:, i:end_idx],
                width=width, height=height,
                sh_degree= self.gs_renderer.sh_degree,
            )
            rendered_colors_list.append(rendered_colors)
            rendered_depths_list.append(rendered_depths)
            # rendered_alphas_list.append(rendered_alphas)

        rendered_colors = torch.cat(rendered_colors_list, dim=1).permute(0, 1, 4, 2, 3).contiguous()
        rendered_depths = torch.cat(rendered_depths_list, dim=1).squeeze(-1)
        # rendered_alphas = torch.cat(rendered_alphas_list, dim=1)
 
        return dict(render_rgb=rendered_colors, render_depth=rendered_depths)
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            preds = dict(
                gs_depth=gs_depth,
                gs_depth_conf=gs_depth_conf,
                camera_params=pose_enc,
            )
            views = dict(
                camera_pose=w2c.inverse(),
            ...
            results = dict(
                depth=gs_depth.squeeze(-1).unsqueeze(2),
                confidence=gs_depth_conf.unsqueeze(2),
                render_rgb=preds["rendered_colors"].permute(0, 1, 4, 2, 3).contiguous(), 
                render_depth=preds["rendered_depths"].squeeze(-1)
            )
            if not self.training:
                results["gaussians"] = Gaussians(
                    # Get Gaussian parameters (already filtered by GaussianSplatRenderer)
                    means = preds["splats"]["means"][0].reshape(-1, 3),
                    scales = preds["splats"]["scales"][0].reshape(-1, 3),
                    quats = preds["splats"]["quats"][0].reshape(-1, 4),
                    colors = (preds["splats"]["sh"][0] if "sh" in preds["splats"] else preds["splats"]["colors"][0]).reshape(-1, 3),
                    opacities = preds["splats"]["opacities"][0].reshape(-1),
                )
