...
def pose_encoding_to_camera(
    ...
    if pose_encoding_type == "absT_quaR":
        if pose_encoding.shape[-1] != 7:
            raise ValueError(f"Expected last dimension to be 7 for 'absT_quaR', got {pose_encoding.shape[-1]}")
        
        abs_T = pose_encoding[..., :3]          # (..., 3)
        quaternion_R = pose_encoding[..., 3:7]  # (..., 4)
        R = quaternion_to_matrix(quaternion_R)  # (..., 3, 3)
    else:
        raise ValueError(f"Unknown pose encoding type: {pose_encoding_type}")

    # Create identity matrix with same device/dtype, then expand to match batch shape
    *batch_shape, _, _ = R.shape  # R is (..., 3, 3)
    c2w_mats = torch.eye(4, 4, dtype=R.dtype, device=R.device)  # (4, 4)
    c2w_mats = c2w_mats.expand(*batch_shape, 4, 4).clone()      # (..., 4, 4)

    # Fill in rotation and translation
    c2w_mats[..., :3, :3] = R        # (..., 3, 3)
    c2w_mats[..., :3, 3] = abs_T     # (..., 3)
