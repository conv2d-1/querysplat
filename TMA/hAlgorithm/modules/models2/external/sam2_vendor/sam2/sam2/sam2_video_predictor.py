...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_mask(
        ...
        consolidated_out = self._consolidate_temp_output_across_obj(
            ...
            is_cond=is_cond,
            consolidate_at_video_res=True,
            with_object_ious=with_object_ious,
        )
        _, video_res_masks = self._get_orig_video_res_output(
            inference_state, consolidated_out["pred_masks_video_res"]
        )
        if with_object_ious:
            ...
            return frame_idx, obj_ids, video_res_masks, consolidated_out['object_ious']
        else:
            return frame_idx, obj_ids, video_res_masks
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if with_object_ious:
            consolidated_out["object_ious"] = torch.full(
                size=(batch_size, 1),
                fill_value=NO_OBJ_SCORE,
                dtype=torch.float32,
                device=inference_state["storage_device"],
        ...
        for obj_idx in range(batch_size):
            ...
            if with_object_ious:
                consolidated_out["object_ious"][obj_idx : obj_idx + 1] = out["object_ious"]
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            if with_object_ious:
                object_ious_per_obj = [None] * batch_size
            for obj_idx in range(batch_size):
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                ...
                if with_object_ious:
                    object_ious_per_obj[obj_idx] = current_out['object_ious']
            # import ipdb; ipdb.set_trace()
            ...
            if with_object_ious:
                all_object_ious = torch.cat(object_ious_per_obj, dim=0)
                yield frame_idx, obj_ids, video_res_masks, all_object_ious
            else:
    ...
    def _run_single_frame_inference(
        ...
        compact_current_out = {
            ...
            "object_ious": current_out["object_ious"]
        }
