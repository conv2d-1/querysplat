...
def _compute_head_preds(
    *,
    ...
    head_input = f_list_A
    if f_mv_A is not None:
        head_input[-1] = f_list_A[-1] + f_mv_A + match_emb_AB
    else:
        head_input[-1] = f_list_A[-1] + match_emb_AB
    warp_and_confidence = head(head_input, img_A=img_A, img_B=img_B, enable_amp=enable_amp)
