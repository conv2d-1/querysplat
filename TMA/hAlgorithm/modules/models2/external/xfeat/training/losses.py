...
from .utils import crop_patches, subpix_softmax2d
...
def keypoint_position_loss(kpts1, kpts2, pts1, pts2, softmax_temp=1.0):
    ...
    with torch.no_grad():
        ...
        kpts1_offsets_xy = torch.cat([kpts1_offsets_x.unsqueeze(-1), kpts1_offsets_y.unsqueeze(-1)], dim=-1)
        # pdb.set_trace()
    ...
    loss = F.nll_loss(kpts1_selected, labels1, reduction="mean") + F.nll_loss(kpts2_selected, labels2, reduction="mean")
...
def hard_triplet_loss(X, Y, margin=0.5):
    ...
    dist_neg = dist_mat + 100.0 * torch.eye(*dist_mat.size(), dtype=dist_mat.dtype, device=dist_mat.get_device() if dist_mat.is_cuda else torch.device("cpu"))
