...
def plot_corrs(p1, p2, src_pts, tgt_pts, path=None):
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(len(p1)):
            with torch.no_grad():
                ...
                mask_valid = (src_pts[:, 0] >= 0) & (src_pts[:, 1] >= 0) & (src_pts[:, 0] < rw) & (src_pts[:, 1] < rh)
                ...
                mask_valid = mask1[batch_idx, src_pts[:, 1].long(), src_pts[:, 0].long()] & mask2[batch_idx, tgt_pts[:, 1].long(), tgt_pts[:, 0].long()]
                tgt_pts = tgt_pts[mask_valid]
                src_pts = src_pts[mask_valid]
                ...
                mask_valid1 = (src_pts[:, 0] >= (0 + padto)) & (src_pts[:, 1] >= (0 + padto)) & (src_pts[:, 0] < (w - padto)) & (src_pts[:, 1] < (h - padto))
                mask_valid2 = (tgt_pts[:, 0] >= (0 + padto)) & (tgt_pts[:, 1] >= (0 + padto)) & (tgt_pts[:, 0] < (w - padto)) & (tgt_pts[:, 1] < (h - padto))
