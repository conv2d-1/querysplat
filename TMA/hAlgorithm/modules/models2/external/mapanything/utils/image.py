...
from .cropping import crop_resize_if_necessary
from .geometry import recover_pinhole_intrinsics_from_ray_directions
from hAlgorithm.modules.models2.external.uniception.models.encoders.image_normalizations import IMAGE_NORMALIZATION_DICT
...
def load_images(
    ...
    for path, img, W1, H1 in loaded_images:
        if cameras is not None:
            camera = cameras[path]
            c2w = camera.get("c2w", None)
            intrinsics = camera.get("intrinsics", None)
        else:
            c2w = intrinsics = None

        # Resize and crop the image to the target size
        res = crop_resize_if_necessary(img, resolution=target_size, intrinsics=intrinsics)
        img = res[0]
        ...
        if c2w is not None:
            imgs[-1]["camera_poses"] = torch.from_numpy(c2w).float()[None] if isinstance(c2w, np.ndarray) else c2w[None]
        if intrinsics is not None:
            imgs[-1]["intrinsics"] = torch.from_numpy(intrinsics).float()[None] if isinstance(intrinsics, np.ndarray) else intrinsics[None]
