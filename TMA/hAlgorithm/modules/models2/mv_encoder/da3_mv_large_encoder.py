...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], prompt_depth=None, prompt_ray=None, input_tokens=None, **kwargs):
        B, S, _, H, W = x.shape
    ...
    def process_attention(self, i, x, block, attn_type="global", pos=None, attn_mask=None):
        ...
        if self.use_checkpoint_start != -1 and i >= self.use_checkpoint_start:
    ...
    def get_intermediate_layers(
        ...
        **kwargs,
        ...
        outputs, aux_outputs, _ = self._get_intermediate_layers_not_chunked(
...
class DinoVisionTransformerWithTimeToken(DinoVisionTransformer):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], prompt_depth=None, time_token=None, input_tokens=None, **kwargs):
        B, S, _, H, W = x.shape
...
class DinoV2WithTimeToken(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for _ in range(self.depth):
            self.pretrained.append(
                DinoVisionTransformerWithTimeToken(
                    img_size=518,
                    patch_size=patch_size,
        ...
        if pretrained_pretrain is not None:
            state_dict = torch.load(pretrained_pretrain, map_location="cpu", weights_only=False)
            for module in self.pretrained:
                res = module.load_state_dict(state_dict, strict=pertrain_strict)
    ...
    def forward(self, x, **kwargs):
        ...
        time_token = kwargs.get("time_token", None)
        tokens = None
        all_outputs = []
        for i in range(self.depth):
            enc = self.pretrained[i]
            raw_outputs, _, tokens = enc._get_intermediate_layers_not_chunked(
                x, self.out_layers, input_tokens=tokens, time_token=time_token, **kwargs,
            )
            if raw_outputs[0][1].shape[-1] == enc.embed_dim:
                outputs = [enc.norm(out[1]) for out in raw_outputs]
            elif raw_outputs[0][1].shape[-1] == (enc.embed_dim * 2):
                outputs = [
                    torch.cat(
                        [out[1][..., :enc.embed_dim], enc.norm(out[1][..., enc.embed_dim:])],
                        dim=-1,
                    )
                    for out in raw_outputs
                ]
            else:
                raise ValueError(f"Invalid output shape: {raw_outputs[0][1].shape}")
            all_outputs.extend(outputs)

        patch_start_idx = 1 + self.pretrained[0].num_register_tokens + (1 if time_token is not None else 0)
        return all_outputs, None, patch_start_idx
...
class DinoV2(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.depth):
            ...
            # if tokens is not None, it means the tokens are already prepared, so we don't need to prepare them again
            if tokens is not None:
                kwargs.pop("cam_token", None)

            raw_outputs, _, tokens = enc._get_intermediate_layers_not_chunked(
                x, self.out_layers, input_tokens=tokens, **kwargs,
            )
            if raw_outputs[0][1].shape[-1] == enc.embed_dim:
                outputs = [enc.norm(out[1]) for out in raw_outputs]
            elif raw_outputs[0][1].shape[-1] == (enc.embed_dim * 2):
        ...
        patch_start_idx = 1 + self.pretrained[0].num_register_tokens
        return all_outputs, None, patch_start_idx
