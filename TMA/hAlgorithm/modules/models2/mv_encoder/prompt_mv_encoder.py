...
class MVEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.num_register_tokens > 0:
            self.register_token = nn.Parameter(torch.randn(1, 2, num_register_tokens, fuse_embed_dim))
        ...
        if self.dino_pretrain is not None:
            dino_ckpt = torch.load(self.dino_pretrain, map_location="cpu", weights_only=False)
            dino_ckpt = self._extract_state_dict(dino_ckpt)
            self._init_blocks_from_dino(dino_ckpt)
            logging.info(f"MixVIT, initialized frame/global blocks from dino_pretrain {self.dino_pretrain}")
        ...
        if self.fuse_embed_dim > 0:
            self.fuse_project = nn.Linear(self.embed_dim + self.prompt_embed_dim, self.fuse_embed_dim, bias=True)
    ...
    def _init_blocks_from_dino(self, dino_ckpt: dict):
        ...
        """
        logging.info("Initializing blocks from DINOv2 pretrained weights")
        
        dino_block_keys = [k for k in dino_ckpt.keys() if k.startswith("blocks.")]
        if not dino_block_keys:
            logging.warning("No 'blocks' found in DINO checkpoint")
            return
        ...
        logging.info(f"Found {num_dino_blocks} blocks in DINO checkpoint")
        if num_dino_blocks == 0:
            logging.warning("No valid block index found in DINO checkpoint keys")
            return
        ...
        for i, block in enumerate(self.frame_blocks):
            ...
            if block_state_dict:
                ...
                if i == 0:
                    logging.info(f"Frame block 0: Missing keys count: {len(missing)}, Unexpected keys count: {len(unexpected)}")
                    if missing:
                        logging.info(f"Frame block 0 missing keys: {missing}")
                    if unexpected:
                        logging.info(f"Frame block 0 unexpected keys: {unexpected}")
        ...
        for i, block in enumerate(self.global_blocks):
            ...
            if block_state_dict:
                ...
                if i == 0:
                    logging.info(
                        f"Global block 0: Missing keys count: {len(missing)}, Unexpected keys count: {len(unexpected)}"
                    )
                    if missing:
                        logging.info(f"Global block 0 missing keys: {missing}")
                    if unexpected:
                        logging.info(f"Global block 0 unexpected keys: {unexpected}")
        
        logging.info("Successfully initialized blocks from DINOv2 weights")

    def forward(self, patch_tokens, prompt_depth, meta_data, **kwargs):
        ...
        if self.prompt_in_chans > 0 and prompt_depth is not None:
            prompt_tokens = self.prompt_patch_embed(prompt_depth)
            if self.prompt_add:
        ...
        for di in range(self.depth):
            ...
            if (self.hooks is not None and di in self.hooks) or (self.hooks is None and di == self.depth - 1):
                ...
                if self.output_frame:
                    output_list.append(frame_intermediates)
                elif self.output_global:
                    output_list.append(global_intermediates)
                else:
