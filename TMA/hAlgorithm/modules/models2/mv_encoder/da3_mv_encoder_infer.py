...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None, cls_token=None, prompt_depth=None, **kwargs):
        B, S, nc, w, h = x.shape
        x = rearrange(x, "b s c h w -> (b s) c h w")
        x = self.patch_embed(x)

        if self.prompt_start == -1 and prompt_depth is not None:
            if self.prompt_patch_embed is not None:
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], prompt_depth=None, memory_efficient_infer=False, **kwargs):
        B, S, _, H, W = x.shape
        ...
        if memory_efficient_infer:
            bar = tqdm(enumerate(self.blocks), total=len(self.blocks), desc="mv encoder blocks")
        else:
            bar = enumerate(self.blocks)
        ...
        for i, blk in bar:
            ...
            if i in blocks_to_take:
                ...
                if memory_efficient_infer:
                    output.append(out_x.detach().cpu())
                else:
                    output.append(out_x)
