...
from hAlgorithm.modules.models2.external.vggt_omega.models.layers import Mlp, RopePositionEmbedding, SelfAttentionBlock
from hAlgorithm.modules.models2.external.vggt_omega.models.layers.vision_transformer import DinoVisionTransformer
...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        self.pre_denormalize = pre_denormalize
    ...
    def forward(self, images, meta_data=None, memory_efficient_infer=False, **kwargs):
        ...
        return outputs, frame_rope, self.patch_token_start
    ...
    def _run_frame_block(
        ...
        can_checkpoint = self.use_checkpoint and self.training and torch.is_grad_enabled() and tokens.requires_grad
        if can_checkpoint:
            tokens = checkpoint(lambda x: self.frame_blocks[block_idx](x, rope_sincos), tokens, use_reentrant=False)
        else:
    ...
    def _run_inter_frame_attention_block(
        ...
        tokens = tokens.view(batch_size, num_frames, num_tokens, embed_dim)
        inter_frame_block = self.inter_frame_blocks[block_idx]
        can_checkpoint = self.use_checkpoint and self.training and torch.is_grad_enabled() and tokens.requires_grad

        if attention_type == "global":
            tokens = tokens.view(batch_size, num_frames * num_tokens, embed_dim)
            if can_checkpoint:
                tokens = checkpoint(lambda x: self.inter_frame_blocks[block_idx](x, None), tokens, use_reentrant=False)
            else:
        ...
        if can_checkpoint:
            camera_and_register_tokens = checkpoint(
                lambda x: inter_frame_block(x, None),
                camera_and_register_tokens,
                use_reentrant=False,
            )
        else:
            camera_and_register_tokens = inter_frame_block(camera_and_register_tokens, None)
