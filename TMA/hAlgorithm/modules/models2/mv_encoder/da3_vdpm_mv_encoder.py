...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        self.time_conditioning_token = nn.Parameter(torch.randn(1, 1, 1, embed_dim))

        self.patch_start_idx = 2 + self.num_register_tokens # camera token + time conditioning token + rigister tokens
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], prompt_depth=None, prompt_ray=None, **kwargs):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # NOTE: add time conditioning token
            if self.alt_start != -1 and i == self.alt_start:
                if kwargs.get("time_token", None) is not None:
                    time_token = kwargs.get("time_token")
                else:
                    time_token = self.time_conditioning_token.expand(B, S, *self.time_conditioning_token.shape[2:])
                x = torch.cat([x[:, :, 0:1], time_token, x[:, :, 1:]], dim=2)
            ...
            if self.prompt_ray_start != -1 and i == self.prompt_ray_start:
                ...
                x[:, :, self.patch_start_idx:] += prompt_ray
