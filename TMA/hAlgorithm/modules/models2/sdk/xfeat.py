...
from hAlgorithm.modules.models2.external.xfeat.interpolator import InterpolateSparse2d
from hAlgorithm.modules.models2.external.xfeat.model import BasicLayer
...
class XFeatModel(nn.Module):
    ...
    def __init__(self, inputs_normalize=True, inputs_channels=1):
        super().__init__()
        ...
        self.skip1 = nn.Sequential(nn.AvgPool2d(4, stride=4), nn.Conv2d(self.inputs_channels, 24, 1, stride=1, padding=0))

        self.block1 = nn.Sequential(
            BasicLayer(self.inputs_channels, 4, stride=1),
        ...
        self.keypoint_head = nn.Sequential(
            BasicLayer(64 * self.inputs_channels, 64, 1, padding=0),
            BasicLayer(64, 64, 1, padding=0),
            BasicLayer(64, 64, 1, padding=0),
            nn.Conv2d(64, 65, 1),
        )
    ...
    def forward(self, x, ray_directions=None, ray_world=None):
        """
        input:
...
class XFeat(nn.Module):
    ...
    def __init__(self, weights=None, top_k=4096, detection_threshold=0.05, downsample=8, inputs_normalize=True, with_ray_directions=False, with_ray_in_world=False, **kwargs):
        super().__init__()

        self.inputs_normalize = inputs_normalize
        self.with_ray_directions = with_ray_directions
        self.with_ray_in_world = with_ray_in_world
        self.net_inputs_channels = 1 if self.inputs_normalize else 3
        if self.with_ray_directions:
            self.net_inputs_channels += 3
        if self.with_ray_in_world:
            self.net_inputs_channels += 6

        self.net = XFeatModel(inputs_normalize=self.inputs_normalize, inputs_channels=self.net_inputs_channels)
        self.top_k = top_k
        ...
        if weights is not None:
            if isinstance(weights, str):
                logging.info("XFeat, loading weights from: " + weights)
    ...
    def detectAndCompute(self, x, top_k=None, detection_threshold=None):
        ...
        valid = scores > 0
        return [{"keypoints": mkpts[b][valid[b]], "scores": scores[b][valid[b]], "descriptors": feats[b][valid[b]]} for b in range(B)]
    ...
    def forward(
        ...
        ray_directions=None,
        ray_world=None,
        ray_directions_image0=None,
        ray_directions_image1=None,
        ray_world_image0=None,
        ray_world_image1=None,
    ):
        if self.training:
            if image is not None and image.ndim == 5:
                inputs = [image]
                if self.with_ray_directions and ray_directions is not None:
                    inputs.append(ray_directions)
                if self.with_ray_in_world and ray_world is not None:
                    inputs.append(ray_world)

                if len(inputs) > 1:
                    image = torch.cat(inputs, dim=2)

                B, V = image.shape[:2]

                image = image.reshape(B * V, *image.shape[-3:])
                feats, kpts, hmap = self.net(image)

                feats = feats.reshape(B, V, *feats.shape[1:])
                kpts = kpts.reshape(B, V, *kpts.shape[1:])
                hmap = hmap.reshape(B, V, *hmap.shape[1:])

                feats1 = feats[:, 0:1].expand(-1, V - 1, -1, -1, -1).reshape(-1, *feats.shape[-3:])
                kpts1 = kpts[:, 0:1].expand(-1, V - 1, -1, -1, -1).reshape(-1, *kpts.shape[-3:])
                hmap1 = hmap[:, 0:1].expand(-1, V - 1, -1, -1, -1).reshape(-1, *hmap.shape[-3:])

                feats2 = feats[:, 1:].reshape(-1, *feats.shape[-3:])
                kpts2 = kpts[:, 1:].reshape(-1, *kpts.shape[-3:])
                hmap2 = hmap[:, 1:].reshape(-1, *hmap.shape[-3:])

            else:
                assert image0.ndim == 4 and image1.ndim == 4
                feats1, kpts1, hmap1 = self.net(image0)
                feats2, kpts2, hmap2 = self.net(image1)

            coords1_list = []

            for b in range(len(positives)):
                if len(positives[b]) > 1:
                    ...
                    coords1_list.append(None)

            outputs = dict(
                coords1=coords1_list,
                kpts1=kpts1,
                kpts2=kpts2,
                feats1=feats1,
                feats2=feats2,
            ...
            inputs0, inputs1 = [image0], [image1]
            if self.with_ray_directions and ray_directions_image0 is not None:
                inputs0.append(ray_directions_image0)
                inputs1.append(ray_directions_image1)
            if self.with_ray_in_world and ray_world_image0 is not None:
                inputs0.append(ray_world_image0)
                inputs1.append(ray_world_image1)

            if len(inputs0) > 1:
                image0 = torch.cat(inputs0, dim=1)
                image1 = torch.cat(inputs1, dim=1)

            matches = self.match_xfeat_star(im_set1=image0, im_set2=image1, top_k=self.top_k)
            outputs = dict(matches=matches)
