...
class QueryMatchRefine(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_uv = query.uv * 2 - 1  # (Q, 2)
        query_grid = query_uv.unsqueeze(0).expand(BP, -1, -1)  # (BP, Q, 2)

        warp = coarse_results["warp"]  # (BP, Q, 2)
        confidence = coarse_results["confidence"]  # (BP, Q, confidence_dim)
        ...
        for scale in self.refine_scales:
            ...
            output[f"refiner_{scale}_warp"] = warp
            output[f"refiner_{scale}_confidence"] = confidence
