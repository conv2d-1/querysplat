...
class SDKPipeline(nn.Module):
    def __init__(
        ...
        input_intrinsics_name="intrinsics",
        extrinsics_name="extrinsics",
        input_extrinsics_name="intrinsics",
        ...
        post_align=False,
        ray_dir_normalize_to_unit_sphere=False,
        l1_loss=None,
        local_loss=None,
        msa_loss=None,
        edge_mask_name=None,
        ...
        **kwargs,
        ...
        self.input_intrinsics_name = input_intrinsics_name
        self.extrinsics_name = extrinsics_name
        self.input_extrinsics_name = input_extrinsics_name
        ...
        self.ray_dir_normalize_to_unit_sphere = ray_dir_normalize_to_unit_sphere
        ...
        self.scale_loss = instantiate_from_config(scale_loss)
        ...
        self.save_output_cfg = dict(
            ...
            save_name_match_rgb=False,
            output_conf_ratio=1.0,
            output_match_input_res=match_input_res
        )
        
        if save_output_cfg is not None:
            self.save_output_cfg.update(save_output_cfg)

        def update_out_cfg(key):
            if key in kwargs:
                self.save_output_cfg[key] = kwargs[key]
        update_out_cfg("save_everything")
        update_out_cfg("save_output_conf")
        update_out_cfg("save_normal")
        update_out_cfg("save_normal_vis")
        update_out_cfg("save_name_match_rgb")
        ...
        self.vis_normal = vis_normal or self.vis_everything
    ...
    def get_inputs(self, batch):
        ...
        input_intrinsics = batch.get(
            self.input_intrinsics_name, None
        )  # could be intrinsics mat or ray dirs
        extrinsics = batch.get(self.extrinsics_name, None)  # [n, 4, 4]
        input_extrinsics = batch.get(
            self.input_extrinsics_name, None
        )  # could be extrinsics mat or quant_trans
    ...
    def get_extend_loss(
        ...
        **kwargs,
        ...
        if (
            self.mask_loss is not None
            and invalid_mask_pred is not None
            and invalid_mask_target is not None
        ):
        ...
        if self.normal_pred_loss is not None and normal_pred is not None:
            if not self.recalculate_normal and normal_target is not None:
                normal_target = normal_target.to(normal_pred.device)
                normal_pred_loss = self.normal_pred_loss(
                    normal_pred,
                    normal_target,
                    valid_mask,
                    target_is_normals=True,
                    name=name,
                )
            else:
                normal_pred_loss = self.normal_pred_loss(
        ...
        if self.scale_loss is not None and scale_pred is not None:
            scale_loss = self.scale_loss(scale_pred, prompt_scale.detach().squeeze())
            loss = update_loss(loss, scale_loss, "scale_loss")
    ...
    def train_step(self, batch):
        ...
        normal_target = batch.get("normal", None)
        if normal_target is not None and normal_target.shape[-1] == 3:
            normal_target = normal_target.permute(0, 3, 1, 2)
        ...
        scale_pred = results.get("scale", None)
        ...
        if self.use_extend_loss:
            loss_extend, loss_dict_extend = self.get_extend_loss(
                ...
                depth_target_raw=batch.get("pointmap_raw", None),
                depth_mask_raw=batch.get("depth_raw_mask", None),
            )
    ...
    def postprocess(
        ...
        normal_pred_align = None
        if normal_pred is not None:
            if self.match_input_res and align_gt is not None:
                h, w = align_gt.squeeze().shape[:2]
                normal_pred_align = torch.nn.functional.interpolate(
                    normal_pred, size=(h, w), mode="bilinear"
                )
                normal_pred_align = torch.nn.functional.normalize(normal_pred_align, dim=1)
                normal_pred_align = normal_pred_align.cpu().squeeze(0).permute(1, 2, 0).float().numpy()  # 3,H,W -> H,W,3
            normal_pred = normal_pred.cpu().squeeze(0).permute(1, 2, 0).float().numpy()  # 3,H,W -> H,W,3

        if normal_gt is not None:
            normal_gt = normal_gt.squeeze(0).permute(1, 2, 0).cpu().numpy() # 3,H,W -> H,W,3
        ...
        if invalid_mask_pred is not None:
            ...
            valid_pointmap = pointmap_pred[invalid_mask_pred.reshape(-1) < 0]
            valid_pointmap_color = pointmap_color[invalid_mask_pred.reshape(-1) < 0]
        else:
        ...
        if scale_pred is not None:
            scale_pred = float(scale_pred.cpu().squeeze())

        if depth_mask is not None:
            depth_mask = depth_mask.squeeze(0).cpu().numpy()
        ...
        return self.output_type(
            ...
            normal=normal_pred,
            scale_pred=scale_pred,
            normal_align=normal_pred_align,
    ...
    def infer(self, **batch):
        ...
        return self.postprocess(
            ...
            normal_gt=batch.get("normal", None),
        )
    ...
    def visualize(self, outputs, meta_data, out_dir, prefix=""):
        ...
        if outputs.normal is not None and self.vis_normal:
            normal_pred = outputs.normal.copy()
            save_normal(
                normal_pred,
                out_dir,
                f"{prefix}normal_pred",
                data_idx,
                info=False,
            )
            from hAlgorithm.modules.metrics.pointmap_eval_metrics import (
                pointmap2normal
            )
            if self.vis_gt:
                if outputs.normal_gt is not None:
                    target_normal = outputs.normal_gt.copy()
                    save_normal(
                        target_normal,
                        out_dir,
                        f"{prefix}normal_gt",
                ...
                if outputs.pointmap_gt is not None:
                    target_points = torch.from_numpy(outputs.pointmap_gt).reshape(
                        [outputs.pointmap_h, outputs.pointmap_w, 3]
                    )
                    target_points_normal = pointmap2normal(
                        target_points, torch.from_numpy(outputs.depth_mask)
                    )[0]
                    target_points_normal = target_points_normal.squeeze(0).permute(1, 2, 0).numpy()
                    save_normal(
                        target_points_normal,
                        out_dir,
                        f"{prefix}point_normal_gt",
                        data_idx,
                        info=False,
                    )
            # if outputs.pointmap is not None:
            #     points = torch.from_numpy(outputs.pointmap).reshape(
            #         [outputs.pointmap_h, outputs.pointmap_w, 3]
            #     )
            #     points_normal = pointmap2normal(points, torch.from_numpy(outputs.depth_mask))[0]
            #     points_normal = points_normal.squeeze(0).permute(1, 2, 0).numpy()
            #     save_normal(
            #         points_normal,
            #         out_dir,
            #         f"{prefix}point_normal",
            #         data_idx,
            #         info=False,
            #     )
