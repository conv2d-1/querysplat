...
class ReconstructPipeline(PromptPointMapPipeline):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.render_depth_loss = render_depth_loss
        self.render_normal_loss = render_normal_loss
        ...
        self.novel_focal = novel_focal  # use a loarger focal for novel view
        ...
        self.save_gaussians_rest = save_gaussians_rest
    ...
    def get_inputs(self, batch):
        ...
        intrinsics = batch.get("intrinsics", None)  # [b, v, 3, 3]
        extrinsics = batch.get(self.mv_extrinsics_name, None)  # [b, v, 4, 4]
        ...
        def intrinsic_to_fxfycxcy(intrinsics):
            # 提取 f_x, f_y, c_x, c_y
            ...
            # 拼接成 [b, v, 4, 1] 的形状
            fxfycxcy = torch.cat((f_x, f_y, c_x, c_y), dim=-2).unsqueeze(-1)
            return fxfycxcy

        fxfycxcy = intrinsic_to_fxfycxcy(intrinsics)
        if not self.extrinsics_c2w:
            c2w = extrinsics.inverse()
        else:
            c2w = extrinsics

        # # random purturb c2w, use to train camera head
        # if name not in ['iphone']:
        #     random_quat = transforms.random_quaternions(c2w.shape[0], device=c2w.device)
        #     # c2w[..., :3, :3] = transforms.quaternion_to_matrix(random_quat*0.05+transforms.matrix_to_quaternion(c2w[..., :3, :3])*0.95)
        #     c2w[..., :3, 3] = c2w[..., :3, 3] + torch.randn_like(c2w[..., :3, 3]) * 0.1
        #     extrinsics = c2w.inverse()  # [b, v, 4, 4]

        if self.wtih_rays:
            rays_o, rays_d = self.compute_rays(
                c2w, fxfycxcy, image.shape[-2], image.shape[-1], image.device
            )
        else:
            rays_o = rays_d = None
    ...
    def get_reconstruct_loss(
        ...
        if (
            self.render_depth_loss
            and render_depth is not None
            and name in ["hablendeder_objs", "scpp"]
        ):
            rescale = (
                1.0 if gaussians.get_norm_scale() is None else gaussians.get_norm_scale()
            )  # TODO: bs>2
            render_depth = render_depth * rescale
            render_depth = self.normalize(render_depth, prompt_scale.squeeze(2), None)
            depth_mask = (
                depth[:, :, 2] > 0.05
            )  # ignore too close depth, which will be invalid in rendered depth
            depth_mask = depth_mask & (render_depth > 0.05)

            render_depth = rearrange(render_depth, "b v ... -> (b v) ...")
            depth_z = rearrange(depth, "b v ... -> (b v) ...")[:, 2]
            depth_mask = rearrange(depth_mask, "b v ... -> (b v) ...")
            depth_l1_loss = torch.abs(depth_z[depth_mask] - render_depth[depth_mask])
            depth_l1_loss = depth_l1_loss[depth_l1_loss < 1.0].mean() * 2.0

            # depth_gradient_loss = self.grad_loss(render_depth, depth_z, depth_mask, name=name) * 0.5
            # if depth_gradient_loss > 1.0:
            #     depth_gradient_loss = 0.0

            # depth_gradient_loss = self.depth_loss(render_depth, depth_z, depth_mask, name=name) * 0.1
            # if depth_gradient_loss > 0.1:
            #     depth_gradient_loss = 0.0

            loss += depth_l1_loss
            # loss += depth_gradient_loss
            loss_dict["rcd_l1_loss"] = depth_l1_loss
            # loss_dict["rcd_grad_loss"] = depth_gradient_loss
        ...
        #     alpha_loss = ((render_alpha - 1) ** 2).mean() * 0.1
        #     loss += alpha_loss
        #     loss_dict["alpha_loss"] = alpha_loss

        if (
            self.render_normal_loss
            and render_normal is not None
            and name in ["replica", "hypersim", "hablendeder_objs", "scpp"]
        ):
            ...
            cos_angle = torch.einsum(
                "nc,nc->n",
                target_normal[normal_mask],
                render_normal[normal_mask],
            )
            normal_loss = 1 - cos_angle**2
            normal_loss = torch.nan_to_num(normal_loss).mean() * 0.5
            loss += normal_loss
            loss_dict["gs_normal_loss"] = normal_loss

        if pose_enc is not None and pose_enc.requires_grad and name not in ["iphone"]:
    ...
    def get_novel_view_loss(
        ...
        :param render_rgb: Rendered RGB image tensor.
        :param render_depth: Rendered depth map tensor.
        :param render_alpha: Rendered alpha map tensor with a larger focal.
        :return: Tuple containing total loss and a dictionary of individual losses.
        """
        rgb = (image.float() + 1) * 0.5
        if render_rgb is not None:
        ...
        if render_alpha is not None:  # regularize alpha to encourage gaussian be more compact
            alpha_loss = (1 - render_alpha).mean() * 0.1
            loss += alpha_loss
            loss_dict["novel_alpha_loss"] = alpha_loss
    ...
    def train_step(self, batch):
        ...
        render_large_focal_alpha = results.get(
            "large_focal_alpha", None
        ...
        if self.enable_novel_view_loss and render_novel_rgb is not None:
            ...
            loss_novel_rc, loss_dict_novel_rc = self.get_novel_view_loss(
                ...
                render_alpha=render_large_focal_alpha,
    ...
    def infer(self, **batch):
        ...
        for fi in range(frame_num):
            for vi in range(view_num):
                ...
                single.video = None
                gs_scale = 1.0
                if gaussians is not None and gaussians[0].get_norm_scale() is not None:
                    gs_scale = gaussians[0].get_norm_scale().item()
                if gaussians is not None and vi == 0 and fi == 0:
                    ...
                    if novel_render_depth is not None:
                        single.novel_render_depth = (
                            novel_render_depth[0].detach().cpu().numpy() * gs_scale
                        )  # [N, H, W]
                ...
                if render_depth is not None:
                    single.render_depth = render_depth[0, index].detach().cpu().numpy() * gs_scale
    ...
    def single_visualize(
        ...
        if outputs.render_depth is not None:
            ...
            pointmap_shape = (outputs.pointmap_h, outputs.pointmap_w)
            gt = outputs.pointmap_gt.copy()[..., 2].reshape(pointmap_shape)
            depth = outputs.render_depth.copy()
            save_error(depth, gt, gs_out_dir, f"{prefix}depth_error", data_idx)
    ...
    def visualize(self, outputs_list, meta_data, out_dir):
        ...
        if self.save_gaussians and outputs_list[0].gaussians is not None:
            ...
            save_path = os.path.join(gs_out_dir, "gaussians.ply")
            gaussians.export_ply(save_rest=self.save_gaussians_rest).write(save_path)
        ...
        # if self.save_gaussians and outputs_list[0].gs_xyz is not None:
        #     save_point_cloud(
        #         outputs_list[0].gs_xyz.copy(), None, gs_out_dir, f"gaussians_xyz", data_idx
        #     )
        ...
        if len(render_rgb_paths) > 0:
            save_path = os.path.join(
                os.path.dirname(gs_out_dir), f"merge_render_rgb_{data_idx:06d}.jpg"
            )
            grid_images(
                save_path=save_path,
    ...
    def save_output(self, outputs, meta_data, out_dir, output_meta_dict=None):
        ...
        for i, outputs_sf in enumerate(outputs):
            ...
            if self.save_output_ply and outputs_sf.glb_mv_pointmap is not None:
            ...
            extrinsics = outputs_sf.extrinsics
            local2glb_points_save_path = None
            if extrinsics is not None:
                local_pointmap = outputs_sf.pointmap
                local2glb_pointmap = (
                ...
                local2glb_pointmap = local2glb_pointmap.T[:, :3]
