...
from hAlgorithm.modules.models.gaussiansv2.decoder_pgsr.cuda_splatting import render_cuda
...
class DecoderSplattingCUDA(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        all_alphas = []
        for i in range(b):
            color, depth, normal, alpha = render_cuda(
                extrinsics[i],
                intrinsics[i],
                near[i],
                far[i],
        ...
        output = DecoderOutput(color=color, depth=depth, normal=normal, alpha=alpha)
