...
def render_cuda(
    ...
    for i in range(v):
        ...
        screenspace_points = torch.zeros_like(means_i, requires_grad=True)
        try:
        ...
        image, radii, depth_image = rasterizer(
            means3D=means_i,
        ...
        all_depth.append(depth_image)
        all_radii.append(radii)
        all_visibility_filter.append(radii > 0)

    # gaussians.grad_screenspace_points = [mean_gradients]
    return (
        torch.stack(all_images),
        torch.stack(all_depth),
