...
class MVCombinedModel(CombinedModel):
    def __init__(
        ...
        self.prompt_z_only = prompt_z_only
    ...
    def forward_mv(self, rgb, prompt_depth=None, rays_o=None, rays_d=None, meta_data=None):
        ...
        if self.prompt_encoder is not None and prompt_depth is not None:
            o_cross_d = torch.cross(rays_o, rays_d, dim=2)
            pose_cond = torch.cat([o_cross_d, rays_d], dim=2)
            pose_cond = pose_cond.view(b * n, *pose_cond.shape[-3:])
            if self.prompt_z_only:
                prompt_depth = torch.cat([prompt_depth[:, 2:3], pose_cond], dim=1)
            else:
                prompt_depth = torch.cat([prompt_depth, pose_cond], dim=1)
        ...
        for feature_name in self.gs_features:
            ...
            elif feature_name == "rays":
                features_dict[feature_name] = pose_cond.view(b, n, -1, h, w)
    ...
    def forward_gs(
        ...
        **kwargs,
        ...
        features_dict, results = self.forward_mv(
            rgb, prompt_depth, meta_data=meta_data, rays_o=rays_o, rays_d=rays_d
        )
        ...
        for feature_name in self.gs_features:
            if feature_name == "rgb":
                feature = self.mono_upsampler(
                    features_dict[feature_name], meta_data=meta_data
                )  # [B, F*V, ph*pw, C] * 4 -> [B*F*V, C, H, W]
                feature = feature.unsqueeze(0)  # [B, F*V, C, H, W]
                # TODO: when batch_size > 1
            elif feature_name == "mv_decoder":
        ...
        pose_enc = results.get("pose_enc", None)  # assume this is c2w
        if self.use_predict_pose and pose_enc is not None:
            trans = pose_enc[..., :3] * prompt_scale[..., 0, 0]  # [B, F*V, 3]
            quat = pose_enc[..., 3:7]  # [B, F*V, 4]

            extrinsics[..., :3, 3] = trans
            extrinsics[..., :3, :3] = transforms.quaternion_to_matrix(
                quat[..., [3, 0, 1, 2]]
            )  # convert to WXYZ order
            if self.extrinsics_c2w:
                extrinsics = extrinsics.inverse()
        else:
            # if not using predict pose, use the original extrinsics
