...
def gaussian_blur_2d(input: torch.Tensor, kernel_size: int, sigma: float) -> torch.Tensor:
    kernel = torch.exp(
        -(
            torch.arange(
                -kernel_size // 2 + 1, kernel_size // 2 + 1, dtype=input.dtype, device=input.device
            )
            ** 2
        )
        / (2 * sigma**2)
...
def point_map_to_depth_legacy(points: torch.Tensor):
    ...
    A = torch.stack([points[..., :2], -uv.expand_as(points[..., :2])], dim=-1).flatten(
        -4, -2
    )  # (..., H * W * 2, 2)

    M = A.transpose(-2, -1) @ A
    solution = (
        torch.inverse(M + 1e-6 * torch.eye(2).to(A)) @ (A.transpose(-2, -1) @ b[..., None])
    ).squeeze(-1)
...
def view_plane_uv_to_focal(uv: torch.Tensor):
    normed_uv = normalized_view_plane_uv(
        width=uv.shape[-2], height=uv.shape[-3], device=uv.device, dtype=uv.dtype
    )
...
def recover_focal_shift(
    ...
    points_lr = F.interpolate(points.permute(0, 3, 1, 2), downsample_size, mode="nearest").permute(
        0, 2, 3, 1
    )
    uv_lr = (
    ...
    optim_shift = torch.tensor(optim_shift, device=points.device, dtype=points.dtype).reshape(
        shape[:-3]
    )

    if focal is None:
        optim_focal = torch.tensor(optim_focal, device=points.device, dtype=points.dtype).reshape(
            shape[:-3]
        )
    else:
...
def mask_aware_nearest_resize(
    mask: torch.BoolTensor, target_width: int, target_height: int
...
def solve_subproblem(X_hat, X, W):
    """
    Solve the ROE alignment subproblem without truncation.

    Args:
        X_hat (torch.Tensor): Array ˆX[1..n].
        X (torch.Tensor): Array X[1..n].
        W (torch.Tensor): Array W[1..n].

    Returns:
        s_star (float): Optimal scale s*.
        l_star (float): Objective value l*.
    """
    # Sort arrays by X[i] / X_hat[i]
    ratios = X / X_hat
    sorted_indices = torch.argsort(ratios)

    X_hat_sorted = X_hat[sorted_indices]
    X_sorted = X[sorted_indices]
    W_sorted = W[sorted_indices]

    # Compute accumulated sum Q[1..n] = cumsum(W * X_hat)
    Q = torch.cumsum(W_sorted * X_hat_sorted, dim=0)

    # Compute D[0..n] = {−Q[n]} ∪ {2 · Q[i] − Q[n]}_{i=1}^{n}
    Q_n = Q[-1]
    D = torch.cat((torch.tensor([-Q_n], device=X_hat.device), 2 * Q - Q_n))

    # Find i* such that D[i − 1] ≤ 0 ≤ D[i]
    i_star = torch.where((D[:-1] <= 0) & (D[1:] >= 0))[0]
    if len(i_star) == 0:
        return torch.inf, torch.inf

    i_star = i_star[0]

    # Compute s* = X[i*] / X_hat[i*]
    s_star = X_sorted[i_star] / X_hat_sorted[i_star]

    # Compute the objective function value at s*
    l_star = torch.sum(W_sorted * torch.abs(s_star * X_hat_sorted - X_sorted))

    return s_star.item(), l_star.item()


def roe_alignment_3d(P_hat, P, W=None):
    """
    Solve the ROE alignment problem with 3D shift.

    Args:
        P_hat (torch.Tensor): Array of shape (N, 3), ˆP [1..N, 1..3].
        P (torch.Tensor): Array of shape (N, 3), P [1..N, 1..3].
        W (torch.Tensor): Array of shape (N,), weights.

    Returns:
        s_star (float): Optimal scale s*.
        t_star (torch.Tensor): Optimal shift t* of shape (3,).
        l_star (float): Objective value l*.
    """
    N = P.shape[0]
    # Expand W to shape (3N,) by repeating each element 3 times
    if W is None:
        W = torch.ones(N, dtype=torch.float32, device=P.device)
    W_expanded = W.repeat_interleave(3)

    # Initialize arrays to store results for each k
    s = torch.zeros(N, device=P.device)
    l = torch.zeros(N, device=P.device)
    t = torch.zeros(N, 3, device=P.device)

    # Perform parallel computation for each k
    for k in range(N):
        # Compute flattened arrays ˆX and X for the k-th reference point
        X_hat = (P_hat - P_hat[k]).reshape(-1)  # FLATTEN(ˆP - ˆP[k])
        X = (P - P[k]).reshape(-1)  # FLATTEN(P - P[k])

        # Solve the subproblem for the current k
        s_k, l_k = solve_subproblem(X_hat, X, W_expanded)
        s[k] = s_k
        l[k] = l_k
    ...
    return s_star.item(), t_star, l_star.item()
