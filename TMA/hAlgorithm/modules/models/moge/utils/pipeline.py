...
def _get_queue_item(queue: Queue, terminate_flag: Event, timeout: float = None) -> _ItemWrapper:
    while True:
        try:
            item: _ItemWrapper = queue.get(
                block=True,
                timeout=(
                    TERMINATE_CHECK_INTERVAL
                    if timeout is None
                    else min(timeout, TERMINATE_CHECK_INTERVAL)
                ),
            )
...
class ConcurrentNode(Node):
    ...
    def __init__(
        self,
        running_as: Literal["thread", "process"] = "thread",
        in_buffer_size: int = 1,
        out_buffer_size: int = 1,
    ) -> None:
...
class WorkerFunction(Worker):
    def __init__(
        self, fn: Callable, running_as: "thread", in_buffer_size: int = 1, out_buffer_size: int = 1
    ) -> None:
        super().__init__(running_as, in_buffer_size, out_buffer_size)
        self.fn = fn
...
class Graph(Node):
    ...
    def __iter__(self):
        ...
        if len(providers) == 0:
            raise ValueError(
                "No provider node found in the pipeline. If you want to iterate over the pipeline, the pipeline must be driven by a provider node."
            )
        with self:
...
class Sequential(Graph):
    ...
    def __init__(
        self,
        nodes: List[Union[Node, Callable]],
        function_running_as: Literal["thread", "process"] = "thread",
        in_buffer_size: int = 1,
        out_buffer_size: int = 1,
    ):
        """
