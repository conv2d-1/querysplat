...
def write_rgbd_zip(
    ...
    segmentation_mask: Union[np.ndarray, bytes] = None,
    segmentation_labels: Union[Dict[str, int], bytes] = None,
    ...
    *,
    ...
    depth_type: Literal["linear", "log", "disparity"] = "linear",
    depth_format: Literal["png", "exr"] = "png",
    depth_max_dynamic_range: float = 1e4,
    ...
    if isinstance(image, bytes):
        ...
        image_bytes = cv2.imencode(".jpg", image, [cv2.IMWRITE_JPEG_QUALITY, image_quality])[
            1
        ].tobytes()
    ...
    if isinstance(depth, bytes):
        ...
        if depth_type == "linear":
            if depth.dtype == np.float16:
                ...
                depth_format = "png"
                depth_bytes = cv2.imencode(
                    ".png", depth, [cv2.IMWRITE_PNG_COMPRESSION, png_compression]
                )[1].tobytes()
        elif depth_type == "log":
            depth_format = "png"
            ...
            depth = (
                (np.log(depth.clip(near, far) / near) / np.log(far / near)).clip(0, 1) * 65535
            ).astype(np.uint16)
            depth_bytes = cv2.imencode(
                ".png", depth, [cv2.IMWRITE_PNG_COMPRESSION, png_compression]
            )[1].tobytes()
    ...
    if segmentation_mask is not None:
        ...
        ), "You provided a segmentation mask, but not the corresponding labels."
    ...
    if normal is not None:
        if isinstance(normal, bytes):
            ...
            normal_bytes = cv2.imencode(
                ".png", normal, [cv2.IMWRITE_PNG_COMPRESSION, png_compression]
            )[1].tobytes()
...
def read_rgbd_zip(
    file: Union[str, Path, IO], return_bytes: bool = False
) -> Dict[str, Union[np.ndarray, Dict[str, Any], bytes]]:
    """
    ...
    with zipfile.ZipFile(file, "r") as z:
        ...
        if "mask.png" in z.namelist():
            ...
            if not return_bytes:
                mask = (
                    cv2.imdecode(np.frombuffer(z.read("mask.png"), np.uint8), cv2.IMREAD_UNCHANGED)
                    > 0
                )
        else:
            mask = None
        ...
        if "segformer_mask.png" in z.namelist():
            ...
            if not return_bytes:
                segmentation_mask = cv2.imdecode(
                    np.frombuffer(segmentation_mask, np.uint8), cv2.IMREAD_UNCHANGED
                )
        ...
        if "normal.png" in z.namelist():
            ...
            if not return_bytes:
                normal = cv2.imdecode(
                    np.frombuffer(z.read("normal.png"), np.uint8), cv2.IMREAD_UNCHANGED
                )
            ...
            if "normal_mask.png" in z.namelist():
                ...
                normal_mask = (
                    cv2.imdecode(np.frombuffer(normal_mask, np.uint8), cv2.IMREAD_UNCHANGED) > 0
                )
            else:
                normal_mask = np.ones(image.shape[:2], dtype=bool)
        else:
...
def write_rgbxyz(
    ...
    if isinstance(points, bytes):
        ...
        points_bytes = cv2.imencode(
            ".exr", points.astype(np.float32), [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT]
        )[1].tobytes()
...
def read_rgbxyz(
    ...
    if is_archive:
        with zipfile.ZipFile(file, "r") as z:
            image = cv2.imdecode(np.frombuffer(z.read("image.jpg"), np.uint8), cv2.IMREAD_COLOR)
            points = cv2.imdecode(
                np.frombuffer(z.read("points.exr"), np.uint8), cv2.IMREAD_UNCHANGED
            )
            if "mask.png" in z.namelist():
                mask = (
                    cv2.imdecode(np.frombuffer(z.read("mask.png"), np.uint8), cv2.IMREAD_UNCHANGED)
    ...
    trimesh.Trimesh(
        ...
        visual=trimesh.visual.texture.TextureVisuals(
            ...
            material=trimesh.visual.material.PBRMaterial(
                baseColorTexture=Image.fromarray(texture), metallicFactor=0.5, roughnessFactor=1.0
            ),
        ),
        process=False,
    ).export(save_path)
