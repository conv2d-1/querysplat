...
def catch_exception(fn):
    def wrapper(*args, **kwargs):
        try:
            ...
            print(
                f"Exception in {fn.__name__}({', '.join(repr(arg) for arg in args)}, {', '.join(f'{k}={v!r}' for k, v in kwargs.items())})"
            )
...
def key_average(list_of_dicts: list) -> Dict[str, Any]:
    ...
    for k in _nested_dict_keys:
        values = [get_nested_dict(d, k) for d in list_of_dicts if get_nested_dict(d, k) is not None]
...
class timeit:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.verbose:
            if self.multiple:
                avg = sum(t.time for t in timeit._history[self.name]) / len(
                    timeit._history[self.name]
                )
...
def multithead_execute(inputs: List[Any], num_workers: int, pbar=None):
    ...
    def decorator(fn: Callable):
        with ThreadPoolExecutor(max_workers=num_workers) as executor, pbar:
            pbar.refresh()
