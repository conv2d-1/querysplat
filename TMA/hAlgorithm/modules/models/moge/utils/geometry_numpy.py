...
def harmonic_mean_numpy(
    ...
    if w is None:
        ...
        w = w.astype(x.dtype)
        return 1 / (
            weighted_mean_numpy(1 / (x + eps), w, axis=axis, keepdims=keepdims, eps=eps) + eps
        )
...
def normalized_view_plane_uv_numpy(
    ...
    v = np.linspace(
        -span_y * (height - 1) / height, span_y * (height - 1) / height, height, dtype=dtype
    )
...
def point_map_to_depth_legacy_numpy(points: np.ndarray):
    ...
    M = A.swapaxes(-2, -1) @ A
    solution = (np.linalg.inv(M + 1e-6 * np.eye(2)) @ (A.swapaxes(-2, -1) @ b[..., None])).squeeze(
        -1
    )
...
def recover_focal_shift_numpy(
    ...
    if mask is None:
        points_lr = cv2.resize(points, downsample_size, interpolation=cv2.INTER_LINEAR).reshape(
            -1, 3
        )
...
def mask_aware_nearest_resize_numpy(
    mask: np.ndarray, target_width: int, target_height: int
    ...
    padded_mask = np.full(
        (*mask.shape[:-2], height + 2 * padding_h, width + 2 * padding_w), False, dtype=bool
    )
    ...
    target_uv = utils3d.numpy.image_uv(
    ...
    target_corner = np.round(target_corner - 0.5).astype(np.int32) + np.array(
        (padding_w, padding_h), dtype=np.int32
    )
    ...
    dist = dist[..., 0, :] + dist[..., 1, :]
    dist = np.where(
        target_window_mask, dist, np.inf
    )  # (..., target_height, tgt_width, filter_size)
    nearest_in_window = np.argmin(
        dist, axis=-1, keepdims=True
    )  # (..., target_height, tgt_width, 1)
    nearest_idx = np.take_along_axis(target_window_indices, nearest_in_window, axis=-1).squeeze(
        -1
...
def rigid_registration(
    p: np.ndarray, q: np.ndarray, w: np.ndarray = None, eps: float = 1e-12
...
def rigid_registration_ransac(
    ...
    best_score, best_inlines = 0.0, np.zeros(n, dtype=bool)
    best_solution = (np.array(1.0), np.zeros(3))

    for i in range(max_iters):
        maybe_inliers = np.random.choice(n, size=hypothetical_size, replace=False)
        try:
            s, t = rigid_registration(p[maybe_inliers], q[maybe_inliers], w[maybe_inliers])
