...
class MoGeModel(nn.Module):
    ...
    def forward(
        self, data: torch.Tensor, mixed_precision: bool = False, training=True
    ...
    def pre_align_ransac(self, prediction, target, mask):
        pts_pred = prediction[mask]
        pts_target = target[mask]
        sample_idx = np.random.choice(len(pts_pred), min(len(pts_pred), 1000), replace=True)
        pts_pred = pts_pred[sample_idx]
        pts_target = pts_target[sample_idx]

        # filter top 10% points of z
        upper = pts_pred[..., 2].quantile(0.9)
        lower = pts_pred[..., 2].quantile(0.1)
        mask = (pts_pred[..., 2] > lower) & (pts_pred[..., 2] < upper)
        pts_pred = pts_pred[mask]
        pts_target = pts_target[mask]
        scale = torch.norm(pts_target.max(0)[0] - pts_target.min(0)[0]).item()

        if pts_pred.shape[0] == 0 or pts_target.shape[0] == 0:
            prediction = torch.zeros_like(prediction)
            return prediction, False
        solution, inliers = rigid_registration_ransac(
            pts_pred.detach().cpu().numpy(),
            pts_target.detach().cpu().numpy(),
            max_iters=100,
            inlier_thresh=scale * 0.1,
        )
        optimal_s = torch.tensor(solution[0], device=pts_pred.device)
        optimal_t = torch.tensor(solution[1], device=pts_pred.device)
        target = (target - optimal_t) / optimal_s
        logging.debug(f"s: {optimal_s}")
        return target, True
    ...
    def pre_align_roe(self, prediction, target, mask):
        ...
        if optimal_s > 300 or optimal_s < 0.1:
            logging.warning("Recalculating optimal_s")
            target_mean = pts_target.mean(dim=0)
            pred_mean = pts_pred.mean(dim=0)
            optimal_s = torch.norm(target_mean, dim=-1) / torch.norm(pred_mean, dim=-1)
            prediction = prediction * optimal_s
            return prediction, False
    ...
    def get_loss(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        data["target"] = target
        mask = ((target[..., 2] > 0) & (target[..., 2] < 500.0))[..., None]  # z should be > 0
        ...
        for i in range(len(pred)):
            ...
            if success == False:
                ...
                pcd.points = o3d.utility.Vector3dVector(
                    pred[i].detach().float().cpu().numpy().reshape([-1, 3])
                )
                o3d.io.write_point_cloud("debug.ply", pcd)
                pcd.points = o3d.utility.Vector3dVector(
                    target[i].detach().float().cpu().numpy().reshape([-1, 3])
                )
