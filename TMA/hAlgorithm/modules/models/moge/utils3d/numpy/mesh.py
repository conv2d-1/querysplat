...
@batched(2, None)
def compute_face_normal(vertices: np.ndarray, faces: np.ndarray) -> np.ndarray:
    """
...
@batched(2, None)
def compute_face_angle(vertices: np.ndarray, faces: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """
    ...
    for i in range(3):
        ...
        face_angle[..., i] = np.arccos(
            np.sum(
                edge1
                / np.clip(np.linalg.norm(edge1, axis=-1, keepdims=True), eps, None)
                * edge2
                / np.clip(np.linalg.norm(edge2, axis=-1, keepdims=True), eps, None),
                axis=-1,
            )
        )
    return face_angle
...
def compute_vertex_normal(
    vertices: np.ndarray, faces: np.ndarray, face_normal: np.ndarray = None
) -> np.ndarray:
    """
    ...
    for n in range(vertices.shape[0]):
        for i in range(3):
            vertex_normal[n, :, 0] += np.bincount(
                faces[:, i], weights=face_normal[n, :, 0], minlength=vertices.shape[1]
            )
            vertex_normal[n, :, 1] += np.bincount(
                faces[:, i], weights=face_normal[n, :, 1], minlength=vertices.shape[1]
            )
            vertex_normal[n, :, 2] += np.bincount(
                faces[:, i], weights=face_normal[n, :, 2], minlength=vertices.shape[1]
            )
...
def compute_vertex_normal_weighted(
    ...
    for n in range(vertices.shape[0]):
        for i in range(3):
            vertex_normal[n, :, 0] += np.bincount(
                faces[n, :, i],
                weights=face_normal[n, :, 0] * face_angle[n, :, i],
                minlength=vertices.shape[1],
            )
            vertex_normal[n, :, 1] += np.bincount(
                faces[n, :, i],
                weights=face_normal[n, :, 1] * face_angle[n, :, i],
                minlength=vertices.shape[1],
            )
            vertex_normal[n, :, 2] += np.bincount(
                faces[n, :, i],
                weights=face_normal[n, :, 2] * face_angle[n, :, i],
                minlength=vertices.shape[1],
            )
    vertex_normal_norm = np.linalg.norm(vertex_normal, axis=-1, keepdims=True)
    vertex_normal_norm[vertex_normal_norm == 0] = 1
...
def merge_duplicate_vertices(
    vertices: np.ndarray, faces: np.ndarray, tol: float = 1e-6
...
def subdivide_mesh_simple(
    vertices: np.ndarray, faces: np.ndarray, n: int = 1
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def mesh_relations(
    ...
    edges, face2edge, occurence = np.unique(
...
@overload
def flatten_mesh_indices(
    faces1: np.ndarray, attr1: np.ndarray, *other_faces_attrs_pairs: np.ndarray
) -> Tuple[np.ndarray, ...]:
    """
...
def flatten_mesh_indices(*args: np.ndarray) -> Tuple[np.ndarray, ...]:
    ...
    T, P = args[0].shape
    assert all(
        arg.shape[0] == T and arg.shape[1] == P for arg in args[::2]
    ), "The faces must have the same shape."
    attr_flat = []
