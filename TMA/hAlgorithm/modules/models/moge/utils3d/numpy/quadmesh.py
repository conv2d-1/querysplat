...
def calc_quad_distortion(
    ...
    angle1 = np.arccos(
        np.clip(np.sum(-edge1 * cross, axis=-1) / (len1 * len_cross), -1, 1)
    ) + np.arccos(
        np.clip(np.sum(cross * edge2, axis=-1) / (len_cross * len2), -1, 1)
    )  # [Q]
    angle2 = np.arccos(np.clip(np.sum(-edge2 * edge3, axis=-1) / (len2 * len3), -1, 1))  # [Q]
    angle3 = np.arccos(
        np.clip(np.sum(-edge3 * -cross, axis=-1) / (len3 * len_cross), -1, 1)
    ) + np.arccos(
        np.clip(np.sum(-cross * edge0, axis=-1) / (len_cross * len0), -1, 1)
    )  # [Q]
    ...
    dist_eng = np.abs(angle0 - angle2) ** 2 / np.minimum(
        np.maximum(np.minimum(angle0, angle2), 1e-10),
        np.maximum(D180 - np.maximum(angle0, angle2), 1e-10),
    ) + np.abs(angle1 - angle3) ** 2 / np.minimum(
        np.maximum(np.minimum(angle1, angle3), 1e-10),
        np.maximum(D180 - np.maximum(angle1, angle3), 1e-10),
    )  # [Q]
...
def calc_quad_smoothness(
    ...
    shared_edge_idx_0 = np.array([[0, 0, 1, 1, 2, 2, 3, 3]]).repeat(Q, axis=0)[
        quad2adj_valid
    ]  # [C]
    shared_edge_idx_1 = np.argmax(
        quad2edge[quad2adj][quad2adj_valid]
        == quad2edge[connections[:, 0], shared_edge_idx_0][:, None],
        axis=-1,
    )  # [C]
    valid_smoothness = (
        np.abs(
            quads_direction[connections[:, 0], shared_edge_idx_0]
...
def sovle_quad(
    ...
    quads_connection, quads_connection_idx = np.unique(
        quads_connection, axis=0, return_index=True
    )  # [C, 2], [C]
    ...
    A_ub_triplet = np.concatenate(
        [
            ...
            np.stack(
                [np.arange(T + C, T + 2 * C), np.arange(Q + C, Q + 2 * C), -np.ones(C)], axis=1
            ),  # [C, 3]
            np.stack(
                [np.arange(T + C, T + 2 * C), quads_connection[:, 0], np.ones(C)], axis=1
            ),  # [C, 3]
            np.stack(
                [np.arange(T + C, T + 2 * C), quads_connection[:, 1], np.ones(C)], axis=1
            ),  # [C, 3]
        ],
        axis=0,
    )  # [3T+6C, 3]
    A_ub_triplet = A_ub_triplet[A_ub_triplet[:, 1] != -1]  # [3T', 3]
    A_ub = sp.sparse.coo_matrix(
        (A_ub_triplet[:, 2], (A_ub_triplet[:, 0], A_ub_triplet[:, 1])), shape=[T + 2 * C, Q + 2 * C]
    )  # [T,
    ...
    for i in range(100):
        ...
        A_eq = sp.sparse.coo_matrix(
            (A_eq_triplet[:, 2], (A_eq_triplet[:, 0], A_eq_triplet[:, 1])),
            shape=[num_valid, Q + 2 * C],
        )  # [num_valid, Q+C]
...
def sovle_quad_qp(
    ...
    # ], axis=0)  # [C+Q, 3]
    C_smoothness = sp.sparse.coo_matrix(
        (C_smoothness_triplet[:, 2], (C_smoothness_triplet[:, 0], C_smoothness_triplet[:, 1])),
        shape=[Q, Q],
    )  # [Q, Q]
    ...
    A_ub_triplet = A_ub_triplet[A_ub_triplet[:, 1] != -1]  # [3T', 3]
    A_ub = sp.sparse.coo_matrix(
        (A_ub_triplet[:, 2], (A_ub_triplet[:, 0], A_ub_triplet[:, 1])), shape=[T, Q]
    )  # [T, Q]
    A_ub = A_ub.tocsc()
    b_ub = np.ones(T)
...
if __name__ == "__main__":
    ...
    quads_weight, conn_min_weight, conn_max_weight = sovle_quad(
        face2edge, edge2face, quad2adj, distortion, smoothness, quad_valid
    )
    ...
    utils3d.io.write_ply(
        f"test/assets/{file}_boundary_edges.ply", vertices=vertices, edges=boundary_edges
    )
    ...
    edge_colors[quad_valid] = cv2.cvtColor(
        cv2.applyColorMap(distortion, cv2.COLORMAP_JET), cv2.COLOR_BGR2RGB
    ).reshape(-1, 3)
    utils3d.io.write_ply(
        f"test/assets/{file}_quad_candidates_distortion.ply",
        **vis_edge_color(vertices, edges, edge_colors),
    )

    edge_colors = np.zeros([edges.shape[0], 3], dtype=np.uint8)
    edge_colors[quad_valid] = cv2.cvtColor(
        cv2.applyColorMap((quads_weight * 255).astype(np.uint8), cv2.COLORMAP_JET),
        cv2.COLOR_BGR2RGB,
    ).reshape(-1, 3)
    utils3d.io.write_ply(
        f"test/assets/{file}_quad_candidates_weights.ply",
        **vis_edge_color(vertices, edges, edge_colors),
    )
    ...
    conns_color = cv2.cvtColor(
        cv2.applyColorMap((smoothness * 255).astype(np.uint8), cv2.COLORMAP_JET), cv2.COLOR_BGR2RGB
    ).reshape(-1, 3)
    utils3d.io.write_ply(
        f"test/assets/{file}_quad_conn_smoothness.ply",
        **vis_edge_color(quad_centers, conns, conns_color),
    )
    conns_color = cv2.cvtColor(
        cv2.applyColorMap((conn_min_weight * 255).astype(np.uint8), cv2.COLORMAP_JET),
        cv2.COLOR_BGR2RGB,
    ).reshape(-1, 3)
    utils3d.io.write_ply(
        f"test/assets/{file}_quad_conn_min.ply", **vis_edge_color(quad_centers, conns, conns_color)
    )
    conns_color = cv2.cvtColor(
        cv2.applyColorMap((conn_max_weight * 255).astype(np.uint8), cv2.COLORMAP_JET),
        cv2.COLOR_BGR2RGB,
    ).reshape(-1, 3)
    utils3d.io.write_ply(
        f"test/assets/{file}_quad_conn_max.ply", **vis_edge_color(quad_centers, conns, conns_color)
    )
