...
from . import mesh, transforms, utils
...
class RastContext:
    ...
    def screen_quad(self) -> moderngl.VertexArray:
        self.screen_quad_vbo = self.mgl_ctx.buffer(
    ...
    def program_vertex_attribute(self, n: int) -> moderngl.Program:
        ...
        if "vertex_attribute_vsh" not in self.__prog_src:
            with open(
                os.path.join(os.path.dirname(__file__), "shaders", "vertex_attribute.vsh"), "r"
            ) as f:
                self.__prog_src["vertex_attribute_vsh"] = f.read()
        if "vertex_attribute_fsh" not in self.__prog_src:
            with open(
                os.path.join(os.path.dirname(__file__), "shaders", "vertex_attribute.fsh"), "r"
            ) as f:
        ...
        if f"vertex_attribute_{n}" not in self.__prog:
            ...
            self.__prog[f"vertex_attribute_{n}"] = self.mgl_ctx.program(
    ...
    def program_texture(self, n: int) -> moderngl.Program:
        ...
        if f"texture_{n}" not in self.__prog:
            ...
            self.__prog[f"texture_{n}"] = self.mgl_ctx.program(
                vertex_shader=vsh, fragment_shader=fsh
            )
...
def rasterize_triangle_faces(
    ...
    ), f"Faces should be a 2D array with shape (T, 3), but got {faces.shape}"
    assert attr.ndim == 2 and attr.shape[1] in [
        1,
        2,
        3,
        4,
    ], f"Vertex attribute only supports channels 1, 2, 3, 4, but got {attr.shape}"
    ...
    # Create framebuffer
    image_tex = ctx.mgl_ctx.texture(
        (width, height),
        C,
        dtype="f4",
        data=np.ascontiguousarray(image[::-1, :, :]) if image is not None else None,
    )
    depth_tex = ctx.mgl_ctx.depth_texture(
        (width, height), data=np.ascontiguousarray(depth[::-1, :]) if depth is not None else None
    )
...
def rasterize_edges(
    ...
    ), f"Edges should be a 2D array with shape (T, 2), but got {edges.shape}"
    assert attr.ndim == 2 and attr.shape[1] in [
        1,
        2,
        3,
...
def texture(
    ...
    screen_quad_vao = ctx.mgl_ctx.vertex_array(
        ctx.program_texture(texture.shape[2]),
        [(screen_quad_vbo, "2f4", "in_vert")],
        index_buffer=screen_quad_ibo,
        index_element_size=4,
    )

    # Create texture, set filter and bind. TODO: min mag filter, mipmap
    texture_tex = ctx.mgl_ctx.texture(
        (texture.shape[1], texture.shape[0]),
        texture.shape[2],
        dtype=texture_dtype,
        data=np.ascontiguousarray(texture),
    )
    ...
    texture_uv = ctx.mgl_ctx.texture(
        (width, height), 2, dtype="f4", data=np.ascontiguousarray(uv.astype("f4", copy=False))
    )
    ...
    image_buffer = np.frombuffer(
        fbo.read(components=texture.shape[2], attachment=0, dtype=texture_dtype),
        dtype=texture_dtype,
    ).reshape((height, width, texture.shape[2]))
...
def warp_image_by_depth(
    ...
    *,
    ...
    if src_image is not None:
        assert (
            src_image.shape[-2:] == src_depth.shape[-2:]
        ), f"Shape of source image {src_image.shape} does not match shape of source depth {src_depth.shape}"
    ...
    assert all(
        x is not None for x in [extrinsics_src, extrinsics_tgt, intrinsics_src, intrinsics_tgt]
    ), "Make sure you have provided all the necessary camera parameters."
    ...
    if ssaa > 1:
        ...
        tgt_depth = (
            tgt_depth.reshape(height, ssaa, width, ssaa, -1).mean(axis=(1, 3))
            if return_depth
            else None
        )
...
def test():
    ...
    cv2.imwrite(
        "CHECKME.png", cv2.cvtColor((image.clip(0, 1) * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)
    )
