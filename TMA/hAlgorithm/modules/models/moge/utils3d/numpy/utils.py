...
def max_pool_1d(x: np.ndarray, kernel_size: int, stride: int, padding: int = 0, axis: int = -1):
    ...
    if padding > 0:
        ...
        padding_arr = np.full(
            (*x.shape[:axis], padding, *x.shape[axis + 1 :]), fill_value=fill_value, dtype=x.dtype
        )
...
def depth_edge(
    ...
    """
    if mask is None:
        diff = max_pool_2d(depth, kernel_size, stride=1, padding=kernel_size // 2) + max_pool_2d(
            -depth, kernel_size, stride=1, padding=kernel_size // 2
        )
    else:
        diff = max_pool_2d(
            np.where(mask, depth, -np.inf), kernel_size, stride=1, padding=kernel_size // 2
        ) + max_pool_2d(
            np.where(mask, -depth, -np.inf), kernel_size, stride=1, padding=kernel_size // 2
        )
...
def normals_edge(
    ...
    if mask is None:
        angle_diff = np.acos((normals[..., None, None] * normals_window).sum(axis=-3)).max(
            axis=(-2, -1)
        )
    else:
        ...
        angle_diff = np.where(
            mask_window, np.acos((normals[..., None, None] * normals_window).sum(axis=-3)), 0
        ).max(axis=(-2, -1))
...
def depth_to_normals(
    depth: np.ndarray, intrinsics: np.ndarray, mask: np.ndarray = None
) -> np.ndarray:
    """
...
def interpolate(
    bary: np.ndarray, tri_id: np.ndarray, attr: np.ndarray, faces: np.ndarray
) -> np.ndarray:
...
def image_mesh_from_depth(
    ...
    *vertice_attrs: np.ndarray,
    ...
    if atol is not None or rtol is not None:
        ...
        image_face_, ref_indices = mesh.remove_unreferenced_vertices(
            image_face_, return_indices=True
        )
    ...
    for attr in vertice_attrs:
        ret.append(
            ...
            if not remove
            else attr.reshape(-1, attr.shape[-1])[ref_indices]
        )
    if return_uv:
...
def chessboard(
    width: int, height: int, grid_size: int, color_a: np.ndarray, color_b: np.ndarray
) -> np.ndarray:
...
def camera_frustum(
    extrinsics: np.ndarray, intrinsics: np.ndarray, depth: float = 1.0
    ...
    edges = np.array(
        [[0, 1], [0, 2], [0, 3], [0, 4], [1, 2], [2, 3], [3, 4], [4, 1]], dtype=np.int32
    )
    faces = np.array(
        [[0, 1, 2], [0, 2, 3], [0, 3, 4], [0, 4, 1], [1, 2, 3], [1, 3, 4]], dtype=np.int32
    )
