...
class RastContext:
    ...
    def __init__(
        self,
        nvd_ctx: Union[dr.RasterizeCudaContext, dr.RasterizeGLContext] = None,
        *,
        backend: Literal["cuda", "gl"] = "gl",
...
def rasterize_triangle_faces(
    ...
    if vertices.shape[-1] == 2:
        vertices = torch.cat(
            [vertices, torch.zeros_like(vertices[..., :1]), torch.ones_like(vertices[..., :1])],
            dim=-1,
        )
    ...
    rast_out, rast_db = dr.rasterize(
        ctx.nvd_ctx, pos_clip, faces, resolution=[height, width], grad_db=True
    )
...
def warp_image_by_depth(
    ...
    *,
    ...
    if image is not None:
        assert (
            image.shape[-2:] == depth.shape[-2:]
        ), f"Shape of image {image.shape} does not match shape of depth {depth.shape}"
    ...
    if padding > 0:
        ...
        if mask is not None:
            depth = torch.where(
                mask, depth, torch.tensor(far, dtype=depth.dtype, device=depth.device)
            )
    ...
    if image is not None:
        ...
        if antialiasing:
            print(
                "\033[93mWarning: you are performing antialiasing on uv. This may cause artifacts.\033[0m"
            )
        if return_uv:
            return_uv = False
