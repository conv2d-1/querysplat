...
def get_rays(extrinsics: Tensor, intrinsics: Tensor, uv: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    """
    uvz = torch.cat([uv, torch.ones_like(uv[..., :1])], dim=-1).to(
        extrinsics
    )  # (n_batch, n_views, n_rays, 3)
...
def get_image_rays(
    extrinsics: Tensor, intrinsics: Tensor, width: int, height: int
) -> Tuple[Tensor, Tensor]:
    """
    Args:
        extrinsics: (..., 4, 4) extrinsics matrices.
...
def get_mipnerf_cones(
    rays_o: Tensor, rays_d: Tensor, z_vals: Tensor, pixel_width: Tensor
) -> Tuple[Tensor, Tensor]:
    """
    Args:
    ...
    sigma_t = t_delta_square / 3.0 - (4.0 / 15.0) * t_delta_quad / (
        3.0 * t_mu_square + t_delta_square
    ).square() * (12.0 * t_mu_square - t_delta_square)
    sigma_r = (pixel_width[..., None, None].square() / 3.0) * (
        t_mu_square / 4.0
        + (5.0 / 12.0) * t_delta_square
        - (4.0 / 15.0) * t_delta_quad / (3.0 * t_mu_square + t_delta_square)
    )
    points_mu = rays_o[:, :, :, None, :] + rays_d[:, :, :, None, :] * mu_t[..., None]
    d_dt = rays_d[..., :, None] * rays_d[..., None, :]  # (..., n_rays, 3, 3)
    points_sigma = sigma_t[..., None, None] * d_dt[..., None, :, :] + sigma_r[..., None, None] * (
        torch.eye(3).to(rays_o) - d_dt[..., None, :, :]
    )
...
def volume_rendering(
    ...
    transparancy = (
        -torch.cat([torch.zeros_like(sigma_delta[..., :1]), sigma_delta[..., :-1]], dim=-1).cumsum(
            dim=-1
        )
    ).exp_()  # First cumsum then exp for numerical stability
    ...
    if depth:
        ...
        depth = (
            torch.sum(weights * z_vals, dim=-1) / weights.sum(dim=-1).clamp_min_(1e-8)
            if depth
            else None
        )
...
def neus_volume_rendering(
    ...
    transparancy = torch.cumprod(
        torch.cat([torch.ones_like(alpha[..., :1]), alpha], dim=-1), dim=-1
    )
    weights = alpha * transparancy
...
def bin_sample(
    ...
    min_value: Number,
    max_value: Number,
    spacing: Literal["linear", "inverse_linear"],
    ...
    z_rand = (
        torch.rand(*size, n_samples, device=device, dtype=dtype) - 0.5
    ) * bin_length + torch.linspace(
        min_value + bin_length * 0.5,
        max_value - bin_length * 0.5,
        n_samples,
        device=device,
        dtype=dtype,
    )
...
def importance_sample(z_vals: Tensor, weights: Tensor, n_samples: int) -> Tuple[Tensor, Tensor]:
    ...
    inds = torch.searchsorted(cdf, u, right=True).clamp(
        0, cdf.shape[-1] - 1
    )  # (..., n_rays, n_samples)
...
def nerf_render_rays(
    nerf: Union[
        ...
        Tuple[Callable[[Tensor], Tuple[Tensor, Tensor]], Callable[[Tensor], Tuple[Tensor, Tensor]]],
    ],
    rays_o: Tensor,
    rays_d: Tensor,
    *,
    ...
    z_coarse = bin_sample(
        ...
        dtype=rays_o.dtype,
        spacing=z_spacing,
    )  # (n_batch, n_views, n_rays, n_samples)
    points_coarse = (
        rays_o[..., None, :] + rays_d[..., None, :] * z_coarse[..., None]
    )  # (n_batch, n_views, n_rays, n_samples, 3)
    ray_length = rays_d.norm(dim=-1)

    #    Query color and density
    color_coarse, density_coarse = nerf_coarse(
        points_coarse, rays_d[..., None, :].expand_as(points_coarse)
    )  # (n_batch, n_views, n_rays, n_samples, 3), (n_batch, n_views, n_rays, n_samples)

    #    Volume rendering
    with torch.no_grad():
        rgb_coarse, depth_coarse, weights = volume_rendering(
            color_coarse, density_coarse, z_coarse, ray_length
        )  # (n_batch, n_views, n_rays, 3), (n_batch, n_views, n_rays, 1), (n_batch, n_views, n_rays, n_samples)
    ...
    if nerf_coarse is nerf_fine:
        ...
        color_fine, density_fine = nerf_fine(
            points_fine, rays_d[..., None, :].expand_as(points_fine)
        )
...
def mipnerf_render_rays(
    ...
    *,
    ...
    #    Query color and density
    color_coarse, density_coarse = mipnerf(
        points_mu_coarse, points_sigma_coarse, rays_d[..., None, :].expand_as(points_mu_coarse)
    )  # (n_batch, n_views, n_rays, n_samples, 3), (n_batch, n_views, n_rays, n_samples)

    #    Volume rendering
    rgb_coarse, depth_coarse, weights_coarse = volume_rendering(
        color_coarse, density_coarse, z_coarse, ray_length
    )  # (n_batch, n_views, n_rays, 3), (n_batch, n_views, n_rays, 1), (n_batch, n_views, n_rays, n_samples)
    ...
    with torch.no_grad():
        weights_coarse = (
            1.0 - uniform_ratio
        ) * weights_coarse + uniform_ratio / weights_coarse.shape[-1]
    ...
    color_fine, density_fine = mipnerf(
        points_mu_fine, points_sigma_fine, rays_d[..., None, :].expand_as(points_mu_fine)
    )

    #   Volume rendering
    rgb_fine, depth_fine, weights_fine = volume_rendering(
        color_fine, density_fine, z_fine, ray_length
    )
...
def neus_render_rays(
    ...
    *,
    ...
    #    Query color and density
    color_coarse, sdf_coarse = neus(
        points_coarse, rays_d[..., None, :].expand_as(points_coarse)
    )  # (n_batch, n_views, n_rays, n_samples, 3), (n_batch, n_views, n_rays, n_samples)

    #    Volume rendering
    with torch.no_grad():
        rgb_coarse, depth_coarse, weights = neus_volume_rendering(
            color_coarse, sdf_coarse, s, z_coarse
        )  # (n_batch, n_views, n_rays, 3), (n_batch, n_views, n_rays, 1), (n_batch, n_views, n_rays, n_samples)
    ...
    if patchify:
        ...
        for i_row in range(n_rows):
            ...
            for i_column in range(n_columns):
                patch_shape = patch_height, patch_width = min(
                    max_patch_height, height - i_row * max_patch_height
                ), min(max_patch_width, width - i_column * max_patch_width)
                uv = image_uv(
                    height,
                    width,
                    i_column * max_patch_width,
                    i_row * max_patch_height,
                    i_column * max_patch_width + patch_width,
                    i_row * max_patch_height + patch_height,
                ).to(extrinsics)
                ...
                rgb_ = rgb_.transpose(-1, -2).unflatten(
                    -1, patch_shape
