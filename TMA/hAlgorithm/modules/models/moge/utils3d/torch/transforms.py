...
def intrinsics_from_focal_center(
    ...
    zeros, ones = torch.zeros(N, dtype=fx.dtype, device=fx.device), torch.ones(
        N, dtype=fx.dtype, device=fx.device
    )
    ret = torch.stack([fx, zeros, cx, zeros, fy, cy, zeros, zeros, ones], dim=-1).unflatten(
        -1, (3, 3)
    )
    return ret
...
def intrinsics_from_fov_xy(
    fov_x: Union[float, torch.Tensor], fov_y: Union[float, torch.Tensor]
) -> torch.Tensor:
    """
...
@batched(2)
def perspective_to_intrinsics(perspective: torch.Tensor) -> torch.Tensor:
    """
    ...
    assert torch.allclose(
        perspective[:, [0, 1, 3], 3], 0
    ), "The perspective matrix is not a projection matrix"
    ret = (
        torch.tensor(
...
def extrinsics_to_view(extrinsics: torch.Tensor) -> torch.Tensor:
    ...
    return (
        extrinsics
        * torch.tensor([1, -1, -1, 1], dtype=extrinsics.dtype, device=extrinsics.device)[:, None]
    )


@batched(2)
def view_to_extrinsics(view: torch.Tensor) -> torch.Tensor:
    """
...
def normalize_intrinsics(
    intrinsics: torch.Tensor, width: Union[int, torch.Tensor], height: Union[int, torch.Tensor]
) -> torch.Tensor:
    """
    ...
    transform = (
        torch.stack(
            [1 / width, zeros, 0.5 / width, zeros, 1 / height, 0.5 / height, zeros, zeros, ones]
        )
...
@batched(1, 0, 0)
def uv_to_pixel(
    uv: torch.Tensor, width: Union[int, torch.Tensor], height: Union[int, torch.Tensor]
) -> torch.Tensor:
    """
    Args:
...
def pixel_to_ndc(
    ...
    if not torch.is_floating_point(pixel):
        pixel = pixel.float()
    ndc = (pixel + 0.5) / (
        torch.stack([width, height], dim=-1).to(pixel)
        * torch.tensor([2, -2], dtype=pixel.dtype, device=pixel.device)
...
@batched(0, 0, 0)
def project_depth(
    depth: torch.Tensor, near: Union[float, torch.Tensor], far: Union[float, torch.Tensor]
) -> torch.Tensor:
    """
...
@batched(2, 2, 2)
def project_cv(
    points: torch.Tensor, extrinsics: torch.Tensor = None, intrinsics: torch.Tensor = None
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def euler_angles_to_matrix(euler_angles: torch.Tensor, convention: str = "XYZ") -> torch.Tensor:
    ...
    matrices = [euler_axis_angle_rotation(c, euler_angles[..., "XYZ".index(c)]) for c in convention]
...
def matrix_to_euler_angles(matrix: torch.Tensor, convention: str) -> torch.Tensor:
    ...
    o = [
        _angle_from_tan(convention[0], convention[1], matrix[..., i2, :], True, tait_bryan),
        central_angle,
        _angle_from_tan(convention[2], convention[1], matrix[..., i0], False, tait_bryan),
    ]
...
def quaternion_to_matrix(quaternion: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    ...
    B = torch.stack([zeros, -z, y, z, zeros, -x, -y, x, zeros], dim=-1).unflatten(-1, (3, 3))
    rot_mat = I + 2 * (A + w[..., None, None] * B)
    return rot_mat


def slerp(
    rot_mat_1: torch.Tensor, rot_mat_2: torch.Tensor, t: Union[Number, torch.Tensor]
) -> torch.Tensor:
...
def interpolate_extrinsics(
    ext1: torch.Tensor, ext2: torch.Tensor, t: Union[Number, torch.Tensor]
) -> torch.Tensor:
...
def interpolate_transform(
    ...
    transform = torch.cat(
        [
            ext,
            torch.tensor([0, 0, 0, 1], dtype=transform.dtype, device=transform.device).expand_as(
                transform[..., :1, :]
            ),
        ],
        dim=-2,
...
def rotate_2d(theta: Union[float, torch.Tensor], center: torch.Tensor = None):
    ...
    return torch.cat(
        [
            ...
            torch.tensor([[0, 0, 1]], dtype=center.dtype, device=center.device).expand(
                *center.shape[:-1], -1, -1
            ),
        ],
        dim=-2,
...
def translate_2d(translation: torch.Tensor):
    ...
    return torch.cat(
        [
            torch.cat(
                [
                    torch.eye(2, dtype=translation.dtype, device=translation.device).expand(
            ...
            torch.tensor([[0, 0, 1]], dtype=translation.dtype, device=translation.device).expand(
                *translation.shape[:-1], -1, -1
            ),
        ],
        dim=-2,
    ...
    return torch.cat(
        [
            torch.cat(
                [
                    scale
                    * torch.eye(2, dtype=scale.dtype, device=scale.device).expand(
                        *scale.shape[:-1], -1, -1
                    ),
            ...
            torch.tensor([[0, 0, 1]], dtype=scale.dtype, device=scale.device).expand(
                *center.shape[:-1], -1, -1
            ),
        ],
        dim=-2,
...
def apply_2d(transform: torch.Tensor, points: torch.Tensor):
    ...
    """
    assert transform.shape[-2:] == (3, 3) or transform.shape[-2:] == (
        2,
        3,
    ), "transform must be 3x3 or 2x3"
