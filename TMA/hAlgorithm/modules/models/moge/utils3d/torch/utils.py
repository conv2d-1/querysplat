...
def sliding_window_1d(
    x: torch.Tensor, window_size: int, stride: int = 1, dim: int = -1
) -> torch.Tensor:
    """
...
def image_uv(
    ...
    if bottom is None:
        bottom = height
    u = torch.linspace(
        (left + 0.5) / width, (right - 0.5) / width, right - left, device=device, dtype=dtype
    )
    v = torch.linspace(
        (top + 0.5) / height, (bottom - 0.5) / height, bottom - top, device=device, dtype=dtype
    )
...
def image_mesh(
    ...
    faces = (
        torch.arange(0, (height - 1) * width, width, device=device, dtype=torch.int32)[
            :, None, None
        ]
        + row_faces[None, :, :]
    ).reshape((-1, 4))
...
def image_mesh_from_depth(
    depth: torch.Tensor, extrinsics: torch.Tensor = None, intrinsics: torch.Tensor = None
) -> Tuple[torch.Tensor, torch.Tensor]:
...
def point_to_normal(point: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
    ...
    pts = F.pad(point.permute(0, 3, 1, 2), (1, 1, 1, 1), mode="constant", value=1).permute(
        0, 2, 3, 1
    )
...
@batched(2, 2, 2)
def depth_to_normal(
    depth: torch.Tensor, intrinsics: torch.Tensor, mask: torch.Tensor = None
) -> torch.Tensor:
    """
    ...
    pts = transforms.unproject_cv(
        uv.reshape(-1, 2), depth.flatten(-2), intrinsics=intrinsics, extrinsics=None
    ).unflatten(-2, (height, width))
...
def masked_min(
    ...
    if dim is None:
        return torch.where(
            mask, input, torch.tensor(torch.inf, dtype=input.dtype, device=input.device)
        ).min()
    else:
        return torch.where(
            mask, input, torch.tensor(torch.inf, dtype=input.dtype, device=input.device)
        ).min(dim=dim, keepdim=keepdim)
...
def chessboard(
    width: int, height: int, grid_size: int, color_a: torch.Tensor, color_b: torch.Tensor
) -> torch.Tensor:
