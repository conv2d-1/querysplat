...
def triangulate(
    faces: torch.Tensor, vertices: torch.Tensor = None, backslash: bool = None
) -> torch.Tensor:
    """
    ...
    if vertices is not None:
        ...
        if backslash is None:
            ...
            backslash = torch.norm(
                vertices[faces_idx[..., 0]] - vertices[faces_idx[..., 2]], p=2, dim=-1
...
@batched(2, None)
def compute_face_angles(vertices: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
    """
    ...
    face_angles = []
    for i in range(3):
        edge1 = torch.index_select(
            vertices, dim=-2, index=faces[:, (i + 1) % 3]
        ) - torch.index_select(vertices, dim=-2, index=faces[:, i])
        edge2 = torch.index_select(
            vertices, dim=-2, index=faces[:, (i + 2) % 3]
        ) - torch.index_select(vertices, dim=-2, index=faces[:, i])
        face_angle = torch.arccos(
            torch.sum(F.normalize(edge1, p=2, dim=-1) * F.normalize(edge2, p=2, dim=-1), dim=-1)
        )
...
def compute_vertex_normal(
    vertices: torch.Tensor, faces: torch.Tensor, face_normal: torch.Tensor = None
) -> torch.Tensor:
    """
    ...
    faces = faces.flatten()
    vertex_normal = torch.index_put(
        torch.zeros_like(vertices),
        (torch.arange(N)[:, None], faces[None, :]),
        face_normal,
        accumulate=True,
    )
...
def compute_vertex_normal_weighted(
    ...
    vertex_normal = torch.index_put(
        torch.zeros_like(vertices),
        (torch.arange(N)[:, None], faces.view(N, -1)),
        face_normal.view(N, -1, 3),
        accumulate=True,
    )
    vertex_normal = F.normalize(vertex_normal, p=2, dim=-1)
    return vertex_normal
...
def remove_corrupted_faces(faces: torch.Tensor) -> torch.Tensor:
    """
...
def merge_duplicate_vertices(
    vertices: torch.Tensor, faces: torch.Tensor, tol: float = 1e-6
...
def subdivide_mesh_simple(
    vertices: torch.Tensor, faces: torch.Tensor, n: int = 1
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    for _ in range(n):
        ...
        faces = torch.cat(
            [
                torch.stack(
                    [faces[:, 0], n_vertices + uni_inv[0], n_vertices + uni_inv[2]], axis=1
                ),
                torch.stack(
                    [faces[:, 1], n_vertices + uni_inv[1], n_vertices + uni_inv[0]], axis=1
                ),
                torch.stack(
                    [faces[:, 2], n_vertices + uni_inv[2], n_vertices + uni_inv[1]], axis=1
                ),
                torch.stack(
                    [n_vertices + uni_inv[0], n_vertices + uni_inv[1], n_vertices + uni_inv[2]],
                    axis=1,
                ),
            ],
            dim=0,
...
def compute_face_tbn(
    ...
    """
    e01 = torch.index_select(pos, dim=-2, index=faces_pos[:, 1]) - torch.index_select(
        pos, dim=-2, index=faces_pos[:, 0]
    )
    e02 = torch.index_select(pos, dim=-2, index=faces_pos[:, 2]) - torch.index_select(
        pos, dim=-2, index=faces_pos[:, 0]
    )
    uv01 = torch.index_select(uv, dim=-2, index=faces_uv[:, 1]) - torch.index_select(
        uv, dim=-2, index=faces_uv[:, 0]
    )
    uv02 = torch.index_select(uv, dim=-2, index=faces_uv[:, 2]) - torch.index_select(
        uv, dim=-2, index=faces_uv[:, 0]
    )
    normal = torch.cross(e01, e02)
    tangent_bitangent = torch.stack([e01, e02], dim=-1) @ torch.inverse(
        torch.stack([uv01, uv02], dim=-1)
    )
...
def compute_vertex_tbn(
    ...
    face_tbn = (
        face_tbn[..., :, None, :, :]
        .repeat(*[1] * len(batch_shape), 1, 3, 1, 1)
        .view(*batch_shape, n_tri * 3, 3, 3)
    )  # (..., T * 3, 3, 3)
    vertex_tbn = torch.index_add(
        torch.zeros(*batch_shape, n_vertices, 3, 3).to(face_tbn),
        dim=-3,
        index=faces_topo.view(-1),
        source=face_tbn,
    )
...
def laplacian(vertices: torch.Tensor, faces: torch.Tensor, weight: str = "uniform") -> torch.Tensor:
    ...
    face_verts = torch.index_select(vertices, -2, faces.view(-1)).view(
    ...
    if weight == "cotangent":
        for i in range(3):
            ...
            cot_angle = (e1 * e2).sum(dim=-1) / torch.cross(e1, e2, dim=-1).norm(
                p=2, dim=-1
            )  # (..., T, 3)
            sum_verts = torch.index_add(
                sum_verts,
                -2,
                faces[:, (i + 1) % 3],
                face_verts[..., (i + 2) % 3, :] * cot_angle[..., None],
            )
            ...
            sum_verts = torch.index_add(
                ...
                -2,
                faces[:, (i + 2) % 3],
                face_verts[..., (i + 1) % 3, :] * cot_angle[..., None],
            )
        ...
        for i in range(3):
            ...
            sum_weights = torch.index_add(
                sum_weights, -1, faces[:, i], torch.ones_like(face_verts[..., i, 0])
            )
    else:
...
def laplacian_smooth_mesh(
    vertices: torch.Tensor, faces: torch.Tensor, weight: str = "uniform", times: int = 5
) -> torch.Tensor:
    """Laplacian smooth with cotangent weights
...
def taubin_smooth_mesh(
    vertices: torch.Tensor, faces: torch.Tensor, lambda_: float = 0.5, mu_: float = -0.51
) -> torch.Tensor:
    """Taubin smooth mesh
