...
def write_extrinsics_as_colmap(
    file: Union[str, Path],
    extrinsics: np.ndarray,
    image_names: Union[str, List[str]] = "image_{i:04d}.png",
    ...
    assert (
        len(extrinsics) == len(image_names) == len(camera_ids)
    ...
    with open(file, "w") as fp:
        ...
        for i, (quat, t, name, camera_id) in enumerate(
            zip(quats.tolist(), trans.tolist(), image_names, camera_ids)
        ):
            # Colmap has wxyz order while scipy.spatial.transform.Rotation has xyzw order.
            qx, qy, qz, qw = quat
            tx, ty, tz = t
            print(
                f"{i + 1} {qw:f} {qx:f} {qy:f} {qz:f} {tx:f} {ty:f} {tz:f} {camera_id:d} {name}",
                file=fp,
            )
            print()


def write_intrinsics_as_colmap(
    file: Union[str, Path],
...
def read_intrinsics_from_colmap(
    file: Union[str, Path], normalize: bool = False
