...
class SDPAAttnBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        batch_first_x = x.transpose(0, 1)
        B, N, C = batch_first_x.shape

        # Project q, k, v
        qkv = (
            self.qkv_proj(batch_first_x)
            .reshape(B, N, 3, self.num_heads, self.head_dim)
            .permute(2, 0, 3, 1, 4)
        )
        q, k, v = qkv.unbind(0)  # Unbind the q, k, v projections

        # Compute scaled dot product attention
        attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=mask, dropout_p=0.0)
        attn_output = attn_output.permute(2, 0, 1, 3).reshape(N, B, -1)
...
class SDPACrossAttnBlock(nn.Module):
    def __init__(self, hidden_size, context_dim, num_heads=1, mlp_ratio=4.0):
        ...
        self.kv_proj = nn.Linear(context_dim, 2 * context_dim // num_heads * num_heads)
    ...
    def forward(self, x, context, mask=None):
        ...
        kv = (
            ...
            .reshape(B, M, 2, self.num_heads, D // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
        k, v = kv.unbind(0)  # Unbind the k, v projections
