...
from .utils import (
    ...
    sample_features4d,
    project_query_to_points3d,
    project_points3d_to_frames,
    get_tracks_from_query,
)
...
class BaseTrackerPredictor(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        cache_key = f"{int(HH)}_{int(WW)}"
        if cache_key in self.pos_embed_cache:
            pos_embed = self.pos_embed_cache[cache_key].clone()
        else:
        ...
        for _ in range(iters):
            ...
            query_ref_token = torch.cat(
                [self.query_ref_token[:, 0:1], self.query_ref_token[:, 1:2].expand(-1, S - 1, -1)],
                dim=1,
            )
        ...
        if self.predict_conf:
            conf_e = self.conf_predictor(track_feats.reshape(B * S * N, self.latent_dim)).reshape(
                B, S, N
            )
            if apply_sigmoid:
...
class BaseTrackerPredictorV2(BaseTrackerPredictor):
    def __init__(
        self, 
        coords_init="copy", 
        add_pose_flow=False, 
        sample_query_frame_feat=True,
        **kwargs
    ):
        super().__init__(**kwargs)
        self.coords_init = coords_init
        assert self.coords_init in ['copy', 'project'], f"Coords must be init with query copy or project, but got {self.coords_init}"

        self.add_pose_flow = add_pose_flow
        self.sample_query_frame_feat = sample_query_frame_feat

    def scale_query(self, query_points, down_ratio):
        # Scale the input query_points because we may downsample the images
        # by down_ratio or self.stride
        # e.g., if a 3x1024x1024 image is processed to a 128x256x256 feature map
        # its query_points should be query_points/4
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        query_points: B x N x 2, the number of batches, tracks, and xy
        query_frames: B x N, the number of batches, tracks, and frame_id
        fmaps: B x S x C x HH x WW, the number of batches, frames, and feature dimension.
                note HH and WW is the size of feature maps instead of original images
        """
        B, N, D = query_points.shape
        ...
        if self.coords_init in ['project']:
            # Init with coords as the projected query points
            if pose_enc is not None and local_depth is not None:
                if isinstance(pose_enc, (list, tuple)):
                    cur_pose_enc = pose_enc[-1]
                else:
                    cur_pose_enc = pose_enc
                _, _, _, H_d, W_d = local_depth.shape
                with torch.cuda.amp.autocast(enabled=False):
                    extrinsics_pred, intrinsics_pred = pose_encoding_to_extri_intri(
                        pose_encoding=cur_pose_enc.float(),
                        image_size_hw=(H_d, W_d),
                        translation_scale=None,
                        build_intrinsics=True,
                    )
                    coords, vis_mask = get_tracks_from_query(
                        query_points,
                        local_depth, 
                        intrinsics.float(), 
                        query_frames,
                        extrinsics_pred.float(),
                    )
            else:
                raise ValueError("No pose and depth to project query !")
            
            coords = coords.to(query_points.dtype)
            query_points = self.scale_query(query_points, down_ratio)
            coords = self.scale_query(coords, down_ratio)
        else:# copy
            # Init with coords as the query points
            # It means the search will start from the position of query points at the reference frames
            query_points = self.scale_query(query_points, down_ratio)
        ...
        # Sample/extract the features of the query points in the query frame
        if query_frames is not None and self.sample_query_frame_feat:
            assert query_frames.shape == query_points.shape[:2]
            query_track_feat = []
            for ii in range(fmaps.shape[1]):
                query_track_feat.append(sample_features4d(fmaps[:, ii], coords[:, ii]))
            query_track_feat = torch.stack(query_track_feat, dim=1)
            gather_idx = query_frames.unsqueeze(1).unsqueeze(-1).expand(B, 1, N, C).long()
            query_track_feat = torch.gather(query_track_feat, dim=1, index=gather_idx).squeeze(1)
        else:
        ...
        for _ in range(iters):
            ...
            # Force coord as query
            # because we assume the query points should not be changed
            if query_frames is not None:
                b_idx = torch.arange(B, device=coords.device).view(-1, 1)  # [B, 1]
                n_idx = torch.arange(N, device=coords.device).view(1, -1)  # [1, N]
                t_idx = query_frames.long()  # [B, N]
                coords[b_idx, t_idx, n_idx] = coords_backup[b_idx, t_idx, n_idx]
            else:
                coords[:, 0] = coords_backup[:, 0]
