...
def pycolmap_to_batch_np_matrix(reconstruction, device="cpu", camera_type="SIMPLE_PINHOLE"):
    ...
    points3D_rgb = np.zeros((max_points3D_id, 3))

    for point3D_id in reconstruction.points3D:
        points3D[point3D_id - 1] = reconstruction.points3D[point3D_id].xyz
        points3D_rgb[point3D_id - 1] = reconstruction.points3D[point3D_id].color
    ...
    return points3D, points3D_rgb, extrinsics, intrinsics, extra_params
...
def batch_np_matrix_to_pycolmap_wo_track(
    ...
    N = len(extrinsics)
    P = len(points3d) if points3d is not None else 0
