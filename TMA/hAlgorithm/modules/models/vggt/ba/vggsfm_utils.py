...
def generate_rank_by_dino(
    images, query_frame_num, image_size=336, model_name="dinov2_vitb14_reg", device="cuda", spatial_similarity=False, pretrain=None, dino_v2_model=None
):
    """
    ...
    if dino_v2_model is None:
        dino_v2_model = torch.hub.load("hAlgorithm/modules/models/facebookresearch_dinov2_main", model_name, source="local", pretrained=False)
        if pretrain is not None:
            dino_v2_model.load_state_dict(torch.load(pretrain, map_location="cpu", weights_only=False), strict=True)
