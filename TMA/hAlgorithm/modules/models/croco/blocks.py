...
class Attention(nn.Module):
    def __init__(
        ...
        self.attn_bias_scale = (
            head_dim**-0.5 * (gamma * math.log(inference_seqlen) / math.log(train_seqlen)) ** 0.5
        )
    ...
    def forward(self, x, xpos):
        ...
        if self.rope is not None:
            with torch.autocast(
                device_type=next(self.parameters()).device.type, dtype=torch.float32
            ):  # FIXME: for some reason Lightning didn't pick up torch.cuda.amp.custom_fwd when using bf16-true
        ...
        if self.attn_implementation == "pytorch_naive":
            ...
            ), "attn_mask not supported for pytorch_naive implementation of scaled dot product attention"
            assert (
                self.is_causal is False
            ), "is_causal not supported for pytorch_naive implementation of scaled dot product attention"
