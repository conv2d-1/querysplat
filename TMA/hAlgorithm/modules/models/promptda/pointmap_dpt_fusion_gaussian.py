...
from hAlgorithm.modules.models.gaussians.gaussian_head import GaussianHead
...
class MTPointMapMultiFrameDPTHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pred_gaussians = gaussians_cfg != None
        if self.pred_gaussians:
            self.gaussian_head = GaussianHead(head_features_1 // 2, gaussians_cfg)
        else:
            print("No gaussian head")
    ...
    def forward(
        ...
        """
        # NOTE: step2: pre cross-view feature fusion (cvf)
        out_list = self.dino_fuser(out_features, patch_h, patch_w)
        path_list = []
        # NOTE: step2.5: cvf fusion with prompt_depth
        for i, out in enumerate(out_list):
            path = self.get_fusion_path_single_batch(out, prompt_depth=prompt_depth[:, i, ...])
            path_list.append(path)

        # NOTE: step2.5: post cvf fusion
        # TODO: why calling this "path"?
        # TODO: allow different number of views
        # TODO: should output feature of all views
        # path_list = self.path_fuser(path_list)

        # NOTE: step 3: depth prediction for each view from cvf
        results_list = []
        depths_list = []
        feats_list = []
        for path in path_list:
            depth_out, feat_out = self.get_output_single_batch(path, patch_h, patch_w)
            results_list.append(depth_out)
            depths_list.append(depth_out["depth"].unsqueeze(1))  # [B, 1, 1, H, W] * v
            feats_list.append(feat_out.unsqueeze(1))  # [B, 1, C, H, W] * v

        gaussians = None
        if self.pred_gaussians:
            depths = rearrange(torch.cat(depths_list, dim=1), "b v c h w -> (b v) c h w")
            feats = rearrange(torch.cat(feats_list, dim=1), "b v c h w -> (b v) c h w")
            # depth = rearrange(prompt_depth, "b v c h w -> (b v) c h w")[:, 2:, ...]
            gaussians = self.gaussian_head(images, feats, depths, extrinsics, intrinsics)

        return results_list, gaussians
