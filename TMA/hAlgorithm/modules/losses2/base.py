...
class Loss(nn.Module):
    def __init__(self, loss_weight=1.0, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if isinstance(value, dict):
                assert "default" in value, f"{key} should contain 'default'."
    ...
    def get_param(self, key, name=None):
        assert hasattr(self, key), f"{key} not found in {self.__class__.__name__}."
    ...
    def forward(self, target=None, predict=None, mask=None, name=None, **kwargs):
        return None
...
def torch_quantile(
    ...
    *,
    ...
    if interpolation == "nearest":
        ...
        raise ValueError("Supported interpolations currently are {'nearest', 'lower', 'higher'} " f"(got '{interpolation}')!")
