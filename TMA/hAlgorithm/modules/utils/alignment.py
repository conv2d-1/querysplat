...
def align_depth_least_square(
    ...
    valid_mask = valid_mask_arr.squeeze().astype(bool)
...
def align_least_square_batch(
    prediction_ori, target_ori, mask, return_scale_shift=False, debug=False
):
    """
    prediction_ori: [B, H, W]
    target_ori: [B, H, W]
    mask: [B, H, W]
    """
    sum_axes = (-2, -1)
    # system matrix: A = [[a_00, a_01], [a_10, a_11]]
    prediction, target = prediction_ori * mask, target_ori * mask
    prediction[~mask] = 0
    target[~mask] = 0

    a_00 = torch.sum(prediction * prediction, sum_axes)
    a_01 = torch.sum(prediction, sum_axes)
    a_11 = torch.sum(mask, sum_axes)

    # right hand side: b = [b_0, b_1]
    b_0 = torch.sum(prediction * target, sum_axes)
    b_1 = torch.sum(target, sum_axes)
    ...
    scale_expanded = x_0.unsqueeze(1).unsqueeze(2)
    shift_expanded = x_1.unsqueeze(1).unsqueeze(2)

    aligned = prediction_ori * scale_expanded + shift_expanded

    if debug:
        print("scale", scale_expanded, "shift", shift_expanded)
        import matplotlib.pyplot as plt

        plt.figure("rel_depth and gt points in l-s fitter")
        plt.subplot(1, 2, 1)
        plt.scatter(prediction[mask].detach().cpu().numpy(), target[mask].detach().cpu().numpy())
        plt.ylabel("target")
        plt.xlabel("prediction")
        plt.subplot(1, 2, 2)
        prediction_inv = 1.0 / prediction - 1.0
        plt.scatter(
            prediction_inv[mask].detach().cpu().numpy(), target[mask].detach().cpu().numpy()
        )
        plt.ylabel("target")
        plt.xlabel("inv prediction")
        plt.show()
        breakpoint()

    if return_scale_shift:
        return aligned, scale_expanded, shift_expanded
    else:
        return aligned
