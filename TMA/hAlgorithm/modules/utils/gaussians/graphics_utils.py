...
def mat_to_quat_trans(mat):
    ...
    if trace > 0:
        ...
        if rot[0, 0] > rot[1, 1] and rot[0, 0] > rot[2, 2]:
            # R_{00} 最大
            s = 2 * torch.sqrt(1 + rot[0, 0] - rot[1, 1] - rot[2, 2])
            w = (rot[2, 1] - rot[1, 2]) / s
            x = 0.25 * s
            y = (rot[0, 1] + rot[1, 0]) / s
            z = (rot[0, 2] + rot[2, 0]) / s
            ...
            # R_{11} 最大
            s = 2 * torch.sqrt(1 + rot[1, 1] - rot[0, 0] - rot[2, 2])
            w = (rot[0, 2] - rot[2, 0]) / s
            x = (rot[0, 1] + rot[1, 0]) / s
            y = 0.25 * s
            z = (rot[1, 2] + rot[2, 1]) / s
            ...
            # R_{22} 最大
            s = 2 * torch.sqrt(1 + rot[2, 2] - rot[0, 0] - rot[1, 1])
            w = (rot[1, 0] - rot[0, 1]) / s
            x = (rot[0, 2] + rot[2, 0]) / s
            y = (rot[1, 2] + rot[2, 1]) / s
            z = 0.25 * s
...
def quat_trans_to_mat(quat, trans):
    w, x, y, z = quat
    quat_mat = torch.tensor([
