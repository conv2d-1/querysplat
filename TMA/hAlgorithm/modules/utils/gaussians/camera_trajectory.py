...
# https://github.com/Drexubery/ViewCrafter/blob/main/utils/pvd_utils.py
def interpolate_poses_spline(poses, n_interp, spline_degree=5, smoothness=0.03, rot_weight=0.1):
    ...
    extra_row = torch.tensor(
        np.repeat([[0, 0, 0, 1]], n_interp * (points.shape[0] - 1), axis=0), dtype=torch.float32
    ).unsqueeze(1)
