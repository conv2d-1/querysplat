...
@torch.no_grad()
def calculate_loss_mask(batch, novel_batch):
    """
    Calcuate the loss mask for the target views in the batch

    Args: batch: batch data of the target views, aka the data to inference gaussian
          novel_batch: batch data of the novel views, aka the target views
    """

    context_depth = batch["depth"].squeeze(2)
    context_intrinsics = batch["intrinsics"]
    context_c2w = batch["extrinsics"].inverse()

    target_depth = novel_batch["depth"].squeeze(2)  # (b, v, h, w)
    target_intrinsics = novel_batch["intrinsics"]  # (b, v, 3, 3)
    target_c2w = novel_batch["extrinsics"].inverse()  # (b, v, 4, 4)

    mask = calculate_in_frustum_mask(
        target_depth, target_intrinsics, target_c2w, context_depth, context_intrinsics, context_c2w
    )
    return mask
