        ...
        if batch_valid_mask.sum() == 0:
            ...
            if frame_num > 0:
                loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(
                    ...
                    gt_pose_encoding[batch_valid_mask][:, :frame_num].clone(),
                    loss_type=loss_type,
                )
            else:
                loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(
...
def camera_loss_single(cur_pred_pose_enc, gt_pose_encoding, loss_type="l1", delta=1.0, scale=None):
    if loss_type == "l1":
        if scale is not None:
            loss_T = (cur_pred_pose_enc[..., :3] * scale[..., 0, 0] - gt_pose_encoding[..., :3]).abs()
        else:
        ...
        if scale is not None:
            loss_T = (cur_pred_pose_enc[..., :3] * scale[..., 0, 0] - gt_pose_encoding[..., :3]).norm(dim=-1, keepdim=True)
        else:
        ...
        loss_T = huber_loss(
            cur_pred_pose_enc[..., :3], gt_pose_encoding[..., :3], delta=delta, name="t"
        )
        loss_R = huber_loss(
            cur_pred_pose_enc[..., 3:7], gt_pose_encoding[..., 3:7], delta=delta, name="r"
        )
        loss_fl = huber_loss(
            cur_pred_pose_enc[..., 7:], gt_pose_encoding[..., 7:], delta=delta, name="fl"
        )
    else:
...
def point_loss(
    ...
    **kwargs,
    ...
    conf_loss_dict = conf_loss_fn(
        ...
        gradient_loss=gradient_loss,
        valid_range=valid_range,
        camera_centric_reg=camera_centric_reg,
        disable_conf=disable_conf,
        all_mean=all_mean,
    )
...
def conf_loss(
    ...
    if disable_conf:
        ...
        conf_loss_first_frame = gamma * loss_reg_first_frame * first_frame_conf[
            first_frame_mask
    ...
    loss_dict = {
        ...
        f"loss_reg1{postfix}": (
            loss_reg_first_frame.detach().mean() if loss_reg_first_frame.numel() > 0 else 0
        ),
        f"loss_reg2{postfix}": (
            loss_reg_other_frames.detach().mean() if loss_reg_other_frames.numel() > 0 else 0
        ),
...
def point_map_to_normal(point_map, mask, eps=1e-6):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        pts = F.pad(point_map.permute(0, 3, 1, 2), (1, 1, 1, 1), mode="constant", value=0).permute(
            0, 2, 3, 1
        )
...
class VGGTCameraLoss(nn.Module):
    def __init__(
        self,
        loss_type="l1",
        gamma=0.6,
...
class VGGTCameraLossV2(VGGTCameraLoss):
    """VGGTCameraLossV2, supervision for each coordinate system."""
    ...
    def normalize_cameras(self, base_index, w2c_pred, w2c_gt):
        ...
        return cur_w2c_pred, cur_w2c_gt
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(num_predictions):
            ...
            for ni in range(N):
                try:
                    cur_extrinsic_pred, cur_extrinsic_gt = self.normalize_cameras(ni, extrinsic_pred, extrinsic_gt)
                except:
                    continue
...
class VGGTCameraLossV3(VGGTCameraLossV2):
    """VGGTCameraLossV3, Pred_T * scale - GT_T"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
