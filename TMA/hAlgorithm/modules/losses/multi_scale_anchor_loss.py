...
class MultiScaleAnchorLoss(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(self.alpha) == len(self.alpha_weights)
    ...
    def forward(self, prediction, target, mask=None, name=None, **kwargs):
        ...
        mask = mask[valid]
        intrinsics = kwargs["intrinsics"][valid]
        ...
        try:
            ...
            neig_masks = self.find_neighbors(
                target_anchors,
                target,
                focal=intrinsics[:, 0:1, 0],
                alpha=self.alpha,
            )
            loss = 0
            for ni, neig_mask in enumerate(neig_masks):
                ...
                loss += torch.mean(diff) * self.alpha_weights[ni]
        except Exception:
            loss = 0 * torch.sum(prediction)
            logging.warning(f"Data {name}, MultiScaleAnchorLoss sample_anchors error")
            return loss

        if torch.isnan(loss).item() | torch.isinf(loss).item():
            loss = 0 * torch.sum(prediction)
            logging.warning(f"Data {name}, MultiScaleAnchorLoss NAN error, {loss}")
    ...
    def debug_tool(
        ...
        save_path = os.path.join("debug/MultiScaleAnchorLoss", "target.ply")
        o3d.io.write_point_cloud(save_path, pcd)

        points = target_anchors.cpu().numpy()[0].reshape(-1, 3)
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points)
        save_path = os.path.join("debug/MultiScaleAnchorLoss", "target_anchors.ply")
        o3d.io.write_point_cloud(save_path, pcd)
