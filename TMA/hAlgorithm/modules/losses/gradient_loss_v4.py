    ...
    def forward(self, C_hat, C, mask, p=1, operator="scharr"):
        ...
        for j in range(self.M):
            ...
            if isinstance(grad_C, (list, tuple)):
                ...
                diff_x = (
                    (torch.abs(grad_C[0] - grad_C_hat[0]) ** p) * mask_9
                ).sum() / mask_9.sum().clip(1)
                diff_y = (
                    (torch.abs(grad_C[1] - grad_C_hat[1]) ** p) * mask_9
                ).sum() / mask_9.sum().clip(1)
    ...
    def debug_func(self, grad_C, grad_C_hat, m, p, operator):
        ...
        for batch_idx in range(grad_C.shape[0]):
            ...
            filename = os.path.join(
                save_dir, f"gt_scale_{m}_operator_{operator}_p{p}_b{batch_idx}.png"
            )
            ...
            filename = os.path.join(
                save_dir, f"pred_scale_{m}_operator_{operator}_b{batch_idx}.png"
            )
            plt.savefig(filename)
            plt.close(fig)
