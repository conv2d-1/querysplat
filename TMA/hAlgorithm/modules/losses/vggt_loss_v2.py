...
from .base import filter_by_quantile, check_and_fix_inf_nan
...
def compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    gt_points_mask = batch['point_masks'].clone()
    ...
    loss_conf, loss_reg, loss_grad, loss_normal = regression_loss(pred_points, gt_points, gt_points_mask, conf=pred_points_conf,
    ...
    loss_dict = {
        ...
        f"loss_normal_point": loss_normal,
    }
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    loss_conf, loss_reg, loss_grad, loss_normal = regression_loss(pred_depth, gt_depth, gt_depth_mask, conf=pred_depth_conf,
                                             gradient_loss_fn=gradient_loss_fn, gamma=gamma, alpha=alpha, valid_range=valid_range, )
    ...
    loss_dict = {
        ...
        f"loss_normal_depth": loss_normal,
    }
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    if "normal" in gradient_loss_fn:
        ...
        loss_normal = gradient_loss_multi_scale_wrapper(
    ...
    return loss_conf, loss_reg, loss_grad, loss_normal
