...
class MVFRPipeline(Pipeline):
    ...
    def infer(self, **batch):
        ...
        # inputs = dict(img=(image + 1) * 0.5) # NOTE
        inputs = dict(img=image)  # NOTE

        if extrinsics is not None:
            inputs["camera_pose"] = extrinsics.inverse()
        if prompt_depth is not None:
            inputs["depthmap"] = prompt_depth
        if intrinsics is not None:
            inputs["camera_intrinsics"] = intrinsics
        ...
        pred_extrinsics = results.get("camera_poses")  # C2W pose (OpenCV) in world coordinates: [B, S, 4, 4]
        if pred_extrinsics is not None:
            pred_extrinsics = pred_extrinsics.inverse()

        pred_intrinsics = results.get("camera_intrs")  # Camera intrinsic matrix: [B, S, 3, 3]

        pred_local_depth = results.get("depth")
        if pred_local_depth is not None:
            pred_local_depth = pred_local_depth.squeeze(-1).unsqueeze(2)
        pred_local_conf = results.get("depth_conf")
        if pred_local_conf is not None:
            pred_local_conf = pred_local_conf.unsqueeze(2)

        pred_global_points = results.get("pts3d")
        if pred_global_points is not None:
            pred_global_points = pred_global_points.permute(0, 1, 4, 2, 3).contiguous()
        pred_global_conf = results.get("pts3d_conf")
        if pred_global_conf is not None:
            pred_global_conf = pred_global_conf.unsqueeze(2)

        pred_local_normal = results.get("normals")
        if pred_local_normal is not None:
            pred_local_normal = pred_local_normal.permute(0, 1, 4, 2, 3).contiguous()
        pred_local_normal_conf = results.get("normals_conf")

        render_rgb = results.get("rendered_colors")
        if render_rgb is not None:
            render_rgb = render_rgb.permute(0, 1, 4, 2, 3).contiguous()
        render_depth = results.get("rendered_depths")
        if render_depth is not None:
            render_depth = render_depth.squeeze(-1)

        # Save Gaussians PLY and render video
        if "splats" in results:
            # Get Gaussian parameters (already filtered by GaussianSplatRenderer)
            means = results["splats"]["means"][0].reshape(-1, 3)
            scales = results["splats"]["scales"][0].reshape(-1, 3)
            quats = results["splats"]["quats"][0].reshape(-1, 4)
            colors = (results["splats"]["sh"][0] if "sh" in results["splats"] else results["splats"]["colors"][0]).reshape(-1, 3)
            opacities = results["splats"]["opacities"][0].reshape(-1)
        ...
        for fi in range(frame_num):
            for vi in range(view_num):
                ...
                single.rgb = image[0, index].permute(1, 2, 0).cpu().numpy()
