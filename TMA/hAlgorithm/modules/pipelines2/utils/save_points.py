...
def save_colmap_points3D_bin(points, colors, path, errors=None, tracks=None):
    """
    Save point cloud to COLMAP's points3D.bin format.

    Args:
        points: (N, 3) float64 or float32 → will be cast to float64
        colors: (N, 3) uint8
        path: output .bin file path
        errors: (N,) float64, reprojection error; if None, use -1.0
        tracks: list of list, each inner list = [img_id1, idx1, img_id2, idx2, ...]
                if None, all tracks are empty
    """
    N = points.shape[0]
    ...
    if errors is None:
        errors = np.full(N, -1.0, dtype=np.float64)
    else:
    ...
    with open(path, "wb") as f:
        # Write number of 3D points (uint64)
        ...
        for i in range(N):
            ...
            err = errors[i]
            track = tracks[i]
            track_length = len(track) // 2  # because track = [img_id, point2D_idx, ...]
            ...
            # Write track length (uint64)
            f.write(struct.pack("<Q", track_length))

            # Write track observations (each: image_id uint32, point2D_idx uint32)
            for j in range(0, len(track), 2):
                img_id = int(track[j])
                p2d_idx = int(track[j + 1])
                f.write(struct.pack("<II", img_id, p2d_idx))

    print(f"Saved {N} points to {path} in COLMAP binary format.")


def save_colmap_points3D(points, colors, filename, errors=None, tracks=None):
    """
    Save point cloud in COLMAP's points3D.txt format.

    Args:
        points (np.ndarray): (N, 3) float, 3D coordinates
        colors (np.ndarray): (N, 3) uint8, RGB colors [0, 255]
        filename (str): Output file path (e.g., "points3D.txt")
        errors (np.ndarray or None): (N,) float, reprojection error. If None, use -1.
        tracks (list of list or None): Observation tracks. Each track is a list of [image_id, point2d_idx, ...].
                                       If None, leave empty.
    """
    N = points.shape[0]
    assert colors.shape[0] == N, "Points and colors must have same number of points"
    assert colors.dtype == np.uint8, "Colors must be uint8 (0-255)"

    if errors is None:
        errors = np.full(N, -1.0, dtype=np.float32)
    else:
    ...
    with open(filename, "w") as f:
        ...
        for i in range(N):
            ...
            r, g, b = colors[i]
            err = errors[i]
            track_str = " ".join(map(str, tracks[i])) if tracks[i] else ""

            line = f"{point_id} {x:.6f} {y:.6f} {z:.6f} {r} {g} {b} {err:.6f} {track_str}".strip()
            f.write(line + "\n")

    print(f"Saved {N} points to {filename} in COLMAP format.")
...
def compute_alignment_error(point_map1, conf1, point_map2, conf2, conf_threshold, s, R, t):
    ...
    print(f"Alignment error statistics [using {len(errors)} points]: " f"mean={mean_error:.4f}, std={std_error:.4f}, " f"median={median_error:.4f}, max={max_error:.4f}")
...
def save_confident_pointcloud_batch(
    ...
    for i in range(b):
        ...
        total_valid += np.count_nonzero(cfs >= conf_threshold)
    ...
    if sample_ratio == 1.0:
        ...
        for i in range(b):
            ...
            if count < num_samples:
                ...
                count, reservoir_pts, reservoir_clr = optimized_vectorized_reservoir_sampling(valid_pts, valid_cls, count, reservoir_pts, reservoir_clr)
...
#         if valid_mask1 is not None and valid_mask1[i] is not None:
#             valid_mask = valid_mask & valid_mask1[i]
#         if valid_mask2 is not None and valid_mask2[i] is not None:
#             valid_mask = valid_mask & valid_mask2[i]

#         print(valid_mask1[i].sum(), valid_mask2[i].sum())
