...
def save_depth_map(depth, out_dir, file_prefix, data_idx, cmap="turbo", min_val=None, max_val=None, info=False):
...
def save_gradient(depth_grad, out_dir, data_idx, info=False):
    ...
    depth_grad_u = colorize_depth_maps(depth_grad_u, depth_grad_u.min(), depth_grad_u.max(), cmap="turbo")
    depth_grad_v = colorize_depth_maps(depth_grad_v, depth_grad_v.min(), depth_grad_v.max(), cmap="turbo")
...
def save_error(pred_depthmap, gt_depthmap, out_dir, file_prefix, data_idx, info=False):
    ...
    try:
        ...
        gt_depthmap_color = colorize_depth_maps(gt_depthmap, gt_depthmap.min(), gt_depthmap.max(), cmap="turbo")
...
def save_normal(normal, out_dir, file_prefix, data_idx, mask=None, cmap="turbo", info=False):
...
def save_warp(p1, p2, warp, overlap, out_dir, file_prefix, data_idx, info=False):
    if warp is None or overlap is None:
        return None
    import torch
    ...
    warp_im = F.grid_sample(
        torch.as_tensor(p2, device='cuda').float().permute(2, 0, 1)[None], torch.as_tensor(warp, device='cuda').float(),
        mode="bilinear", align_corners=False
    )[0].permute(1, 2, 0)
    white_im = torch.ones((H, W, 3)).cuda() * 255
    warp_vis = overlap * warp_im + (1 - overlap) * white_im
    warp_vis = warp_vis.squeeze().cpu().numpy().astype(np.uint8)
    combined = np.concatenate([p1, warp_vis, p2], axis=1)
