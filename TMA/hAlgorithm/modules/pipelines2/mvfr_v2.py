...
from hAlgorithm.modules.utils.ray import (
    ...
    get_rays_in_camera_frame_fisheye,
...
class MVFRPipeline(Pipeline):
    """Pipeline for Multi-View Feed-forward Reconstruction. Supports MVFR Match Training."""
    ...
    def __init__(
        ...
        target_match_gt_intrinsics_name=None,
        precomputed_warp_name=None,
        precomputed_warp_mask_name=None,
        align_name=None,
        rgb_mask=None,
        # extra inputs
        ...
        segformer_cfg=None,
        # others
        coarse_match_name="coarse",
        final_match_name="final",
        **kwargs,
    ):
        ...
        self.target_match_gt_intrinsics_name = target_match_gt_intrinsics_name
        self.precomputed_warp_name = precomputed_warp_name
        self.precomputed_warp_mask_name = precomputed_warp_mask_name
        self.align_name = align_name
        self.rgb_mask = rgb_mask

        self.coarse_match_name = coarse_match_name
        self.final_match_name = final_match_name
    ...
    def get_inputs(self, batch):
            ...
            if self.with_ray_directions:
                ...
                if camera_type == "PINHOLE":
                    ...
                    ray_directions = get_rays_in_camera_frame_fisheye(
                        ...
                        k_coeffs=distort_k.reshape(-1, 5),
                        sensor_size=sensor_size.reshape(-1, 2),
                        crop_offset=crop_offset.reshape(-1, 2) if crop_offset is not None else None,
                    )
                else:
        ...
        if self.extrinsics_name is not None and self.extrinsics_name in batch:
            ...
            if self.extrinsics_noise_name is not None and self.extrinsics_noise_name in batch:
        ...
        target_match_gt_depth = target_match_gt_extrinsics = target_match_gt_intrinsics = None
        ...
        if self.target_match_gt_intrinsics_name is not None and self.target_match_gt_intrinsics_name in batch:
            target_match_gt_intrinsics = batch[self.target_match_gt_intrinsics_name].to(self.device)

        precomputed_warp = precomputed_warp_mask = None
        if self.precomputed_warp_name is not None and self.precomputed_warp_name in batch:
            precomputed_warp = {
                k: v.to(device=self.device, dtype=image.dtype) for k, v in batch[self.precomputed_warp_name].items()
            }
            warp_mask_key = self.precomputed_warp_mask_name
            if warp_mask_key is None:
                warp_mask_key = f"{self.precomputed_warp_name}_mask"
            if warp_mask_key in batch:
                precomputed_warp_mask = {
                    k: v.to(device=self.device) for k, v in batch[warp_mask_key].items()
                }
            ...
            if precomputed_pair_idx is not None:
                ...
                meta_data["pair_idx"] = precomputed_pair_idx
        ...
        if self.training and self.da3_model is not None and ("names" not in self.da3_model_cfg or name in self.da3_model_cfg["names"]):
            target_local_depth, target_depth_mask, prompt_depth, prompt_depth_mask, intrinsics = self.infer_da3_model(
                image=image,
        ...
        if self.training and self.segformer is not None and target_invalid_mask is None:
            target_invalid_mask = self.infer_segformer(image=image)
        ...
        if image.ndim == 4:
            ...
            target_match_gt_intrinsics = add_dim(target_match_gt_intrinsics)
    ...
    def infer_sv_model(self, image, scale, prompt_depth, prompt_depth_mask, meta_data):
        ...
        with torch.inference_mode():
            ...
            local_depth = results["depth"].to(self.dtype)
    ...
    def infer_da3_model(self, image, intrinsics, extrinsics, prompt_depth_mask):
        ...
        depth = depth.clone()
        depth_mask = depth_mask.bool().clone()
        prompt_depth = prompt_depth.to(self.dtype).clone()
        prompt_depth_mask = prompt_depth_mask.bool().to(self.dtype).clone()
        intrinsics = intrinsics.clone()

        return depth, depth_mask, prompt_depth, prompt_depth_mask, intrinsics

    def infer_segformer(self, image):
        with torch.inference_mode():
            with torch.autocast("cuda", enabled=True, dtype=self.dtype):
    ...
    def get_track_inputs(self, batch):
        ...
        if "track_query_points" in batch:
            track_query_points = batch["track_query_points"].to(device=self.device, dtype=self.dtype)
    ...
    def get_warp_gt(self, depths, intrinsics, extrinsics, pair_idx, H, W, camera_type="PINHOLE", meta_data=None):
        ...
        for pair in pair_idx:
            depth1 = depths[:, pair[0], -1] # B, H_depth, W_depth
            ...
            for scale in self.match_scales:
                ...
                if camera_type == "FISHEYE_BLENDER":
                    ...
                    distort_k = meta_data["distort_k"]  # [B, V, 5] or [B, 5]
                    sensor_size = meta_data["sensor_size"]  # [B, V, 2] or [B, 2]
                    crop_offset = meta_data.get("crop_offset", None)  # [B, V, 2] or [B, 2] or None
                    assert distort_k is not None and sensor_size is not None
        ...
        if match_results is not None and self.dense_match_loss is not None:
            ...
            if target_match_gt_depth is not None:
                assert target_match_gt_intrinsics is not None, \
                    "target_match_gt_depth requires matching target_match_gt_intrinsics"
            ...
            loss, loss_dict = self.get_dense_match_loss(
                ...
                depths=match_gt_depth, intrinsics=match_gt_intrinsics,
    ...
    def infer(self, **batch):
        ...
        if pred_ray_directions is not None:
            pred_ray_directions = pred_ray_directions / pred_ray_directions.norm(dim=2, keepdim=True).clip(min=1e-8)
            if rgb_mask is not None:
                pred_ray_directions = pred_ray_directions * rgb_mask.float()
        ...
        # 'coarse', 'final', 'pair_idx', 'refiner_4', 'refiner_2', 'refiner_1'
        if pred_matching is not None:
            ...
            if self.final_match_name in pred_matching:
                warp_AB, confidence_AB = pred_matching[self.final_match_name]["warp_AB"], pred_matching[self.final_match_name]["confidence_AB"]
            if self.coarse_match_name in pred_matching:
                coarse_warp_AB, coarse_confidence_AB = pred_matching[self.coarse_match_name]["warp_AB"], pred_matching[self.coarse_match_name]["confidence_AB"]
                coarse_overlap_AB = coarse_confidence_AB
            
            warp_refine = {name.split("_")[1]:pred_matching[name]["warp_AB"] for name in ['refiner_4', 'refiner_2']}
            ...
            if precomputed_warp is not None:
                warp = precomputed_warp.get(1, None)
                warp_mask = precomputed_warp_mask.get(1, None) if precomputed_warp_mask is not None else None
            else:
                if target_match_gt_depth is not None:
                    assert target_match_gt_intrinsics is not None, \
                        "target_match_gt_depth requires matching target_match_gt_intrinsics"
                ...
                if match_gt_depth is not None:
                    camera_type = meta_data.get("camera_type", ["PINHOLE"])[0] # Batch
                    if isinstance(camera_type, (list, tuple)):
                        camera_type = camera_type[0] # View
        ...
        for fi in range(frame_num):
            for vi in range(view_num):
                ...
                if num_pair > 0 and pred_matching is not None:
                    # find all pair that pair[0] == index
                    ...
                    for pair_i, cur_pair in cur_pairs:
                        ...
                        matching_results.append(DenseMatchingOutput(
                            ...
                            warp_coarse=bhwc_interpolate(coarse_warp_AB[:, pair_i], (H, W)) if coarse_warp_AB is not None else None,
                            overlap_coarse=bhwc_interpolate(coarse_overlap_AB[:, pair_i], (H, W)) if coarse_overlap_AB is not None else None,

                            warp_refine = {s:bhwc_interpolate(warp[:, pair_i], (H, W)) for s, warp in warp_refine.items()} if warp_refine is not None else None,
    ...
    def visualize(self, outputs_list, meta_data, out_dir):
        ...
        # Dense Match Visualization
        if self.save_output_cfg.get("save_match"):
            os.makedirs(match_out_dir, exist_ok=True)
            vis_match_results(cfg=self.save_output_cfg, mv_outputs=outputs_list, match_out_dir=match_out_dir, data_idx=data_idx, frame_num=frame_num, view_num=view_num, gt_out_dir=gt_match_out_dir)
