...
from hAlgorithm.modules.models.vggt.utils.geometry import *
...
def build_tracks_by_depth(
    ...
    if sample_frames > 0:
        T = min(sample_frames, B)
        t_indices = torch.randint(0, T, size=(H, W), device=world_points.device)  # [H, W]
        h_idx = torch.arange(H, device=world_points.device).view(-1, 1)  # [H, 1]
        w_idx = torch.arange(W, device=world_points.device).view(1, -1)  # [1, W]
        query_world_points = world_points[t_indices, h_idx, w_idx] # H,W,3
        query_point_masks = point_masks[t_indices, h_idx, w_idx] # H,W
    else:
    ...
    if (query_point_masks).sum() > 0:
        ...
        image_points, cam_points = project_world_points_to_cam(
            valid_query_points, extrinsics, intrinsics
        )
        ...
        for shift in [(0, 0), (1, 0), (0, 1), (1, 1)]:
            ...
            # NOTE: add mask
            cur_mask = point_masks[batch_indices, cur_uv_int[..., 1], cur_uv_int[..., 0]]
            cur_depth_inside_flag = cur_depth_inside_flag & cur_mask
        ...
        if debug:
            print(
                f"uv_inside_flag: {uv_inside_flag.sum()}, depth_inside_flag: {depth_inside_flag.sum()}"
            )

        # NOTE: Compute distances to current and projected points
        if points is not None:
            dis_diff_flag = [torch.ones(uv_int.shape[1]).bool()]
            N, _, H, W = points.shape
            for i in range(1, N):
                cur_mask = point_masks[i, uv_int[i, :, 1], uv_int[i, :, 0]]
                cur_points = points[i, :, uv_int[i, :, 1], uv_int[i, :, 0]]
                cur_points_dis = torch.norm(cur_points, dim=0)
                projected_points_dis = torch.norm(cam_points[i, :3, :], dim=0)
                diff = (cur_points_dis - projected_points_dis).abs()

                cur_dis_inside_flag = (
                    cur_mask
                    & (diff < cur_points_dis * pos_dis_thres)
                    & (diff < projected_points_dis * pos_dis_thres)
                )
                dis_diff_flag.append(cur_dis_inside_flag)

            dis_diff_flag = torch.stack(dis_diff_flag, dim=0)
            depth_inside_flag = torch.logical_and(depth_inside_flag, dis_diff_flag)

            if debug:
                print(
                    f"depth_inside_flag: {depth_inside_flag.sum()}, dis_diff_flag: {dis_diff_flag.sum()}"
                )
        ...
        positive_tracks = torch.zeros(
            B, target_track_num, 2, device=world_points.device, dtype=torch.float32
        )
        positive_vis_masks = torch.zeros(
            B, target_track_num, device=world_points.device, dtype=torch.bool
        )

    if neg_ratio > 0:
        sampled_neg_track_num = (
            target_track_num * 4
        )  # we sample more negative tracks to ensure the quality

        perb_range = [int(W * neg_sample_size_ratio), int(H * neg_sample_size_ratio)]

        # sample negative query points
        us = torch.randint(
            low=0, high=W, size=(1, sampled_neg_track_num), device=world_points.device
        )
        vs = torch.randint(
            low=0, high=H, size=(1, sampled_neg_track_num), device=world_points.device
        )
        neg_query_uvs = torch.stack([us, vs], dim=-1)

        # construct negative tracks
        delta_us = (
            torch.rand(size=(B, sampled_neg_track_num), device=world_points.device) * perb_range[0]
        )
        delta_vs = (
            torch.rand(size=(B, sampled_neg_track_num), device=world_points.device) * perb_range[1]
        )
        delta_us[0] = 0
        delta_vs[0] = 0
        ...
        # negative_uv_check = (
        #     (negative_tracks[..., 0] >= boundary_thres)
        #     & (negative_tracks[..., 0] < (W - boundary_thres))
        #     & (negative_tracks[..., 1] >= boundary_thres)
        #     & (negative_tracks[..., 1] < (H - boundary_thres))
        # )
        # negative_uv_check = negative_uv_check.sum(0) >= negative_tracks.shape[0]

        # negative_epipolar_check = negative_epipolar_check & negative_uv_check
    ...
    sampled_positive_tracks, sampled_positive_vis_masks = sample_positive_tracks(
        positive_tracks, positive_vis_masks, target_pos_track_num, seed=seed, sample_frames=sample_frames
    )
    ...
    if target_neg_track_num > 0:
        ...
        final_tracks[:, sampled_pos_track_num : sampled_pos_track_num + sampled_neg_track_num] = (
            sampled_neg_tracks
        )
    else:
        sampled_neg_track_num = 0
...
def sample_positive_tracks(tracks, tracks_mask, track_num, half_top=True, seq_name=None, seed=None, sample_frames=0):
    # tracks: (B, T, 2)
    # tracks_mask: (B, T)
    # track_num: int
    # half_top: bool
...
def track_epipolar_check(tracks, extrinsics, intrinsics, use_essential_mat=False):
    ...
    if use_essential_mat:
        ...
        sampson_distances = sampson_epipolar_distance(
            tracks[0:1].expand(B - 1, -1, -1), tracks[1:], fundamental_mats
        )
...
def visualize_tracks_on_images(
    ...
    for b in range(B):
        ...
        for s in range(S):
            ...
            if track_vis_mask is not None:
                invalid_indices = torch.where(~track_vis_mask[b, s])[0]
                for i in invalid_indices:
                    x, y = cur_tracks_np[i]
                    pt = (int(round(x)), int(round(y)))
                    if pt[0] < 0 or pt[0] >= W or pt[1] < 0 or pt[1] >= H:
                        continue
                    cv2.circle(img_bgr, pt, radius=3, color=(0, 0, 0), thickness=-1)
            else:
            ...
            if s == 0:
                first_img_rgb = img_rgb
                first_cur_tracks_np = cur_tracks_np
            else:
                merge_img_rgb = np.concatenate([first_img_rgb, img_rgb], axis=1)
                first_valid_indices = track_vis_mask[b, 0]
                for i in valid_indices:
                    if first_valid_indices[i]:
                        x, y = first_cur_tracks_np[i]
                        pt0 = (int(round(x)), int(round(y)))

                        x, y = cur_tracks_np[i]
                        pt1 = (int(round(x)) + W, int(round(y)))

                        # track_colors_rgb[i] is (R,G,B). For OpenCV circle, we need BGR
                        R, G, B = track_colors_rgb[i]
                        color_rgb = (int(R), int(G), int(B))
                        cv2.line(merge_img_rgb, pt0, pt1, color=color_rgb, thickness=1)
                    else:
                        x, y = first_cur_tracks_np[i]
                        pt0 = (int(round(x)), int(round(y)))
                        ...
                        x, y = cur_tracks_np[i]
                        pt1 = (int(round(x)) + W, int(round(y)))
                        if pt1[0] >= 2 * W or pt1[1] >= 2 * H:
                            continue

                        cv2.line(merge_img_rgb, pt0, pt1, color=(0, 0, 0), thickness=1)
...
def visualize_tracks_neg_on_images(
    ...
    Args:
        images: torch.Tensor (B, S, 3, H, W) if CHW or (B, S, H, W, 3) if HWC.
        tracks: torch.Tensor (B, S, N, 2), last dim = (x, y).
        track_pos_mask: torch.Tensor (B, S, N) or None.
        out_dir: folder to save visualizations.
        image_format: "CHW" or "HWC".
        normalize_mode: "[0,1]", "[-1,1]", or None for direct raw -> 0..255
        cmap_name: a matplotlib colormap name for color_from_xy.
    ...
    track_vis_mask = (~track_pos_mask.cpu().clone()).unsqueeze(1).repeat(1, S, 1).bool()
            ...
            if s == 0:
                ...
                for i in valid_indices:
                    if first_valid_indices[i]:
                        ...
                        R, G, B = track_colors_rgb[i]
                        color_bgr = (int(B), int(G), int(R))
                        cv2.line(merge_img_rgb, pt0, pt1, color=color_bgr, thickness=1)

                out_path = os.path.join(out_dir, f"tracks_neg_b{b}_0-{s}.png")
                cv2.imwrite(out_path, merge_img_rgb)

        # Concatenate all frames horizontally: (H, S*W, 3)
        row_img = np.concatenate(frame_images, axis=1)

        out_path = os.path.join(out_dir, f"tracks_neg_b{b}.png")
        cv2.imwrite(out_path, row_img)
        print(f"[INFO] Saved color-by-XY track visualization for sample b={b} -> {out_path}")
