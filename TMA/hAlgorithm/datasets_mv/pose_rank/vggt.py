...
def extrinsic_distance_batch_chunked(extrinsics, lambda_t=1.0, chunk_size=1000):
    ...
    for i_start in range(0, N, chunk_size):
        ...
        for j_start in range(0, N, chunk_size):
            ...
            dists[i_start:i_end, j_start:j_end] = (
                rot_diff[i_start:i_end, j_start:j_end] + lambda_t * trans_diff
            )
    return dists


def compute_ranking(
    extrinsics, lambda_t=1.0, normalize=True, batched=False, index=None, chunk_size=0
):
    if normalize:
        extrinsics = np.copy(extrinsics)
        camera_center = np.copy(extrinsics[:, :3, 3])
        camera_center_scale = np.linalg.norm(camera_center, axis=1)
        avg_scale = np.max(camera_center_scale)
        extrinsics[:, :3, 3] = extrinsics[:, :3, 3] / avg_scale

    if batched and (index is None):
        if len(extrinsics) > 6000 or chunk_size > 0:
            dists = extrinsic_distance_batch_chunked(
                extrinsics, lambda_t=lambda_t, chunk_size=chunk_size
            )
        else:
        ...
        if index is not None:
            dists = np.stack(
                [
                    extrinsic_distance(extrinsics[index], extrinsics[i], lambda_t=lambda_t)
                    for i in range(N)
                ],
                axis=0,
            )
            ...
            for i in range(N):
                for j in range(N):
                    dists[i, j] = extrinsic_distance(
                        extrinsics[i], extrinsics[j], lambda_t=lambda_t
                    )
...
if __name__ == "__main__":
    ...
    scenes = list(datas.keys())
    pose_rank = defaultdict(dict)

    for scene in tqdm(scenes, total=len(scenes), desc="Loading"):
        cur_total_datas = []
        frames = datas[scene]
        ...
        for view_id, views in tmp_infos.items():
            frame_id_list = [view["frame_id"] for view in views]
            rank = compute_ranking([np.linalg.inv(np.array(view["extrinsics"])) for view in views])
            for i, frame_id in enumerate(frame_id_list):
                if frame_id not in pose_rank[scene]:
                    pose_rank[scene][frame_id] = dict()
                pose_rank[scene][frame_id][view_id] = [[frame_id, views[k]["view_id"]] for k in rank[i][:256].tolist()]

    path = os.path.basename(json_path)[:-5] + "_pose_rank.json"
    with open(path, "w") as f:
        json.dump(pose_rank, f, ensure_ascii=False)

    # np.save(path, pose_rank)
