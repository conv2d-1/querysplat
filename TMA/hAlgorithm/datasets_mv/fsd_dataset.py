...
class FSDDatasetMV(BaseDatasetMV):
...
class FSDDatasetMVRandomBaseline(BaseDatasetMV):
    ...
    def process_stereo_images(
        self, left_img, right_img, disparity, intrinsic_left_param, intrinsic_right_param, baseline
    ):
        ...
        # 将有效的投影点填充到右相机深度图中
        valid_mask = (
            (u_right >= 0) & (u_right < width) & (v_right >= 0) & (v_right < height) & (Z_right > 0)
        )
    ...
    def load_mf_data(self, idx):
        ...
        (
            ...
            left_rgb, right_rgb, disparity, left_intrinsics, right_intrinsics, baseline
        )
