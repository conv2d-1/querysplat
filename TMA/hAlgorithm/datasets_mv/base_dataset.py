...
class BaseDatasetMV(BaseDataset):
    def __init__(
        self,
        ...
        if version == "1.0":
            ...
            for scene, info in data.items():
                ...
                for frame in frames:
                    ...
                    if cam_params is not None and f"cam{view_id}" in cam_params and len(cam_params[f"cam{view_id}"]["K"]) > 4:
                        continue
                    if cam_params is None or f"cam{view_id}" not in cam_params or len(cam_params[f"cam{view_id}"]["K"]) > 4:
                        continue
                    ...
                    cam2world_pose_mvg = frame.pop("cam2world_pose_mvg", None)
                    cam2world_pose_lcc = frame.pop("cam2world_pose_lcc", None)
                    ...
                    if self.prompt_extrinsics_name is not None:
                        ...
                        elif cam2world_pose_lcc is not None and self.prompt_extrinsics_type == "lcc":
                            frame[self.prompt_extrinsics_name] = np.linalg.inv(np.array(cam2world_pose_lcc)).tolist()
                        else:
                            continue
                    ...
                    if self.extrinsics_name is not None:
                        ...
                        elif cam2world_pose_mvg is not None and self.extrinsics_type == "mvg":
                            frame[self.extrinsics_name] = np.linalg.inv(np.array(cam2world_pose_mvg)).tolist()
                        elif cam2world_pose_lcc is not None and self.prompt_extrinsics_type == "lcc":
                            frame[self.extrinsics_name] = np.linalg.inv(np.array(cam2world_pose_lcc)).tolist()
                        else:
                            continue
                    ...
                    if cam_params is not None and self.intrinsics_name is not None:
                        frame[self.intrinsics_name] = cam_params[f"cam{int(view_id)}"]["K"]
    ...
    def load_mf_files(self, datas):
        ...
        for scene in tqdm(data_infos, total=len(data_infos), desc=f"{self.name}, Loading", disable=not is_main_process):
            if self.mf_scene is not None and len(self.mf_scene) > 0:
                if isinstance(self.mf_scene, str):
                    if scene != self.mf_scene:
                        continue
                elif isinstance(self.mf_scene, list):
                    if scene not in self.mf_scene:
                        continue
                else:
                    raise NotImplementedError(
                        f"mf_scene {self.mf_scene} is not implemented"
                    )
                print(f"scene: {scene}")
            if (
        ...
        if self.phase == "test":
            ...
            if self.debug:
                ...
                try:
                    ...
                    # 对于一些可预期的“跳过样本”型异常（例如 tracking GT 缺失），
                    # 不打印完整 traceback、防止日志刷屏，只做轻量日志并改用新索引重试。
                    msg = str(e)
                    is_expected_skip = (
                        isinstance(e, RuntimeError) and (
                            "no tracking GT found, skipping" in msg
                            or "static batch, skipping" in msg
                        )
                    ) or (
                        isinstance(e, ValueError) and (
                            "depth_mask is empty" in msg
                            or "sparse_gt is empty after filtering" in msg
                        )
                    )
                    if is_expected_skip:
                        logging.info(
                            f"Dataset:{self.name} skip invalid sample at index {idx}, reason: {e}"
                        )
                    else:
                        traceback.print_exc()
                        logging.error(
    ...
    def get_mf_data_for_trainval(self, idx):
        ...
        if len(self.kosmo_scene_infos) > 0:
    ...
    def normalize_cameras_base_first(self, datas):
        ...
        if "extrinsics" in datas[0]:
            ...
            for data in datas:
                ...
                if "pointmap_raw" in datas[0]:
                    data["pointmap_raw_reff"] = fix(data["pointmap_raw"], data["extrinsics_reff"])
...
if __name__ == "__main__":
    ...
    path = "hAlgorithm/configs/mv_v1.0/dataset_configs_matching_pair/train_all_260123.yaml"
