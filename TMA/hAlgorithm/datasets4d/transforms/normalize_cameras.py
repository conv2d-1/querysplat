...
def normalize_cameras(
    cameras, compute_optical=True, first_camera=True, scale=1.0, normalize_T=False
):
    """
    ...
    if normalize_T:
        new_cameras, t_scale = normalize_Trans(new_cameras)
        scale = scale * t_scale
...
def normalize_Trans(new_cameras):
    ...
    return new_cameras, t_gt_scale


def first_camera_transform(cameras):
    # Let distance from first camera to origin be unit
    new_cameras = cameras.clone()
    new_transform = new_cameras.get_world_to_view_transform()

    w2c = new_transform.get_matrix()
    w2c[:, :3, 3] = w2c[:, 3, :3]
    w2c[:, 3, :3] = 0

    first_extrinsics_inv = w2c[0].inverse()
    new_matrix = w2c @ first_extrinsics_inv.unsqueeze(0)
