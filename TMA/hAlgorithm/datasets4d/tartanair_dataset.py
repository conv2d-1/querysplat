...
class TartanAirDataset4D(BaseDataset4D):
    ...
    def load_inf_mask(self, curr_depth, curr_sem, curr_rgb, data_info, **kwargs):
        ...
        if (
            self.inf_mask_sem_names is not None and 
            len(self.inf_mask_sem_names) > 0 and
            curr_sem is not None and
            'sky' in semantic_labels
        ):
