...
# ignore_keys = ["data_info", "kosmo_frames", "kosmo_scene_infos", "camera_type"]
ignore_keys = [
    "data_info", "kosmo_frames", "kosmo_scene_infos", "camera_type", 
    "masks", "labels", "boxes", "area", "iscrowd", "label_data", 
    "trajs_2d", "trajs_3d", "valids", "visibs"
]



def default_convert(data, input_key=None):
    r"""
    ...
    if isinstance(data, torch.Tensor):
        ...
        try:
            if isinstance(data, collections.abc.MutableMapping):
                ...
                clone.update({key: default_convert(data[key], input_key=key) for key in data})
                return clone
            else:
                return elem_type({key: default_convert(data[key], input_key=key) for key in data})
        except TypeError:
            # The mapping type may not support `copy()` / `update(mapping)`
            # or `__init__(iterable)`.
            return {key: default_convert(data[key], input_key=key) for key in data}
...
def collate(batch, *, collate_fn_map: Optional[Dict[Union[Type, Tuple[Type, ...]], Callable]] = None, input_key=None):
    r"""
    ...
    if isinstance(elem, collections.abc.Mapping):
        try:
            if isinstance(elem, collections.abc.MutableMapping):
                ...
                clone.update({key: collate([d[key] for d in batch], collate_fn_map=collate_fn_map, input_key=key) for key in elem})
                return clone
            else:
                return elem_type({key: collate([d[key] for d in batch], collate_fn_map=collate_fn_map, input_key=key) for key in elem})
        except TypeError:
            # The mapping type may not support `copy()` / `update(mapping)`
            # or `__init__(iterable)`.
            return {key: collate([d[key] for d in batch], collate_fn_map=collate_fn_map, input_key=key) for key in elem}
...
def pad_trajs_to_batch_max(trajs_list, pad_value=0):
    """
    Pad trajectory tensors to the maximum size in the batch.
    
    Args:
        trajs_list: List of tensors with shape [N_i, ...] where N_i varies per sample
        pad_value: Value to use for padding (default: 0)
        
    Returns:
        Padded tensor of shape [B, max_N, ...]
        Valid mask of shape [B, max_N] indicating original (non-padded) positions
    """
    if not trajs_list:
        return None, None
    
    # Find max N across batch
    max_N = max(t.shape[0] for t in trajs_list)
    
    # Get the rest of the shape
    rest_shape = trajs_list[0].shape[1:]
    dtype = trajs_list[0].dtype
    device = trajs_list[0].device
    
    B = len(trajs_list)
    padded = torch.full((B, max_N, *rest_shape), pad_value, dtype=dtype, device=device)
    valid_mask = torch.zeros(B, max_N, dtype=torch.bool, device=device)
    
    for i, t in enumerate(trajs_list):
        N = t.shape[0]
        padded[i, :N] = t
        valid_mask[i, :N] = True
...
def collate_with_traj_padding(batch, traj_keys=('trajs_2d', 'trajs_3d', 'valids', 'visibs')):
    """
    Custom collate function that handles variable-length trajectory data.
    
    Trajectory keys are dynamically padded to batch maximum, other keys use default_collate.
    
    Args:
        batch: List of dicts from dataset __getitem__
        traj_keys: Tuple of keys that contain trajectory data needing dynamic padding
        
    Returns:
        Collated batch dict with padded trajectories
    """
    if not batch:
        return {}
    ...
    # Collect trajectory data separately
    ...
    # Collate non-trajectory data with default_collate
    if non_traj_batch[0]:  # If there's any non-traj data
        result = default_collate(non_traj_batch)
    
    # Pad trajectory data to batch max
    for k in traj_keys:
        if traj_data[k] and traj_data[k][0] is not None:
            padded, valid_mask = pad_trajs_to_batch_max(traj_data[k])
            result[k] = padded
            # Store the padding mask (only once, since all traj keys share same N)
