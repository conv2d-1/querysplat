...
class PoseRankDataset(MatchingDataset):
        ...
        for scene in tqdm(data_infos, total=len(data_infos), desc=f"{self.name}, Loading"):
            ...
            if self.mf_to_mv and not self.mf_to_sf:
                ...
                for view_id, views in tmp_infos.items():
                    ...
                    viewss = [[views[j] for j in rank[i][: self.clip_maxlen]] for i in range(len(rank))]
            ...
            if not self.mf_to_sf:
                if isinstance(self.sampling_strategy, str):
                    sampling_number = self._parse_sampling_strategy(self.sampling_strategy, len(cur_total_datas))
                    ...
                    cur_total_datas = [cur_total_datas[int(i)] for i in self.sampling_strategy if len(cur_total_datas) > int(i)]
                else:
                    raise NotImplementedError(f"sampling_strategy {self.sampling_strategy} is not implemented")
                ...
                if isinstance(self.mf_sampling_strategy, str):
                    sampling_number = self._parse_sampling_strategy(self.mf_sampling_strategy, len(cur_total_datas))
                    ...
                    raise NotImplementedError(f"mf_sampling_strategy {self.mf_sampling_strategy} is not implemented")
            else:
