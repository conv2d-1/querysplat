...
class DynamicStereoDataset(MatchingDataset):
    ...
    def load_depth(self, depth_path, image, const, dtype):
        ...
        if data_type in img_file_type:
            ...
            data = np.frombuffer(np.array(data, dtype=np.uint16), dtype=np.float16).astype(np.float32).reshape((data.size[1], data.size[0]))
