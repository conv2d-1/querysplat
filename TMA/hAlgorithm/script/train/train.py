...
def main():
    ...
    dist_kwargs = DistributedDataParallelKwargs(
        find_unused_parameters=True,
        # A full scratch run keeps the 1.65B backbone trainable. Reusing DDP
        # buckets as gradient storage avoids a second full-size gradient copy.
        gradient_as_bucket_view=os.environ.get(
            "DDP_GRADIENT_AS_BUCKET_VIEW", "0"
        ).lower()
        in {"1", "true", "yes", "on"},
    )
    init_kwargs = InitProcessGroupKwargs(
        timeout=datetime.timedelta(seconds=int(os.environ.get("NCCL_TIMEOUT", 3600)))
    )
    ...
    if accelerator.is_main_process:
        ...
        config_logging_v2(console_level=logging.WARNING)
    ...
    if accelerator.is_main_process:
        os.system(f"git diff -w > {os.path.join(logging_dir, 'code.diff')} 2>/dev/null")
    ...
    if accelerator.is_main_process:
        ...
        if not args.test and not args.only_vis:
    ...
    if "mem" in cfg["trainer"] and not args.test and not args.only_vis:
        try:
    ...
    cfg["trainer"]["num_processes"] = accelerator.num_processes
    ...
    if not args.test and not args.only_vis:
        train_dataset, train_dataloader, train_nums = prepare_data_loaders(
            "train",
            cfg,
            configs_dir,
    ...
    if not args.test:
        if "vis" in cfg["data"]:
            ...
            vis_datasets, vis_dataloaders, vis_nums = val_datasets, val_dataloaders, val_nums
    else:
        vis_datasets = vis_dataloaders = None
