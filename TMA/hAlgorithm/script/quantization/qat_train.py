...
from hAlgorithm.script.quantization.quant_utils.pytorch_helper import get_layers
    ...
    if trainer.get(select_key, None) is not None:
        if isinstance(trainer[select_key], str):
            if trainer[select_key] != "":
                select_dataset = [
                    item.strip() for item in trainer[select_key].split(",")
                ]
...
def main():
    ...
    if (
        select_dataset is not None
        and cfg["trainer"].get("select_max_depth", None) is not None
    ):
        if isinstance(cfg["trainer"]["select_max_depth"], (int, float)):
            ...
            if cfg["trainer"]["select_max_depth"] != "":
                select_max_depth = [
                    item.strip()
                    for item in cfg["trainer"]["select_max_depth"].split(",")
                ]
    ...
    if not args.test:
        ...
        if max_iter is not None:
            cfg["trainer"]["max_epoch"] = math.ceil(
                max_iter * total_batch_size / num_train_samples
            )
        else:
            cfg["trainer"]["max_iter"] = math.ceil(
                max_epoch * num_train_samples / total_batch_size
            )
        ...
        if accelerator.is_main_process:
            ...
            logging.info(
                f"  Num train examples = {train_nums}, sum = {num_train_samples}"
            )
            logging.info(
        ...
        if accelerator.is_main_process:
            ...
            logging.info(
                f"  Num vis examples = {sum([len(dataset) for dataset in vis_datasets])}"
            )
            logging.info(f"  logging_dir = {args.output_dir}")
            logging.info("")
    ...
    # Initialize model
    ckpt_path = cfg["trainer"]["load_from"]
    cfg["trainer"]["load_from"] = None
    cfg["trainer"]["resume"] = None
