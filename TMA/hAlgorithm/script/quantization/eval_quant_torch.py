...
def parse_args():
    ...
    if args.output_dir in ["None", "none", "null"] and args.ckpt is not None:
        ckpt_abs_path = os.path.abspath(args.ckpt)  # 处理相对路径
        base_dir = os.path.dirname(ckpt_abs_path)
        args.output_dir = base_dir
    args.output_dir = os.path.join(args.output_dir, f"torch_eval_{now}")
...
def main():
    ...
    model_wrapper = TorchModel(model.model, half=True)
