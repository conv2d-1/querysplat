...
def main():
    ...
    for dataloader in val_dataloaders[:1]:
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            pred_local_depth = outputs["pred_local_depth"].cpu()
            pred_local_conf = outputs["pred_local_conf"].cpu()
            pred_local_normal = outputs["pred_local_normal"].cpu()
            pred_local_invalid_mask = outputs["pred_local_invalid_mask"].cpu()
            ...
            output.rgb = (image[0].permute(1, 2, 0).cpu().numpy() + 1) * 0.5
