...
def build_engine(
    ...
    """
    logging.info(f"Start Building Engine ...")
    # 日志设置（VERBOSE 可帮助调试）
    ...
    logging.info(f"✅ Engine saved to: {engine_file_path}")
    logging.info(f"   FP16: {'Enabled' if fp16_mode else 'Disabled'}")
...
def infer_with_torch(engine, input_tensors_dict):
    ...
    """
    image_shape = input_tensors_dict["image"].shape  # [B, 1, 3, H, W]
    B, _, _, H, W = image_shape
...
def main():
    ...
    for dataloader in val_dataloaders[:1]:
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            # 准备输入数据
            image = batch["image"].clone().unsqueeze(1) # b, 1, c, h, w
            ...
            h, w = image.shape[-2:]
            if pos is None or pos_nodiff is None:
