...
def main():
    ...
    if args.onnx is None:
        ...
        for block in model.model.fuse_encoder.pretrained.blocks:
        ...
        _prepare_rope = model.model.fuse_encoder.pretrained._prepare_rope
        _prepare_pos_embed = model.model.depth_head._prepare_pos_embed
        model.model.depth_head.pos_embed = True
        
        pos_embed_size = [ #channel, psize
            (96, 60),
            (192, 60),
            (384, 60),
            (768, 60),
        ...
        for i, batch in enumerate(val_dataloaders[0]):
            ...
            image = torch.nn.functional.interpolate(image[0], size=(840,840))[None]
            prompt_depth = batch[model.prompt_depth_name].cuda()
            prompt_depth = torch.nn.functional.interpolate(prompt_depth[0], size=(840,840))
            prompt_scale = batch[model.scale_name][0][..., None, None, None].cuda()
            h, w = image.shape[-2:]
            patch_h, patch_w = (
                h // model.model.fuse_encoder.pretrained.patch_size,
                w // model.model.fuse_encoder.pretrained.patch_size,
            )
            setattr(model, "patch_h", patch_h)
            setattr(model, "patch_w", patch_w)
            if args.fp16:
                image = image.half()
                prompt_depth = prompt_depth.half()
                prompt_scale = prompt_scale.half()
            pos, pos_nodiff = _prepare_rope(1, 1, h, w, image.device)
            # pos = pos.to(dtype=torch.int32)
            # pos_nodiff = pos_nodiff.to(dtype=torch.int32)
            pos_embed = {}
            for i, (channel, psize) in enumerate(pos_embed_size):
                pos_embed[f"pos_stage{i}"] = _prepare_pos_embed(
                    psize, psize, channel, image, w, h
                )
            pos_embed_shape = {
                k: v.shape for k, v in pos_embed.items()
            }
            pos_embed_val = tuple(pos_embed.values())
            ...
            output_names = [
                ...
                "pred_local_conf",
                "pred_local_normal",
                "pred_local_invalid_mask",
            ]
            # dynamic_axes = {
            #     "image": {0: "batch", 2: "height", 3: "width"},
            #     "prompt_depth": {0: "batch", 2: "pheight", 3: "pwidth"},  # 假设与 image 同尺寸
            #     "prompt_scale": {0: "batch"},
                
            #     "pred_local_depth": {0: "batch", 2: "height", 3: "width"},
            #     "pred_local_conf": {0: "batch", 2: "height", 3: "width"},
            #     "pred_local_normal": {0: "batch", 2: "height", 3: "width"},
            #     "pred_local_invalid_mask": {0: "batch", 2: "height", 3: "width"},
            # }
            dynamic_axes = {
            ...
            torch.onnx.export(
                ...
                opset_version=15,  # ONNX 操作集版本，通常选择 11 或更高
                export_params=True,  # 导出模型参数
                do_constant_folding=True,  # 启用常量折叠优化
            )
            overwrite_input_shapes = {
                "image": [1, 1, 3, 840, 840],
                "prompt_depth": [1, 3, 840, 840],
                "prompt_scale": [1, 1, 1, 1],
                "pos": [1, 1, 3601, 2], # B,S,P,2
                "pos_nodiff": [1, 1, 3601, 2], # B,S,P,2
            } 
            overwrite_input_shapes.update(pos_embed_shape)
