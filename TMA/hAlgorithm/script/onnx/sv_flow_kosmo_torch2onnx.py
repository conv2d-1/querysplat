...
def main():
    ...
    if args.onnx is None:
        ...
        from hAlgorithm.modules.models2.temporal_encoder.attention import Attention, CrossAttention
    
        for block in model.model.fuse_encoder.dec_blocks:
            block.attn.__class__ = Attention
            block.cross_attn.__class__ = CrossAttention
        for block in model.model.fuse_encoder.dec_blocks_state:
            block.attn.__class__ = Attention
            block.cross_attn.__class__ = CrossAttention
        ...
        for i, batch in enumerate(val_dataloaders[0]):
            ...
            prompt_depth = batch[model.prompt_depth_name][:,0].cuda()
            prompt_depth = torch.nn.functional.interpolate(prompt_depth, size=(476,476))
            prompt_scale = batch[model.scale_name][..., None, None, None][:, 0].cuda()
            h, w = image.shape[-2:]
            patch_h, patch_w = (
            ...
            overwrite_input_shapes = {
                "image": [1, 3, 840, 840],
                "prompt_depth": [1, 3, 476, 476],
                "prompt_scale": [1, 1, 1, 1],
                "flow_state": [1, 768, 768],
            }
