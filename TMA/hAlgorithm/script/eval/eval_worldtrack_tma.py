#!/usr/bin/env python3
"""Evaluate TMA on WorldTrack / TapVid3D — **Dynamic Points** (3D tracking, warp3d).

Primary metrics in summary.json: ``avg_pts_global`` (APD), ``tau_global`` (τ@0.1m),
``epe_global`` (aligned 3D EPE vs JSON ``trajs_3d``). Scene flow is a separate entry point:
``eval_worldtrack_tma_sceneflow.py``. See ``docs/WORLDTRACK_SCENEFLOW_AND_DYNAMIC_POINTS_EVAL.md``.
"""
...
from hAlgorithm.eval.worldtrack_vis import (  # noqa: E402
    export_frame0_gt_colored_ply,
    export_frame0_pred_dense_colored_ply,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="WorldTrack eval for TMA with Open-d4rt metrics.")
    ...
    p.add_argument(
        "--dynamic-query-threshold",
        type=float,
        default=None,
        help=(
            "If set (e.g. 0.01), keep only frame-0 queries whose GT motion from frame 0 "
            "exceeds this threshold (m). For SynthVerse use world-space trajs_3d motion."
        ),
    )
    p.add_argument("--output-dir", default="tmp/eval_worldtrack_tma")
    ...
    p.add_argument(
        "--seq-name",
        default="",
        help="Run only this sequence (overrides --limit-seqs ordering).",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "'warp3d_delta' = src_points + warp3d_delta (requires both). "
            "No GT / pointmap / (warp3d - warp3d_delta) derivation. Default: warp3d."
        ),
    )
    p.add_argument(
        "--use-warp3d-delta",
    ...
    p.add_argument(
        "--visualize",
        action="store_true",
        help="Save tracks.npz and render Open-d4rt-style overlay videos per sequence.",
    )
    p.add_argument(
        "--vis-frame0-pointmap-ply",
        action="store_true",
        help=(
            "Export frame-0 PLY: sparse GT (green) + sparse pred (red) + dense pred (image RGB). "
    ...
    p.add_argument(
        "--resume",
        action="store_true",
        help="Skip sequences that already have per-sequence JSON in output-dir/subset.",
    )
    args = p.parse_args()
    args.pred_3d_source = resolve_pred_3d_source(
        pred_3d_source=args.pred_3d_source,
        use_warp3d_delta=bool(args.use_warp3d_delta),
    )
    return args
...
def _load_model(cfg: dict, ckpt_path: Path, device: torch.device):
    ...
    model.to(device)
    # Pipeline.device is normally set in accelerator_prepare during train/test.
...
def _worldtrack_skip_redundant_warp3d_gather() -> Iterator[None]:
    """Skip full-grid ``gather`` when motion head already outputs sparse ``[B, Q, 3]``."""
...
def _worldtrack_infer_compat(model) -> Iterator[None]:
    """Sparse-query WorldTrack eval without editing ``mvfr_query_v4``.

    - Drop dense ``depth`` / ``global_points`` (infer reshapes them to H×W).
    - ``(0, 0)`` identity pair is added inside ``infer`` (``_ensure_ref_identity_pairs``).
    """
...
def _infer_batch(model, batch: dict, mixed_precision: str):
    ...
    # Let ``model.infer`` / ``get_inputs`` move tensors (some fields need CPU numpy).
    with (
...
def _extract_frame0_model_rgb_uint8(batch: Dict[str, Any]) -> np.ndarray:
    """Frame-0 RGB in **model input resolution** (matches ``query_uv`` / dense grid)."""
...
def _infer_batch_dense_frame0(model, batch: dict, mixed_precision: str):
    """Run model.infer without overriding query bank (dense full-UV)."""
    # IMPORTANT:
    # WorldTrack dataset batches contain sparse tracking queries (`trajs_2d/trajs_3d` etc.).
    # If we pass them through, `infer()` will feed `query_points` and the model will stay sparse.
    # For dense per-pixel warp3d, drop tracking query tensors so the query bank falls back to full UV grid.
    batch = dict(batch)
    for k in (
        "trajs_2d",
        "trajs_3d",
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        seq_gt = load_sequence_from_json(
            ...
            coord_convention=coord_convention,
            dynamic_query_threshold=dynamic_query_threshold,
        )  # loader uses real frame-0 (H,W) for query gate even when load_rgb=False
        num_frames_eff = int(seq_gt.num_frames)
        query_idx = align_batch_queries_with_sequence(
            batch, seq_gt, sequence_name=seq_name
        )
        ...
        if (
            ...
            and outputs[-1].get("type") is not None
        ):
            batch = outputs.pop()
        ...
        gt_cam_tma = extract_tma_gt_cam_from_outputs(
            outputs, num_frames=num_frames_eff, q_count=q_count
        )
        ...
        metrics = metrics_for_sequence(
            gt_tracks_world=gt_q,
            pred_tracks_ref0=pred_q,
            compute_dyn=True,
            eval_dynamic_only=dynamic_query_threshold is not None,
            dynamic_threshold=float(dynamic_query_threshold)
        ...
        metrics["scale_global"] = align["scale_global"]
        metrics["pred_3d_source"] = pred_3d_source
        metrics["pred_field"] = pred_field_label(pred_3d_source)
        metrics["video_name"] = seq_name
        ...
        logging.info(
            "%s | %s | T=%d Q=%d | APD=%.4f tau=%.4f EPE=%.4f | scale=%.4f | "
            "ref0-vs-GT=%.3f cam-vs-JSON=%.3f ref0-vs-tmaGT=%.3f field=%s",
            subset_name,
            seq_name,
            num_frames_eff,
            metrics["num_queries"],
            metrics[APD_GLOBAL_KEY],
            metrics.get(TAU_GLOBAL_KEY, float("nan")),
            metrics[EPE_GLOBAL_KEY],
            metrics.get("scale_global", float("nan")),
            metrics.get("sanity_epe_ref0_vs_gt", float("nan")),
            metrics.get("sanity_epe_cam_vs_json", float("nan")),
            metrics.get("sanity_epe_ref0_vs_tma_tgt", float("nan")),
            metrics.get("pred_field", "warp3d"),
        )

        if save_per_sequence:
            (subset_out / f"{seq_name}.json").write_text(
        ...
        if visualize:
            ...
            vis_paths = visualize_tracks_package(
                vis_dir,
        ...
        # Frame-0 PLY: combined overview + separate GT / dense-pred image-colored clouds.
        if vis_frame0_pointmap_ply:
            ...
            seq_vis_rgb = load_sequence_from_json(
                json_path,
                seq_name,
                data_root=data_root,
                num_frames=num_frames_eff,
            ...
            frame0_dir = (subset_out / seq_name / "vis") if (visualize or save_per_sequence) else subset_out
            ...
            gt_meta = export_frame0_gt_colored_ply(
                frame0_dir / "frame0_gt_colored.ply",
                gt_tracks_ref0_frame0=gt_q[0],
                video_rgb_frame0=seq_vis_rgb.video_rgb[0],
                gt_tracks_uv_pixel_frame0=seq_vis_rgb.tracks_uv[0, query_idx],
                visibility_frame0=seq_vis_rgb.visibility[0, query_idx],
            )
            pred_meta = export_frame0_pred_dense_colored_ply(
                frame0_dir / "frame0_pred_dense_colored.ply",
                gt_tracks_ref0_frame0=gt_q[0],
                pred_tracks_ref0_frame0=pred_q[0],
                outputs_dense=outputs_dense,
                video_rgb_frame0=model_rgb0,
            ...
            (frame0_dir / "frame0_gt_colored.meta.json").write_text(
                json.dumps(gt_meta, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            (frame0_dir / "frame0_pred_dense_colored.meta.json").write_text(
                json.dumps(pred_meta, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
...
def main() -> int:
    ...
    logging.info(
        "Prediction source: %s (%s)",
        args.pred_3d_source,
        pred_field_label(args.pred_3d_source),
    )
    ...
    json_paths = [
        prefer_worldtrack_json_path(Path(p), data_root=data_root_for_json)
    ...
    all_summary: Dict[str, Any] = {
        "inputs": {
            ...
            "dynamic_query_threshold": args.dynamic_query_threshold,
            "json_paths": [str(p) for p in json_paths],
        },
        "subsets": {},
    }

    lines = ["WorldTrack TMA Summary"]
    for json_path, subset_name in zip(json_paths, subset_names):
        ...
        if args.resume:
            ...
            if done:
                ...
                logging.info(
                    "Resume: skipping %d/%d sequences already in %s",
                    before - len(seq_filter),
                    before,
                    subset_out,
                )
