#!/usr/bin/env bash
# TMA bigdata_v1 latest checkpoint — Dynamic Points on 5 subsets (adt/pstudio/po/ds/synthverse).
# Uses free GPUs 3,4,5 with a small queue for ds + synthverse.
set -euo pipefail
...
TAG="${TAG:-20260616}"
LOG_DIR="${LOG_DIR:-tmp/eval_logs_tma_latest_dp_${TAG}}"
mkdir -p "$LOG_DIR"
...
OUTPUT_BASE="${OUTPUT_BASE:-tmp/eval_worldtrack_tma_latest_dp_${TAG}}"
...
subset_map = {
    "adt": ("adt_mini", "adt_mini"),
    "pstudio": ("pstudio_mini", "pstudio_mini"),
    "po": ("po_mini", "po_mini"),
    "ds": ("ds_mini", "ds_mini"),
    "synthverse": ("synthverse_subset50_dynamic", "synthverse_subset50_dynamic"),
}
merged = {
    "inputs": {
        "config": config,
...
lines = ["WorldTrack TMA latest DP (5 subsets)"]
for subset, (name, _) in subset_map.items():
...
echo "Launching TMA latest DP (5 subsets) -> ${OUTPUT_BASE}" | tee "${LOG_DIR}/launcher.log"

# First wave: 3 TapVid3D subsets on free GPUs
for i in "${!SUBSETS[@]}"; do
  run_subset "${GPUS[$i]}" "${SUBSETS[$i]}" &
done
wait

# Second wave: ds + synthverse dynamic-only (via run_eval_worldtrack_tma.sh SUBSET=synthverse)
run_subset 5 ds &
wait
run_subset 5 synthverse &
wait
