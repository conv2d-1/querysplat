...
def process_single_pair(
    ...
    if fisheye_meta is not None:
        ...
        if fisheye_meta.get("is_fisheye_A", True):
            kptsA = kptsA + offset
        if fisheye_meta.get("is_fisheye_B", True):
            kptsB = kptsB + offset
...
def _save_visualization(
    ...
    max_h = max(x1.shape[1], x2.shape[1])
    max_w = max(x1.shape[2], x2.shape[2])
    if x1.shape[1] < max_h or x1.shape[2] < max_w:
        x1 = F.pad(x1, (0, max_w - x1.shape[2], 0, max_h - x1.shape[1]))
    if x2.shape[1] < max_h or x2.shape[2] < max_w:
        x2 = F.pad(x2, (0, max_w - x2.shape[2], 0, max_h - x2.shape[1]))
    ...
    max_h = max(h_a, h_b)
    if h_a < max_h:
        pad = np.zeros((max_h - h_a, w_a, 3), dtype=img_A_np.dtype)
        img_A_np = np.concatenate([img_A_np, pad], axis=0)
    if h_b < max_h:
        pad = np.zeros((max_h - h_b, w_b, 3), dtype=img_B_np.dtype)
        img_B_np = np.concatenate([img_B_np, pad], axis=0)
...
def worker_fn(args, cfg, all_pairs):
    ...
    def load_single_image(img_name):
        ...
        is_fisheye = (W_orig == 4000 and H_orig == 3000)
        if fisheye_crop is not None and not is_fisheye:
            print(f"[skip fisheye] {img_name}: resolution {W_orig}x{H_orig} != 4000x3000, skipping crop & mask")

        if fisheye_crop is not None and is_fisheye:
        ...
        if fisheye_mask_bool is not None and is_fisheye:
        ...
        return img_name, img_vis, img_resized, mask_np, W, H, orig_size, is_fisheye
    ...
    def assemble_pair(idx):
        ...
        img_A_vis, img_A_resized, mask_A_np, W_A, H_A, orig_A, is_fisheye_A = image_dict[img_A_name]
        img_B_vis, img_B_resized, mask_B_np, W_B, H_B, orig_B, is_fisheye_B = image_dict[img_B_name]
        ...
        if fisheye_crop is not None:
            fisheye_meta = {
                "orig_A": orig_A,
                "orig_B": orig_B,
                "is_fisheye_A": is_fisheye_A,
                "is_fisheye_B": is_fisheye_B,
