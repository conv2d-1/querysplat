...
This script is designed for inference with the MVFR Motion network that includes
a scene flow head for 3D motion prediction. After inference, motion visualizations
are automatically saved.

MVFRMotion requires external camera parameters (intrinsics and extrinsics) - 
it is NOT a pure image-based model like VDPM.

Usage:
    python hAlgorithm/script/infer/motion_head/mvfr_motion_kosmo_infer.py \
        --config <config_path> \
        --data <json_path> \
        --output_dir ./results/mvfr_motion_output

Example:
    python hAlgorithm/script/infer/motion_head/mvfr_motion_kosmo_infer.py \
        --config hAlgorithm/configs/motion_head/motion_head_da3l_b1_8gpus.py \
        --data /path/to/data.json \
        --output_dir ./results/mvfr_motion_demo
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="MVFR Motion Network Inference with Scene Flow Visualization")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./results/mvfr_motion_demo",
...
def load_model(args, cfg):
    ...
    logging.info("Loading MVFR Motion model...")
...
def main():
    ...
    logging.info(f"MVFRMotion requires external camera parameters (intrinsics and extrinsics)")
