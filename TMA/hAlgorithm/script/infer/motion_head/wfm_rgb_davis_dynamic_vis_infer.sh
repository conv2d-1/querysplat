#!/usr/bin/env bash
# DAVIS: (0,0) dense static warp3d + (0,t) GT-mask dynamic warp3d → Rerun .rrd
#
# Default sequence: bear (480p, 50 frames, debug_trajectory sampling).
#
# Usage:
#   bash hAlgorithm/script/infer/motion_head/wfm_rgb_davis_dynamic_vis_infer.sh
#
#   sequence=blackswan static_downsample=2 bash ...
#   sequences="bear blackswan" bash ...
...
output_dir="${output_dir:-${REPO_ROOT}/results/wfm_rgb_davis_dynamic_vis}"
exp_name="${exp_name:-davis_dynamic}"
...
cmd=(
    "${ACC_PYTHON}" hAlgorithm/script/infer/motion_head/wfm_rgb_davis_dynamic_vis_infer.py
