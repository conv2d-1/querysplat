...
This script is designed for inference with the Movies network that includes
a motion head for scene flow prediction. After inference, motion visualizations
are automatically saved.

Usage:
    python hAlgorithm/script/infer/movies_demo.py \
        --config <config_path> \
        --load_from latest \
        --data <json_path> \
        --output_dir ./results/movies_demo

Example:
    python hAlgorithm/script/infer/movies_demo.py \
        --config results/movies_exp/movies_config.py \
        --load_from latest \
        --data /path/to/data.json \
        --output_dir ./results/movies_output
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Movies Network Inference with Motion Visualization")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of frames to process per batch (default: 2).",
    )
    parser.add_argument(
        "--view_id",
        type=int,
...
def load_model(args, cfg):
    ...
    # Set device and dtype for inference (normally set in accelerator_prepare during training)
...
def prepare_batch_from_frame(frame_data, data_root, device, process_res=504, view_id=None, depth_key="depth"):
    """
    ...
    Expected output tensor shapes:
        ...
        - sparse_pointmap_max_range: [B, N] (scale for each view)
    """
    import cv2
    ...
    for frame in frame_data:
        ...
        # Load intrinsics from cam_in or K field
        ...
        if cam_in is not None:
            ...
            # Also handle fisheye format [fx, fy, cx, cy, k1, k2, ...]
    ...
    # Compute scale from depth (matching with_global_scale=True in training)
    # scale = mean(||pointmap_reff points||) = average distance from origin after alignment
    ...
    if has_depth and has_intrinsics and has_extrinsics:
        ...
        for i, depth in enumerate(depth_list):
            ...
            if isinstance(cam_in, list) and len(cam_in) >= 4:
                # K is [fx, fy, cx, cy] format (or fisheye with extra params)
                fx, fy, cx, cy = cam_in[0], cam_in[1], cam_in[2], cam_in[3]
                ...
                # K is 3x3 matrix format [[fx,0,cx],[0,fy,cy],[0,0,1]]
            ...
            # P_reff = extrinsics_reff^{-1} @ P_cam (camera to world in aligned frame)
            # But extrinsics_reff = w2c_aligned, so c2w_aligned = inv(w2c_aligned)
        ...
        # Fallback: compute from camera positions (with_pose_max_scale equivalent)
        if w2c_aligned is not None:
            c2w_aligned = torch.linalg.inv(w2c_aligned)  # [N, 4, 4]
            camera_positions = c2w_aligned[:, :3, 3]  # [N, 3] - actual camera positions
    ...
    # Model will expand to [B, N, 1, 1, 1] via [..., None, None, None]
    scale_tensor = torch.full((1, num_images), depth_scale_value, dtype=torch.float32, device=device)
    
    # Build meta_data
    # For single view_id: frames=N, views=1
    # For all views: need to compute properly
...
def main():
    ...
    if args.data is not None:
        ...
        for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
            ...
            # Prepare batch (multiple frames, already filtered by view_id)
