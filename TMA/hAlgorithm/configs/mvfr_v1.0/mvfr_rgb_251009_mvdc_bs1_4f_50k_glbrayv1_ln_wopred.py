...
max_iter = 50000
select_dataset="hablendeder_objs,infinigen,infinigen_iphone,infinigen_objs,scpp"
...
model = dict(
    ...
    model=dict(
        ...
        ray_encoder=dict(
            type="hAlgorithm.modules.models2.encoder.ray_encoder.RayEncoder",
            model=dict(
        ...
        fuse_encoder=dict(
            type="hAlgorithm.modules.models2.mv_encoder.vggt_mv_encoder.RayExtraMVEncoder",
            patch_size=patch_size,
            embed_dim=1024,
            depth=24,
            num_heads=16,
        ...
        depth_head=dict(
            ...
            in_channels=[1024*2, 1024*2, 1024*2, 1024*2],
        ...
        # gs_far=100, 
        # # gaussian_head=dict(
        # #     type='hAlgorithm.modules.models.head.gaussian_head.GaussianHead',
        # #     feat_dim=128,
        # #     downsample=False,
        # #     cfg=dict(
        # #         sh_degree=3,
        ...
        # # ),
        # gaussian_head=dict(
        #     type='hAlgorithm.modules.models.head.gaussian_transformer_head.GaussianHead',
        #     in_channels=1024*2,
        #     patch_size=patch_size,
        #     rgb_features_channels=1024,
        #     rgb_features_index=0,
        #     head_mlp_ratio=None,
        #     depth=1,
        #     cfg=dict(
        ...
        #     type='hAlgorithm.modules.models.gaussiansv2.decoder_pgsr.decoder_splatting_cuda.DecoderSplattingCUDA'
        # ),
    ...
    local_depth_grad_loss=dict(
        ...
        loss_weight=dict(
            default=0.0,
            hypersim=8.0, # l1 + gradl1 + gradop + norm
            blendedmvs=0.0, # l1 + gradl1
            kubric4d=8.0, # l1 + gradl1 + gradop + norm
            nrgbd=0.0, # l1 + gradl1 + gradop
            s7=0.0, # l1 + gradl1 + norm
            scannet=0.0, # l1 + gradl1 + gradop + norm
            
            hablendeder=8.0, # l1 + gradl1 + gradop + norm
            hablendeder_objs=8.0, # l1 + gradl1 + gradop + norm
            habitat=0.0, # l1 + gradl1 + gradop + norm
            pointodyssey=0.0, # l1 + gradl1 + gradop + norm

            scpp=0.0, # l1 + gradl1 + gradop + norm
            megasynth=8.0, # l1 + gradl1 + gradop + norm
            mvssynth=8.0, # l1 + gradl1 + gradop + norm
            infinigen=8.0, # l1 + gradl1 + gradop + norm
            hm3d=0.0,  # l1 + gradop + norm

            ase=0.0, # l1 + gradl1 + gradop + norm

            infinigen_iphone=8.0,
            infinigen_objs=8.0,
        ),
    ),
    ...
    # local_ray_directions_loss=dict(
    #     type="hAlgorithm.modules.losses2.ray_loss.FactoredLLoss",
    #     loss_weight=1.0,
    #     loss_type="l1",
    #     reduction="mean",
    # ),
    ...
    global_points_normal_loss=dict(
    ...
    task_weight=dict(lcl=1.0, glb=0.1, l2g=1.0, cm=3.0, tk=0.03, rc=1.0),
...
trainer = dict(
    ...
    optimizer={
        "type":"AdamW",
        "model.ray_in_world_encoder": dict(lr=1e-5, betas=(0.9, 0.999), weight_decay=1e-3, eps=1e-10),
    ...
    # load_from="/mnt/netdata/Team/AI/personal/ts/weights/vggt/vggt_to_hdepth.pth",
