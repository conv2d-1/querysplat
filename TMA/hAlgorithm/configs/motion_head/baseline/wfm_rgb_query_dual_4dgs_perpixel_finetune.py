"""Dual-query per-pixel 4DGS finetune (self-contained config).

Two decoupled query groups:
  * **Sparse traj query** (from GT ``trajs_2d``) → pair decoder → motion losses
    (warp3d / warp2d / warp3d_delta).
  * **Dense H×W query** (built inside :class:`MVQueryDual4DGS`) → Gaussian head
    → gsplat render loss (``max_render_points=0``, per-pixel).

Init from motion-pretrained checkpoint (``trainer.load_from``):
  ``results/baseline_v4/.../finetune_psdutio_20260609-115200/checkpoint/latest/ckpt.pth``

Memory notes (A800 80GB): motion ``num_samples`` reduced; ``num_render_frames=1``.
Per-pixel Gaussian render is unchanged (``max_render_points=0``).
"""
...
select_dataset = (
    "pointodyssey,kubric4d,dynamicreplica,cotracker3kubric,"
    "hasim,hasim_benchmark_running,"
    "hasim_character_medium,hasim_character_follow,hasim_character,hasim_character_easy,pstudio,"
    "hypersim,dl3dv,"
)

select_val_dataset = select_vis_dataset = (
    "pointodyssey,kubric4d,hasim,hasim_benchmark_running"
)
...
data = dict(
    train="hAlgorithm/configs/unified_query/dataset_configs/4d/with_hasim_0526_wild3_stereo4d.yaml",
...
model = dict(
    ...
    sparse_dataset_names=sparse_dataset_names,
...
trainer = dict(
    ...
    optimizer={
        ...
        "model.sparse_gaussian_head": dict(lr=2e-4, betas=(0.9, 0.999), weight_decay=1e-3, eps=1e-10),
    ...
    load_from=(
        "/mnt/home/tcchen/workspace/TMA-origin-dev/results/baseline_v4/"
        "wfm_rgb_query_260526_bs1_4f_50k_hyp0.1_dl3dv0.1_waymo_kitti2_stereo4d_bigdata_v1_finetune_psdutio_20260609-115200/"
...
output_dir = (
    "/mnt/home/tcchen/workspace/TMA-origin-dev/results/baseline_v4/"
    "wfm_rgb_query_dual_4dgs_perpixel_finetune"
)
