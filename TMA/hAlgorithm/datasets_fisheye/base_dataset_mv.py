...
from hAlgorithm.datasets_fisheye.base_dataset import BaseDatasetFisheye as BaseDataset
from hAlgorithm.datasets_fisheye.utils.fisheye_warp import get_gt_warp_fisheye_equidistant

class BaseDatasetFisheyeMV(BaseDataset):
    def __init__(
        self,
        mf_sampling_strategy: str = "all",
        ...
        with_max_mean_scale: bool = False,
        with_global_max_scale: bool = False,
        with_pose_max_scale: bool = False,
        local_scale_mode: str = "max",
        ...
        delete_portrait: bool = False,
        each_frame_is_clip: bool = False,
        drop_last=True,
        # trajectory_noise_prob: float = None,
        prompt_extrinsics_type: str = "slam",
        extrinsics_type: str = "slam",
        pseudo_pair_prob: float = 0.0,
        pseudo_pair_transforms: dict = None,
        pseudo_pair_match_scales: list = None,
        **kwargs,
    ):
        """
        ...
        self.mf_scene_delete = mf_scene_delete
        ...
        self.mf_sampling_strategy = mf_sampling_strategy
        self.mf_scene_sampling_strategy = mf_scene_sampling_strategy

        self.delete_portrait = delete_portrait
        ...
        self.with_max_mean_scale = with_max_mean_scale
        self.with_global_max_scale = with_global_max_scale
        self.with_pose_max_scale = with_pose_max_scale
        self.local_scale_mode = local_scale_mode
        ...
        self.sample_query_uvt = sample_query_uvt
        ...
        self.each_frame_is_clip = each_frame_is_clip
        self.drop_last = drop_last

        self.kosmo_scene_infos = dict()
        self.kosmo_frames = dict()

        # self.trajectory_noise_prob = trajectory_noise_prob

        self.prompt_extrinsics_type = prompt_extrinsics_type
        self.extrinsics_type = extrinsics_type

        self.pseudo_pair_prob = pseudo_pair_prob
        self.pseudo_pair_match_scales = pseudo_pair_match_scales or [1]
        self.pseudo_pair_transforms = pseudo_pair_transforms or dict(
        ...
        _pp_seed = self.pseudo_pair_transforms.get("seed", None)
        if _pp_seed is not None:
            random.seed(_pp_seed)
    ...
    def data_format_to_base(self, version, data, info_index=None):
        if version == "1.0":
            base_data = dict()
            for scene, info in data.items():
                base_data[scene] = []

                frames = info.pop("frames", None)
                cam_params = info.pop("cam_params", None)

                # NOTE: match multi json
                kosmo_scene_name = scene if info_index is None else f"{info_index}_{scene}"
                # NOTE: without frames and cam_params
                self.kosmo_scene_infos[kosmo_scene_name] = {k: v for k, v in info.items() if v is not None}
                # NOTE: delete cam_param gamma
                if cam_params is not None:
                    self.kosmo_scene_infos[kosmo_scene_name]["cam_params"] = dict()
                    for cam, cam_param in cam_params.items():
                        if "gamma" in cam_param and cam_param["gamma"] is None:
                            cam_param.pop("gamma")
                        self.kosmo_scene_infos[kosmo_scene_name]["cam_params"][cam] = cam_param
                ...
                for frame in frames:
                    ...
                    frame_id, view_id = frame["frame_id"], frame["view_id"]
                    if self.is_fisheye_camera():
                        intrinsics_num = 8
                    else:
                        intrinsics_num = 4
                    if cam_params is not None and f"cam{view_id}" in cam_params and len(cam_params[f"cam{view_id}"]["K"]) != intrinsics_num:
                        continue
                    
                    # NOTE: save outputs with kosmo json
                    if frame_id not in self.kosmo_frames:
                        self.kosmo_frames[frame_id] = dict()
                    self.kosmo_frames[frame_id][view_id] = {k: v for k, v in frame.items() if v is not None}

                    frame.pop("lidar2world_pose", None)
                    cam2world_pose = frame.pop("cam2world_pose", None)
                    cam2world_pose_colmap = frame.pop("cam2world_pose_colmap", None)

                    if self.prompt_extrinsics_name is not None:
                        if cam2world_pose is not None and self.prompt_extrinsics_type == "slam":
                            frame[self.prompt_extrinsics_name] = np.linalg.inv(np.array(cam2world_pose)).tolist()
                        elif cam2world_pose_colmap is not None and self.prompt_extrinsics_type == "colmap":
                            frame[self.prompt_extrinsics_name] = np.linalg.inv(np.array(cam2world_pose_colmap)).tolist()
                        else:
                            continue

                    if self.extrinsics_name is not None:
                        if cam2world_pose is not None and self.extrinsics_type == "slam":
                            frame[self.extrinsics_name] = np.linalg.inv(np.array(cam2world_pose)).tolist()
                        elif cam2world_pose_colmap is not None and self.extrinsics_type == "colmap":
                            frame[self.extrinsics_name] = np.linalg.inv(np.array(cam2world_pose_colmap)).tolist()
                        else:
                            continue

                    # if self.prompt_extrinsics_name is not None and cam2world_pose_colmap is None:
                    #     continue

                    if cam_params is not None and self.intrinsics_name is not None:
                        frame[self.intrinsics_name] = cam_params[f"cam{int(view_id)}"]["K"][:4]
                        if self.is_fisheye_camera() and self.distorts_name is not None:
                            frame[self.distorts_name] = cam_params[f"cam{int(view_id)}"]["K"][4:]

                    if frame_id not in new_info:
                        new_info[frame_id] = dict()
                    new_info[frame_id][view_id] = {key:val for key, val in frame.items() if val is not None}

                for frame_id, views in new_info.items():
                    base_data[scene].append(
                        dict(
                            frame_id=frame_id,
                            views=[view for view_id, view in views.items()],
                        )
                    )
        else:
        ...
        if isinstance(json_path, (list, tuple)):
            ...
            for i, json_path_i in enumerate(json_path):
                ...
                try:
                    ...
                    logging.error(f"{json_path_i} is not a valid json file.")
                    continue
                if isinstance(scene, dict):
                    version = scene.get("version")
                if version is not None and version == "1.0":
                    data = self.data_format_to_base(version=version, data=data, info_index=i)
            ...
            with open(json_path, "r") as json_file:
                ...
                if isinstance(scene, dict):
                    version = scene.get("version")
                if version is not None and version == "1.0":
                    data = self.data_format_to_base(version=version, data=data)
    ...
    def scene_sampling(self, datas):
        ...
        if isinstance(self.mf_scene_sampling_strategy, str):
            ...
            elif self.mf_scene_sampling_strategy.startswith(
                "["
            ) and self.mf_scene_sampling_strategy.endswith("]"):
                data_infos = [
                    data_infos[int(i)]
                    for i in eval(self.mf_scene_sampling_strategy)
    ...
    def load_mf_files(self, datas):
        ...
        for scene in tqdm(data_infos, total=len(data_infos), desc=f"{self.name}, Loading"):
            ...
            if len(frames) > 0:
                ...
                self.mf_data_scene_views.append(0)

            if self.mf_to_mv:
                scene_mf_nums = 0
                for view_id, views in tmp_infos.items():
                    if self.each_frame_is_clip and self.clip_maxlen is not None:
                        split_views = []
                        for i in range(0, len(views) - self.clip_maxlen + 1):
                            split_views.append(views[i : i + self.clip_maxlen])
                            assert len(split_views[-1]) == self.clip_maxlen
                        cur_total_datas.extend(split_views)
                        scene_mf_nums += len(split_views)
                    elif self.clip_maxlen is not None:
                        if self.drop_last:
                            split_views = [
    ...
    def load_hdf5_mf_files(self, datas, frame_view_ids=None):
        ...
        for scene in tqdm(data_infos, total=len(data_infos), desc=f"{self.name}, Loading"):
            ...
            if self.delete_portrait and "rgb_h" in datas[scene] and datas[scene]["rgb_h"] > datas[scene]["rgb_w"]:
                continue
            ...
            for hdf5_id in hdf5_ids:
                ...
                if self.normal_hdf5 is not None and self.normal_hdf5 in datas[scene]:
                    view[self.normal_hdf5] = datas[scene][self.normal_hdf5]
    ...
    def get_mf_data_for_trainval(self, idx):
        ...
        pseudo_pair_mode = False
        if (
            len(mf_data["data"]) == 1
            and self.pseudo_pair_prob > 0
            and random.random() < self.pseudo_pair_prob
        ):
            mf_data["data"].append(copy.deepcopy(mf_data["data"][0]))
            fake_info = copy.deepcopy(mf_data["info"][0])
            fake_info["view_id"] = fake_info.get("view_id", 0) + 1
            mf_data["info"].append(fake_info)
            pseudo_pair_mode = True
        ...
        # if self.trajectory_noise_prob is not None and self.trajectory_noise_prob > 0:
        #     if self.trajectory_noise_prob >= 1.0:
        #         trajectory_noise = True
        #     else:
        #         if self.extrinsics_noise_seed is not None:
        #             random.seed(self.extrinsics_noise_seed)
        #         trajectory_noise = random.random() < self.trajectory_noise_prob
        ...
        for i, _ in enumerate(mf_data["data"]):
            ...
            for _mk in ("distort_k", "sensor_size", "crop_offset"):
                if _mk in data_batch["meta_data"]:
                    meta_keep[_mk] = data_batch["meta_data"][_mk]
            data_batch["meta_data"] = meta_keep
            ...
            if self.debug:
                print(
                    ...
                    f'vertical_flip {transform_info.get("vertical_flip", None)},',
        ...
        pseudo_warp_scales = None
        pseudo_warp_mask_scales = None
        if pseudo_pair_mode and len(mf_data_batch_list) == 2:
            pseudo_warp_scales, pseudo_warp_mask_scales = self.generate_pseudo_pair(
                mf_data_batch_list[0], mf_data_batch_list[1],
            )

        if self.normalize_cameras and not pseudo_pair_mode:
        ...
        mf_data_batch["meta_data"].update(
            {
                ...
                "camera_type": [self.camera_type] * len(view_ids),
            }
        )
        ...
        # NOTE: save output with kosmo format
        if len(self.kosmo_scene_infos) > 0 and self.phase != "train":
            scene = mf_data_batch["meta_data"]['data_info'][0]['scene']
            mf_data_batch["meta_data"]["kosmo_scene_infos"] = self.kosmo_scene_infos[scene]

            frame_ids = [int(data_info['frame_id']) for data_info in mf_data_batch['meta_data']['data_info']]
            view_ids = [int(data_info['view_id']) for data_info in mf_data_batch['meta_data']['data_info']]
            kosmo_frames = dict()
            for frame_id, view_id in zip(frame_ids, view_ids):
            ...
            mf_data_batch["meta_data"]["kosmo_frames"] = kosmo_frames
        ...
        if self.track_points_nums > 0:
            ...
            if self.sample_query_uvt:
                query_points_uvt = sample_query_points_uvt(tracks[None], track_vis_masks[None])[0]
        ...
        if pseudo_warp_scales is not None:
            mf_data_batch["warp_scales"] = {
                k: v.unsqueeze(0) for k, v in pseudo_warp_scales.items()
            }
            mf_data_batch["warp_mask_scales"] = {
                k: v.unsqueeze(0).float() for k, v in pseudo_warp_mask_scales.items()
            }
            mf_data_batch["meta_data"]["pseudo_pair"] = True
            mf_data_batch["meta_data"]["pair_idx"] = [(0, 1)]
        ...
        if self.debug:
            ...
            if pseudo_warp_scales is not None:
                self._debug_pseudo_pair_warp(idx, mf_data_batch, pseudo_warp_scales)
    ...
    def check_data_extrinsics(self, data_dict, eps=5e-3):
        if data_dict["curr_extrinsics"] is None:
            return True
    ...
    def _get_normalized_black(self):
        """Return normalized black pixel value [C, 1] if Normalize transform exists, else None."""
        if not hasattr(self, "data_transforms"):
            return None
        transforms = getattr(self.data_transforms, "transforms", [])
        for t in transforms:
            if hasattr(t, "mean") and hasattr(t, "std"):
                black = -t.mean.squeeze() / t.std.squeeze()
                return black[:, None]
        return None

    def _build_pseudo_pair_transform(self, H, W):
        """Sample a random 2D affine transform and return the 3x3 matrix (pixel space)."""
        conf = self.pseudo_pair_transforms
        T = np.eye(3, dtype=np.float64)

        if random.random() < conf.get("horizontal_flip_p", 0):
            T = np.array([[-1, 0, W - 1], [0, 1, 0], [0, 0, 1]], dtype=np.float64) @ T
        if random.random() < conf.get("vertical_flip_p", 0):
            T = np.array([[1, 0, 0], [0, -1, H - 1], [0, 0, 1]], dtype=np.float64) @ T

        if random.random() < conf.get("scale_p", 0):
            lo, hi = conf.get("scale_range", [0.8, 1.2])
            s = random.uniform(lo, hi)
            cx, cy = W / 2.0, H / 2.0
            S = np.array([[s, 0, cx * (1 - s)], [0, s, cy * (1 - s)], [0, 0, 1]], dtype=np.float64)
            T = S @ T

        if random.random() < conf.get("rotate_p", 0):
            max_deg = conf.get("rotate_max_deg", 15)
        ...
        if random.random() < conf.get("translate_p", 0):
            mx, my = conf.get("translate_range", [0.1, 0.1])
            tx = random.uniform(-mx * W, mx * W)
            ty = random.uniform(-my * H, my * H)
            Sh = np.array([[1, 0, tx], [0, 1, ty], [0, 0, 1]], dtype=np.float64)
            T = Sh @ T

        return T

    def _compute_warp_from_transform(self, T, H, W, h_out, w_out):
        """Compute normalised warp grid and validity mask for a given transform T.

        Args:
            T: 3x3 forward transform (pixel space, frame0 -> frame1).
            H, W: full-resolution spatial size (used for pixel <-> norm conversion).
            h_out, w_out: output grid spatial size (for multi-scale support).
        Returns:
            warp: [h_out, w_out, 2] normalised coordinates in frame1.
            warp_mask: [h_out, w_out] bool validity mask.
        """
        T_t = torch.from_numpy(T).float()
        gy, gx = torch.meshgrid(
            torch.linspace(-1 + 1 / h_out, 1 - 1 / h_out, h_out),
            torch.linspace(-1 + 1 / w_out, 1 - 1 / w_out, w_out),
            indexing="ij",
        )
        px = (gx + 1) * W / 2
        py = (gy + 1) * H / 2

        px_new = T_t[0, 0] * px + T_t[0, 1] * py + T_t[0, 2]
        py_new = T_t[1, 0] * px + T_t[1, 1] * py + T_t[1, 2]

        warp = torch.stack([2 * px_new / W - 1, 2 * py_new / H - 1], dim=-1)
        warp_mask = (px_new >= 0) & (px_new < W) & (py_new >= 0) & (py_new < H)
        return warp, warp_mask

    def generate_pseudo_pair(self, data0, data1):
        """Generate a pseudo second frame from data0 by applying a random 2D transform.

        Modifies *data1* in-place (image, depth_mask).
        Returns warp_scales and warp_mask_scales dicts keyed by scale.
        Both frames have depth / pointmap / normal zeroed out.
        """
        C, H, W = data0["image"].shape

        T = self._build_pseudo_pair_transform(H, W)
        T_inv = np.linalg.inv(T)
        T_inv_t = torch.from_numpy(T_inv).float()

        inv_gy, inv_gx = torch.meshgrid(
        ...
        inv_px = (inv_gx + 1) * W / 2
        inv_py = (inv_gy + 1) * H / 2
        src_px = T_inv_t[0, 0] * inv_px + T_inv_t[0, 1] * inv_py + T_inv_t[0, 2]
        src_py = T_inv_t[1, 0] * inv_px + T_inv_t[1, 1] * inv_py + T_inv_t[1, 2]
        inv_grid = torch.stack([2 * src_px / W - 1, 2 * src_py / H - 1], dim=-1)[None]

        fill_value = self._get_normalized_black()
        data1["image"] = F.grid_sample(
            data0["image"][None], inv_grid, mode="bilinear", padding_mode="zeros", align_corners=False,
        )[0]
        oob_mask = (inv_grid[0, ..., 0].abs() > 1) | (inv_grid[0, ..., 1].abs() > 1)
        if fill_value is not None:
            data1["image"][:, oob_mask] = fill_value

        if "depth_mask" in data0:
            dm = data0["depth_mask"].float()
            if dm.dim() == 2:
                dm = dm[None]
            data1["depth_mask"] = (
                F.grid_sample(dm[None], inv_grid, mode="nearest", padding_mode="zeros", align_corners=False)[0]
                > 0.5
            )
            if data0["depth_mask"].dim() == 2:
                data1["depth_mask"] = data1["depth_mask"].squeeze(0)

        if "rgb_fisheye_mask" in data0 and data0["rgb_fisheye_mask"] is not None:
            fm = data0["rgb_fisheye_mask"].float()
            if fm.dim() == 2:
                fm = fm[None]
            data1["rgb_fisheye_mask"] = (
                F.grid_sample(fm[None], inv_grid, mode="nearest", padding_mode="zeros", align_corners=False)[0]
                > 0.5
            )
            if data0["rgb_fisheye_mask"].dim() == 2:
                data1["rgb_fisheye_mask"] = data1["rgb_fisheye_mask"].squeeze(0)

        if "invalid_mask" in data0 and data0["invalid_mask"] is not None:
            im = data0["invalid_mask"].float()
            if im.dim() == 2:
                im = im[None]
            warped = (
                F.grid_sample(im[None], inv_grid, mode="nearest", padding_mode="zeros", align_corners=False)[0]
                > 0.5
            )
            warped = warped | oob_mask[None]
            data1["invalid_mask"] = warped
            if data0["invalid_mask"].dim() == 2:
                data1["invalid_mask"] = data1["invalid_mask"].squeeze(0)

        for key in ("depth", "pointmap", "pointmap_reff", "normal"):
            if key in data0:
                data0[key] = torch.zeros_like(data0[key])
            if key in data1:
                data1[key] = torch.zeros_like(data1[key])

        if "extrinsics" in data1:
            data1["extrinsics"] = torch.eye(4, dtype=data1["extrinsics"].dtype)
        if "extrinsics_reff" in data1:
            data1["extrinsics_reff"] = torch.eye(4, dtype=data1["extrinsics_reff"].dtype)

        warp_scales = {}
        warp_mask_scales = {}
        fisheye_mask_0 = data0.get("rgb_fisheye_mask", None)
        invalid_mask_0 = data0.get("invalid_mask", None)
        for scale in self.pseudo_pair_match_scales:
            h_s, w_s = int(H / scale), int(W / scale)
            warp_s, mask_s = self._compute_warp_from_transform(T, H, W, h_s, w_s)
            if fisheye_mask_0 is not None:
                fm = fisheye_mask_0.float()
                if fm.dim() == 2:
                    fm = fm[None, None]
                elif fm.dim() == 3:
                    fm = fm[None]
                fm_s = F.interpolate(fm, size=(h_s, w_s), mode="nearest")[0, 0] > 0.5
                mask_s = mask_s & fm_s
            if invalid_mask_0 is not None:
                im = invalid_mask_0.float()
                if im.dim() == 2:
                    im = im[None, None]
                elif im.dim() == 3:
                    im = im[None]
                im_s = F.interpolate(im, size=(h_s, w_s), mode="nearest")[0, 0] > 0.5
                mask_s = mask_s & ~im_s
            warp_scales[scale] = warp_s
            warp_mask_scales[scale] = mask_s

        return warp_scales, warp_mask_scales

    def _debug_pseudo_pair_warp(self, idx, mf_data_batch, warp_scales):
        """Save a visual check of the pseudo-pair warp at scale 1."""
        from hAlgorithm.modules.models2.external.romav2.utils.utils import tensor_to_pil

        scale_key = min(warp_scales.keys())
        warp = warp_scales[scale_key][None]
        N, _, H, W = mf_data_batch["image"].shape
        x1 = mf_data_batch["image"][0]
        x2 = mf_data_batch["image"][1]
        warp_mask = mf_data_batch["warp_mask_scales"][scale_key][0]

        warp_im = F.grid_sample(
            x2[None], warp, mode="bilinear", align_corners=False,
        )[0]
        white_im = torch.ones_like(x1)
        ...
        if isinstance(self.data_path, (list, tuple)):
            ...
            data_path = self.data_path
        outdir = os.path.join(
            "./debug/dataset/", self.name,
            os.path.splitext(os.path.basename(data_path))[0],
            "pseudo_pair_warp", f"{idx:04d}",
        )
        os.makedirs(outdir, exist_ok=True)
        path = os.path.join(outdir, "warp_AB.png")
        tensor_to_pil(vis_im).save(path)
        logging.info(f"pseudo pair warp debug saved to {path}")
    ...
    def normalize_cameras_base_first(self, datas):
        ...
        if "extrinsics" in datas[0]:
            ...
            if self.with_global_scale:
                ...
                mask = np.concatenate(
                    [data["depth_mask"].reshape(-1).numpy() for data in datas],
                    axis=0,
                )
                global_scale = np.mean(np.linalg.norm(points[mask], axis=1))
                ...
                points = torch.cat(
                    [data["pointmap_reff"].reshape(3, -1).permute(1, 0) for data in datas],
                    dim=0,
                )
                mask = torch.cat(
                    [data["depth_mask"].reshape(-1) for data in datas],
                    dim=0,
                )
                global_scale = torch.max(torch.norm(points[mask], dim=1))
        ...
        if "prompt_extrinsics" in datas[0]:
            first_extrinsics_inv = datas[0]["prompt_extrinsics"].inverse()

            sparse_pointmap_max_range = []
            for data in datas:
                # Compute new extrinsics relative to the first camera
                data["prompt_extrinsics_reff"] = data["prompt_extrinsics"] @ first_extrinsics_inv
    ...
    def create_track_points(self, idx, data_batch_list, image_show_list):
        ...
        final_tracks, final_vis_masks, final_pos_masks = build_tracks_by_depth(
            ...
            sample_frames=self.track_frame_num,
            debug=self.debug,
        )
        ...
        # set invisible track to -1 to avoid inf
        final_tracks = final_tracks.clamp(-9999, 9999)
...
if __name__ == "__main__":
    ...
    path = "hAlgorithm/configs/fisheye_v1.0/svd_normal/train_svd_0120.yaml"
    # path = "hAlgorithm/configs/fisheye_v1.0/kosmo_fisheye.yaml"
    ...
    for config in dataset_configs:
        ...
        base_config = dict(
            ...
            test_transforms=[
                ...
                #     # is_lidar=True,
                # ),
                dict(type="hAlgorithm.datasets.transforms.transforms.RandomVerticalFlip", prob=1.0),
            ...
            mf_to_sf=True,
            debug=True,
            fisheye_crop_pixel_pointmap=0,
            name="fisheye_0122_flip_debug",
            clip_maxlen=4,
            clip_sampler=dict(
