...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(out_all_map, colors_precomp, all_maps, means3D, scales, rotations, cov3Ds_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer, cam_q_w2c)
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_out_observe, grad_out_all_map, grad_out_plane_depth):
        ...
        all_map_pixels, colors_precomp, all_maps, means3D, scales, rotations, cov3Ds_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer, cam_q_w2c = ctx.saved_tensors
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                grad_means2D, grad_means2D_abs, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, gard_all_map, grad_cam_q_w2c, grad_cam_t_w2c = _C.rasterize_gaussians_backward(*args)
            except Exception as ex:
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, means2D_abs, opacities, shs = None, colors_precomp = None, 
                scales = None, rotations = None, cov3D_precomp = None, all_map=None, cam_q_w2c = None, cam_t_w2c = None):
        ...
        if cam_q_w2c is None:
            cam_q_w2c = torch.Tensor([])
        if cam_t_w2c is None:
            cam_t_w2c = torch.Tensor([])
